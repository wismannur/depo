...
class clogin extends cemployees {
	...
	function __construct() {
		...
		if (!isset($GLOBALS["employees"])) $GLOBALS["employees"] = &$this;
	...
	function getKodeDepo() {
		$.ajax({
			...
			data: {
				...
				as: 'json',
				db: 'db_inventory_depo'
			},
			success: function (data) {
				let result = JSON.parse(data)
				localStorage.setItem("kode_depo", result[0].kode_depo)
				getNamaDepo()
			}
		});
	};
	...
	function getNamaDepo() {
		$.ajax({
			...
			data: {
				...
				as: 'json',
				db: 'db_inventory_pusat'
			},
			success: function (data) {
				let result = JSON.parse(data)
				localStorage.setItem("nama_depo", result[0].nama_depo)
			}
		});
	};
