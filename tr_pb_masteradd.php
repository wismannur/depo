...
$tr_pb_master_add = NULL; // Initialize page object first

class ctr_pb_master_add extends ctr_pb_master {
	...
	// Page object name
	var $PageObjName = 'tr_pb_master_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["pb_id"] != "") {
				...
				$this->setKey("pb_id", $this->pb_id->CurrentValue); // Set up key
			} else {
				$this->setKey("pb_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "tr_pb_masterview.php")
	...
	function LoadDefaultValues() {
		...
		$this->pb_number->CurrentValue = NULL;
		$this->pb_number->OldValue = $this->pb_number->CurrentValue;
		$this->pb_date->CurrentValue = ew_CurrentDate();
		$this->kode_depo->CurrentValue = NULL;
		$this->kode_depo->OldValue = $this->kode_depo->CurrentValue;
		$this->pb_notes->CurrentValue = NULL;
		$this->pb_notes->OldValue = $this->pb_notes->CurrentValue;
	...
	function NewRow() {
		...
		$row['pb_number'] = $this->pb_number->CurrentValue;
		$row['pb_date'] = $this->pb_date->CurrentValue;
		$row['kode_depo'] = $this->kode_depo->CurrentValue;
		$row['pb_notes'] = $this->pb_notes->CurrentValue;
	...
	function ValidateForm() {
		...
		if (in_array("tr_pb_item", $DetailTblVar) && $GLOBALS["tr_pb_item"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		// pb_number
		$this->pb_number->SetDbValueDef($rsnew, $this->pb_number->CurrentValue, NULL, FALSE);

		// pb_date
		$this->pb_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->pb_date->CurrentValue, 0), NULL, FALSE);

		// kode_depo
		$this->kode_depo->SetDbValueDef($rsnew, $this->kode_depo->CurrentValue, NULL, FALSE);

		// pb_notes
		$this->pb_notes->SetDbValueDef($rsnew, $this->pb_notes->CurrentValue, NULL, FALSE);
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("tr_pb_item", $DetailTblVar) && $GLOBALS["tr_pb_item"]->DetailAdd) {
				$GLOBALS["tr_pb_item"]->master_id->setSessionValue($this->pb_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["tr_pb_item_grid"])) $GLOBALS["tr_pb_item_grid"] = new ctr_pb_item_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "tr_pb_item"); // Load user level of detail table
				$AddRow = $GLOBALS["tr_pb_item_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["tr_pb_item"]->master_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("tr_pb_item", $DetailTblVar)) {
				...
				if ($GLOBALS["tr_pb_item_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["tr_pb_item_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["tr_pb_item_grid"]->CurrentMode = "add";
					$GLOBALS["tr_pb_item_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($tr_pb_master_add)) $tr_pb_master_add = new ctr_pb_master_add();

// Page init
$tr_pb_master_add->Page_Init();

// Page main
$tr_pb_master_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tr_pb_master_add->Page_Render();
?>
...
var CurrentForm = ftr_pb_masteradd = new ew_Form("ftr_pb_masteradd", "add");

// Validate form
ftr_pb_masteradd.Validate = function() {
...
ftr_pb_masteradd.Form_CustomValidate = 
...
ftr_pb_masteradd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
ftr_pb_masteradd.Lists["x_kode_depo"] = {"LinkField":"x_kode_depo","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama_depo","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_depo"};
ftr_pb_masteradd.Lists["x_kode_depo"].Data = "<?php echo $tr_pb_master_add->kode_depo->LookupFilterQuery(FALSE, "add") ?>";
...
<?php $tr_pb_master_add->ShowPageHeader(); ?>
<?php
$tr_pb_master_add->ShowMessage();
?>
<form name="ftr_pb_masteradd" id="ftr_pb_masteradd" class="<?php echo $tr_pb_master_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tr_pb_master_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tr_pb_master_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tr_pb_master">
<input type="hidden" name="a_add" id="a_add" value="A">
<input type="hidden" name="modal" value="<?php echo intval($tr_pb_master_add->IsModal) ?>">
<div id="tpd_tr_pb_masteradd" class="ewCustomTemplate"></div>
<script id="tpm_tr_pb_masteradd" type="text/html">
<div id="ct_tr_pb_master_add"><div class="col-sm-12 panel-custom" style="">
	<div class="col-sm-3">
...
<?php if (!$tr_pb_master_add->IsMobileOrModal) { ?>
<div class="ewDesktop"><!-- desktop -->
<?php } ?>
<?php if ($tr_pb_master_add->IsMobileOrModal) { ?>
<div class="ewAddDiv hidden"><!-- page* -->
<?php } else { ?>
<table id="tbl_tr_pb_masteradd" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable hidden"><!-- table* -->
<?php } ?>
<?php if ($tr_pb_master->pb_number->Visible) { // pb_number ?>
<?php if ($tr_pb_master_add->IsMobileOrModal) { ?>
	<div id="r_pb_number" class="form-group">
		<label id="elh_tr_pb_master_pb_number" for="x_pb_number" class="<?php echo $tr_pb_master_add->LeftColumnClass ?>"><script id="tpc_tr_pb_master_pb_number" class="tr_pb_masteradd" type="text/html"><span><?php echo $tr_pb_master->pb_number->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_pb_master_add->RightColumnClass ?>"><div<?php echo $tr_pb_master->pb_number->CellAttributes() ?>>
<script id="tpx_tr_pb_master_pb_number" class="tr_pb_masteradd" type="text/html">
...
<?php } else { ?>
	<tr id="r_pb_number">
		<td class="col-sm-2"><span id="elh_tr_pb_master_pb_number"><script id="tpc_tr_pb_master_pb_number" class="tr_pb_masteradd" type="text/html"><span><?php echo $tr_pb_master->pb_number->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_pb_master_add->IsMobileOrModal) { ?>
	<div id="r_pb_date" class="form-group">
		<label id="elh_tr_pb_master_pb_date" for="x_pb_date" class="<?php echo $tr_pb_master_add->LeftColumnClass ?>"><script id="tpc_tr_pb_master_pb_date" class="tr_pb_masteradd" type="text/html"><span><?php echo $tr_pb_master->pb_date->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_pb_master_add->RightColumnClass ?>"><div<?php echo $tr_pb_master->pb_date->CellAttributes() ?>>
<script id="tpx_tr_pb_master_pb_date" class="tr_pb_masteradd" type="text/html">
...
<script type="text/html" class="tr_pb_masteradd_js">
ew_CreateDateTimePicker("ftr_pb_masteradd", "x_pb_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php echo $tr_pb_master->pb_date->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_pb_date">
		<td class="col-sm-2"><span id="elh_tr_pb_master_pb_date"><script id="tpc_tr_pb_master_pb_date" class="tr_pb_masteradd" type="text/html"><span><?php echo $tr_pb_master->pb_date->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_pb_master_add->IsMobileOrModal) { ?>
	<div id="r_kode_depo" class="form-group">
		<label id="elh_tr_pb_master_kode_depo" for="x_kode_depo" class="<?php echo $tr_pb_master_add->LeftColumnClass ?>"><script id="tpc_tr_pb_master_kode_depo" class="tr_pb_masteradd" type="text/html"><span><?php echo $tr_pb_master->kode_depo->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_pb_master_add->RightColumnClass ?>"><div<?php echo $tr_pb_master->kode_depo->CellAttributes() ?>>
<script id="tpx_tr_pb_master_kode_depo" class="tr_pb_masteradd" type="text/html">
...
<?php } else { ?>
	<tr id="r_kode_depo">
		<td class="col-sm-2"><span id="elh_tr_pb_master_kode_depo"><script id="tpc_tr_pb_master_kode_depo" class="tr_pb_masteradd" type="text/html"><span><?php echo $tr_pb_master->kode_depo->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_pb_master_add->IsMobileOrModal) { ?>
	<div id="r_pb_notes" class="form-group">
		<label id="elh_tr_pb_master_pb_notes" for="x_pb_notes" class="<?php echo $tr_pb_master_add->LeftColumnClass ?>"><script id="tpc_tr_pb_master_pb_notes" class="tr_pb_masteradd" type="text/html"><span><?php echo $tr_pb_master->pb_notes->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_pb_master_add->RightColumnClass ?>"><div<?php echo $tr_pb_master->pb_notes->CellAttributes() ?>>
<script id="tpx_tr_pb_master_pb_notes" class="tr_pb_masteradd" type="text/html">
...
<?php } else { ?>
	<tr id="r_pb_notes">
		<td class="col-sm-2"><span id="elh_tr_pb_master_pb_notes"><script id="tpc_tr_pb_master_pb_notes" class="tr_pb_masteradd" type="text/html"><span><?php echo $tr_pb_master->pb_notes->FldCaption() ?></span></script></span></td>
...
<?php } ?>
<?php
	if (in_array("tr_pb_item", explode(",", $tr_pb_master->getCurrentDetailTable())) && $tr_pb_item->DetailAdd) {
?>
...
<?php } ?>
<?php if (!$tr_pb_master_add->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $tr_pb_master_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tr_pb_master_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
...
ew_ApplyTemplate("tpd_tr_pb_masteradd", "tpm_tr_pb_masteradd", "tr_pb_masteradd", "<?php echo $tr_pb_master->CustomExport ?>", ewVar.templateData.rows[0]);
jQuery("script.tr_pb_masteradd_js").each(function(){ew_AddScript(this.text);});
</script>
<script type="text/javascript">
ftr_pb_masteradd.Init();
</script>
<?php
$tr_pb_master_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tr_pb_master_add->Page_Terminate();
?>
