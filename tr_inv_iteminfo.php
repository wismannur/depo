...
//
// Table class for tr_inv_item
//
class ctr_inv_item extends cTable {
	var $master_id;
	var $row_id;
	var $item_id;
	var $item_code;
	...
	function __construct() {
		...
		$this->TableVar = 'tr_inv_item';
		$this->TableName = 'tr_inv_item';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tr_inv_item`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// master_id
		$this->master_id = new cField('tr_inv_item', 'tr_inv_item', 'x_master_id', 'master_id', '`master_id`', '`master_id`', 3, -1, FALSE, '`master_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// row_id
		$this->row_id = new cField('tr_inv_item', 'tr_inv_item', 'x_row_id', 'row_id', '`row_id`', '`row_id`', 3, -1, FALSE, '`row_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// item_id
		$this->item_id = new cField('tr_inv_item', 'tr_inv_item', 'x_item_id', 'item_id', '`item_id`', '`item_id`', 3, -1, FALSE, '`item_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// item_code
		$this->item_code = new cField('tr_inv_item', 'tr_inv_item', 'x_item_code', 'item_code', '`item_code`', '`item_code`', 200, -1, FALSE, '`item_code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->item_code->Sortable = TRUE; // Allow sort
		$this->fields['item_code'] = &$this->item_code;

		// item_name
		$this->item_name = new cField('tr_inv_item', 'tr_inv_item', 'x_item_name', 'item_name', '`item_name`', '`item_name`', 200, -1, FALSE, '`item_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
		$this->item_name->Sortable = TRUE; // Allow sort
		$this->fields['item_name'] = &$this->item_name;

		// udet_id
		$this->udet_id = new cField('tr_inv_item', 'tr_inv_item', 'x_udet_id', 'udet_id', '`udet_id`', '`udet_id`', 3, -1, FALSE, '`udet_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// item_qty
		$this->item_qty = new cField('tr_inv_item', 'tr_inv_item', 'x_item_qty', 'item_qty', '`item_qty`', '`item_qty`', 5, -1, FALSE, '`item_qty`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// item_price
		$this->item_price = new cField('tr_inv_item', 'tr_inv_item', 'x_item_price', 'item_price', '`item_price`', '`item_price`', 5, -1, FALSE, '`item_price`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// item_disc
		$this->item_disc = new cField('tr_inv_item', 'tr_inv_item', 'x_item_disc', 'item_disc', '`item_disc`', '`item_disc`', 5, -1, FALSE, '`item_disc`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// item_amt
		$this->item_amt = new cField('tr_inv_item', 'tr_inv_item', 'x_item_amt', 'item_amt', '`item_qty`*(`item_price`-(\'item_price\'*`item_disc`/100))', '`item_qty`*(`item_price`-(\'item_price\'*`item_disc`/100))', 5, -1, FALSE, '`item_qty`*(`item_price`-(\'item_price\'*`item_disc`/100))', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// unit_id
		$this->unit_id = new cField('tr_inv_item', 'tr_inv_item', 'x_unit_id', 'unit_id', '`unit_id`', '`unit_id`', 200, -1, FALSE, '`unit_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "tr_inv_master") {
	...
	// Master filter
	function SqlMasterFilter_tr_inv_master() {
		return "`inv_id`=@inv_id@";
	}

	// Detail filter
	function SqlDetailFilter_tr_inv_master() {
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tr_inv_item`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "tr_inv_itemedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "tr_inv_itemadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tr_inv_itemview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tr_inv_itemview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "tr_inv_itemadd.php?" . $this->UrlParm($parm);
		else
			$url = "tr_inv_itemadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("tr_inv_itemedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("tr_inv_itemadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tr_inv_itemdelete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "tr_inv_master" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) {
			...
			if (!$Doc->ExportCustom) {
				...
				if ($this->master_id->Exportable) $Doc->ExportAggregate($this->master_id, '');
				if ($this->row_id->Exportable) $Doc->ExportAggregate($this->row_id, '');
				if ($this->item_id->Exportable) $Doc->ExportAggregate($this->item_id, '');
				if ($this->item_code->Exportable) $Doc->ExportAggregate($this->item_code, '');
				if ($this->item_name->Exportable) $Doc->ExportAggregate($this->item_name, '');
				if ($this->udet_id->Exportable) $Doc->ExportAggregate($this->udet_id, '');
				if ($this->item_qty->Exportable) $Doc->ExportAggregate($this->item_qty, '');
				if ($this->item_price->Exportable) $Doc->ExportAggregate($this->item_price, '');
				if ($this->item_disc->Exportable) $Doc->ExportAggregate($this->item_disc, '');
				if ($this->item_amt->Exportable) $Doc->ExportAggregate($this->item_amt, 'TOTAL');
				if ($this->unit_id->Exportable) $Doc->ExportAggregate($this->unit_id, '');
				$Doc->EndExportRow();
			}
		}
		if (!$Doc->ExportCustom) {
	...
	function GetAutoFill($id, $val) {
		...
		if (preg_match('/^x(\d)*_item_id$/', $id)) {
			...
			$sSqlWrk = "SELECT `product_code` AS FIELD0, `product_name` AS FIELD1, `unit_id` AS FIELD2, `unit_price` AS FIELD3, `udet_id` AS FIELD4 FROM `view_products_unit_price`";
	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		//echo "Row Inserted"
	//$inv_amt_item = ew_ExecuteScalar("SELECT SUM((item_price-(item_price*item_disc/100))*item_qty) FROM tr_inv_item WHERE master_id = ".$rsnew["master_id"]."");
	//ew_Execute("UPDATE tr_inv_master SET inv_amt = ".$inv_amt_item." WHERE inv_id = ".$rsnew["master_id"]."");
