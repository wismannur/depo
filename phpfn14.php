...
function &DbHelper($dbid = "") {
	if ($dbid == "db_inventory_pusat" || $dbid === 1) // db_inventory_pusat
		$dbclass = "cdb_inventory_pusat_db";
	else // DB
		$dbclass = "cdb_inventory_depo_db";
...
function CurrentProjectID() {
	...
	return "{810F9B4D-139E-435A-ABF7-3C676CFEC5C5}";
}
...
class cExportExcel5 extends cExportBase {
	...
	static $WidthMultiplier = 0.15; // Cell width multipler for image fields
	static $HeightMultiplier = 0.8; // Row height multipler for image fields
	static $MaxImageWidth = 400; // Max image width <= 400 is recommended
	...
	function ExportValueBy(&$fld, $col, $row) {
		...
		if ($fld->FldViewTag == "IMAGE") { // Image
			...
			if (!$fld->UploadMultiple || !ew_ContainsStr($imagefn, ",")) {
				$fn = ew_ServerMapPath($imagefn, TRUE);
				...
				if ($totalW > 0) // Width
					$sheet->getColumnDimension($letter)->setAutoSize(FALSE)->setWidth($totalW * self::$WidthMultiplier); // Set column width, no auto size
				if ($maxH > 0) // Height
...
class cAdvancedSecurity {
	...
	function User_Validated(&$rs) {
		...
		// Menyimpan nilai kode depo untuk keperluan filter table dan insert/udpate	
		//if (IsLoggedIn() && !IsSysAdmin()) {
	   	//} else {
	   	//};

		$_SESSION['KodeDepo'] = $rs['kode_depo'];
		$_SESSION['NamaDepo'] = GetNamaDepo($rs['kode_depo']);		
		$_SESSION['userID'] = CurrentUserID();
		$_SESSION['userName'] = CurrentUserName();
	   	$_SESSION['default_jatuh_tempo'] = 7;
	}
