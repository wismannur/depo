...
$tr_inv_master_delete = NULL; // Initialize page object first

class ctr_inv_master_delete extends ctr_inv_master {
	...
	// Page object name
	var $PageObjName = 'tr_inv_master_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("tr_inv_masterlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in tr_inv_master class, tr_inv_masterinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("tr_inv_masterlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($tr_inv_master_delete)) $tr_inv_master_delete = new ctr_inv_master_delete();

// Page init
$tr_inv_master_delete->Page_Init();

// Page main
$tr_inv_master_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tr_inv_master_delete->Page_Render();
?>
...
var CurrentForm = ftr_inv_masterdelete = new ew_Form("ftr_inv_masterdelete", "delete");

// Form_CustomValidate event
ftr_inv_masterdelete.Form_CustomValidate = 
...
ftr_inv_masterdelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
ftr_inv_masterdelete.Lists["x_customer_id"] = {"LinkField":"x_customer_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_customer_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_customer"};
ftr_inv_masterdelete.Lists["x_customer_id"].Data = "<?php echo $tr_inv_master_delete->customer_id->LookupFilterQuery(FALSE, "delete") ?>";
ftr_inv_masterdelete.AutoSuggests["x_customer_id"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $tr_inv_master_delete->customer_id->LookupFilterQuery(TRUE, "delete"))) ?>;
ftr_inv_masterdelete.Lists["x_area_id"] = {"LinkField":"x_area_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_area_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_callarea"};
ftr_inv_masterdelete.Lists["x_area_id"].Data = "<?php echo $tr_inv_master_delete->area_id->LookupFilterQuery(FALSE, "delete") ?>";
ftr_inv_masterdelete.Lists["x_sales_id"] = {"LinkField":"x_sales_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_sales_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_salesman"};
ftr_inv_masterdelete.Lists["x_sales_id"].Data = "<?php echo $tr_inv_master_delete->sales_id->LookupFilterQuery(FALSE, "delete") ?>";
ftr_inv_masterdelete.Lists["x_paid[]"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftr_inv_masterdelete.Lists["x_paid[]"].Options = <?php echo json_encode($tr_inv_master_delete->paid->Options()) ?>;
...
<?php $tr_inv_master_delete->ShowPageHeader(); ?>
<?php
$tr_inv_master_delete->ShowMessage();
?>
<form name="ftr_inv_masterdelete" id="ftr_inv_masterdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tr_inv_master_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tr_inv_master_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tr_inv_master">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($tr_inv_master_delete->RecKeys as $key) { ?>
...
<?php if ($tr_inv_master->inv_number->Visible) { // inv_number ?>
		<th class="<?php echo $tr_inv_master->inv_number->HeaderCellClass() ?>"><span id="elh_tr_inv_master_inv_number" class="tr_inv_master_inv_number"><?php echo $tr_inv_master->inv_number->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_inv_master->inv_date->Visible) { // inv_date ?>
		<th class="<?php echo $tr_inv_master->inv_date->HeaderCellClass() ?>"><span id="elh_tr_inv_master_inv_date" class="tr_inv_master_inv_date"><?php echo $tr_inv_master->inv_date->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_inv_master->due_date->Visible) { // due_date ?>
		<th class="<?php echo $tr_inv_master->due_date->HeaderCellClass() ?>"><span id="elh_tr_inv_master_due_date" class="tr_inv_master_due_date"><?php echo $tr_inv_master->due_date->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_inv_master->customer_id->Visible) { // customer_id ?>
		<th class="<?php echo $tr_inv_master->customer_id->HeaderCellClass() ?>"><span id="elh_tr_inv_master_customer_id" class="tr_inv_master_customer_id"><?php echo $tr_inv_master->customer_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_inv_master->area_id->Visible) { // area_id ?>
		<th class="<?php echo $tr_inv_master->area_id->HeaderCellClass() ?>"><span id="elh_tr_inv_master_area_id" class="tr_inv_master_area_id"><?php echo $tr_inv_master->area_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_inv_master->inv_total->Visible) { // inv_total ?>
		<th class="<?php echo $tr_inv_master->inv_total->HeaderCellClass() ?>"><span id="elh_tr_inv_master_inv_total" class="tr_inv_master_inv_total"><?php echo $tr_inv_master->inv_total->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_inv_master->sales_id->Visible) { // sales_id ?>
		<th class="<?php echo $tr_inv_master->sales_id->HeaderCellClass() ?>"><span id="elh_tr_inv_master_sales_id" class="tr_inv_master_sales_id"><?php echo $tr_inv_master->sales_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_inv_master->paid->Visible) { // paid ?>
		<th class="<?php echo $tr_inv_master->paid->HeaderCellClass() ?>"><span id="elh_tr_inv_master_paid" class="tr_inv_master_paid"><?php echo $tr_inv_master->paid->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$tr_inv_master_delete->RecCnt = 0;
$i = 0;
while (!$tr_inv_master_delete->Recordset->EOF) {
	$tr_inv_master_delete->RecCnt++;
	$tr_inv_master_delete->RowCnt++;

	// Set row properties
	$tr_inv_master->ResetAttrs();
	$tr_inv_master->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$tr_inv_master_delete->LoadRowValues($tr_inv_master_delete->Recordset);

	// Render row
	$tr_inv_master_delete->RenderRow();
?>
	<tr<?php echo $tr_inv_master->RowAttributes() ?>>
<?php if ($tr_inv_master->inv_number->Visible) { // inv_number ?>
		<td<?php echo $tr_inv_master->inv_number->CellAttributes() ?>>
<span id="el<?php echo $tr_inv_master_delete->RowCnt ?>_tr_inv_master_inv_number" class="tr_inv_master_inv_number">
...
<span id="el<?php echo $tr_inv_master_delete->RowCnt ?>_tr_inv_master_inv_date" class="tr_inv_master_inv_date">
...
<span id="el<?php echo $tr_inv_master_delete->RowCnt ?>_tr_inv_master_due_date" class="tr_inv_master_due_date">
...
<span id="el<?php echo $tr_inv_master_delete->RowCnt ?>_tr_inv_master_customer_id" class="tr_inv_master_customer_id">
...
<span id="el<?php echo $tr_inv_master_delete->RowCnt ?>_tr_inv_master_area_id" class="tr_inv_master_area_id">
...
<span id="el<?php echo $tr_inv_master_delete->RowCnt ?>_tr_inv_master_inv_total" class="tr_inv_master_inv_total">
...
<span id="el<?php echo $tr_inv_master_delete->RowCnt ?>_tr_inv_master_sales_id" class="tr_inv_master_sales_id">
...
<span id="el<?php echo $tr_inv_master_delete->RowCnt ?>_tr_inv_master_paid" class="tr_inv_master_paid">
...
<?php
	$tr_inv_master_delete->Recordset->MoveNext();
}
$tr_inv_master_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tr_inv_master_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ftr_inv_masterdelete.Init();
</script>
<?php
$tr_inv_master_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tr_inv_master_delete->Page_Terminate();
?>
