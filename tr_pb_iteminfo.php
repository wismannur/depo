...
//
// Table class for tr_pb_item
//
class ctr_pb_item extends cTable {
	var $row_id;
	var $master_id;
	var $item_id;
	var $item_code;
	...
	function __construct() {
		...
		$this->TableVar = 'tr_pb_item';
		$this->TableName = 'tr_pb_item';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tr_pb_item`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// row_id
		$this->row_id = new cField('tr_pb_item', 'tr_pb_item', 'x_row_id', 'row_id', '`row_id`', '`row_id`', 3, -1, FALSE, '`row_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// master_id
		$this->master_id = new cField('tr_pb_item', 'tr_pb_item', 'x_master_id', 'master_id', '`master_id`', '`master_id`', 3, -1, FALSE, '`master_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// item_id
		$this->item_id = new cField('tr_pb_item', 'tr_pb_item', 'x_item_id', 'item_id', '`item_id`', '`item_id`', 3, -1, FALSE, '`item_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// item_code
		$this->item_code = new cField('tr_pb_item', 'tr_pb_item', 'x_item_code', 'item_code', '`item_code`', '`item_code`', 200, -1, FALSE, '`item_code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->item_code->Sortable = TRUE; // Allow sort
		$this->fields['item_code'] = &$this->item_code;

		// item_name
		$this->item_name = new cField('tr_pb_item', 'tr_pb_item', 'x_item_name', 'item_name', '`item_name`', '`item_name`', 200, -1, FALSE, '`item_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->item_name->Sortable = TRUE; // Allow sort
		$this->fields['item_name'] = &$this->item_name;

		// udet_id
		$this->udet_id = new cField('tr_pb_item', 'tr_pb_item', 'x_udet_id', 'udet_id', '`udet_id`', '`udet_id`', 3, -1, FALSE, '`udet_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// item_qty
		$this->item_qty = new cField('tr_pb_item', 'tr_pb_item', 'x_item_qty', 'item_qty', '`item_qty`', '`item_qty`', 5, -1, FALSE, '`item_qty`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// unit_id
		$this->unit_id = new cField('tr_pb_item', 'tr_pb_item', 'x_unit_id', 'unit_id', '`unit_id`', '`unit_id`', 200, -1, FALSE, '`unit_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "tr_pb_master") {
			if ($this->master_id->getSessionValue() <> "")
				$sMasterFilter .= "`pb_id`=" . ew_QuotedValue($this->master_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_tr_pb_master() {
		return "`pb_id`=@pb_id@";
	}

	// Detail filter
	function SqlDetailFilter_tr_pb_master() {
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tr_pb_item`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "tr_pb_itemedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "tr_pb_itemadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tr_pb_itemview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tr_pb_itemview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "tr_pb_itemadd.php?" . $this->UrlParm($parm);
		else
			$url = "tr_pb_itemadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("tr_pb_itemedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("tr_pb_itemadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tr_pb_itemdelete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "tr_pb_master" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
			$url .= (strpos($url, "?") !== FALSE ? "&" : "?") . EW_TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_pb_id=" . urlencode($this->master_id->CurrentValue);
		}
		return $url;
	}
