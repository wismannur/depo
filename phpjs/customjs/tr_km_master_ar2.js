...
$(document).ready(function(){
    ...
    function jumlahBayar() {
        let jumlahField = totalField('#tbl_tr_km_item_ar2grid')
        var countJumlahBayar = 0
        for (let i = 0; i < jumlahField; i++) {
            let checkDataIndex = $('#tbl_tr_km_item_ar2grid').find('tbody tr').eq(i).attr('data-rowindex')
            let paid_amt = $('#tbl_tr_km_item_ar2grid').find('tbody > tr').eq(i).find('td').eq(5).find('span > input').eq(0)
    ...
    function calculateRow() {
        ...
        for (let i = 0; i < jumlahField; i++) {
            let checkDataIndex = $('#tbl_tr_km_item_ar2grid').find('tbody tr').eq(i).attr('data-rowindex')
            let inv_amt = $('#tbl_tr_km_item_ar2grid').find('tbody > tr').eq(i).find('td').eq(4).find('span > input').eq(0)
            let paid_amt = $('#tbl_tr_km_item_ar2grid').find('tbody > tr').eq(i).find('td').eq(5).find('span > input').eq(0)
            
            let customer_id = $('#tbl_tr_km_item_ar2grid').find('tbody > tr').eq(i).find('td').eq(1).find('span').eq(0)
            let btnCloseOutlet = $('#tbl_tr_km_item_ar2grid').find('tbody > tr').eq(i).find('td').eq(1).find('span > div > span').eq(1)
            let listOutlet = $('#tbl_tr_km_item_ar2grid').find('tbody > tr').eq(i).find('td').eq(1).find('span > div > div > div > div')
            
            let inv_number = $('#tbl_tr_km_item_ar2grid').find('tbody > tr').eq(i).find('td').eq(2).find('span').eq(0)
            let btnCloseInvoice = $('#tbl_tr_km_item_ar2grid').find('tbody > tr').eq(i).find('td').eq(2).find('span > div > span').eq(1)
            let listInvoice = $('#tbl_tr_km_item_ar2grid').find('tbody > tr').eq(i).find('td').eq(2).find('span > div > div > div > div')
            if (checkDataIndex > 0) {
                for (let j = 3; j < 6; j++) {
                    $('#tbl_tr_km_item_ar2grid').find('tbody > tr').eq(i).find('td').eq(j).find('span > input').eq(0).css({'text-align': 'right'})
                }
