...
$(document).ready(function(){
	...
	function result(eq) {
		let item_qty = $('#tbl_tr_inv_itemgrid').find('tbody tr').eq(eq).find('td').eq(4).find('input').eq(0).val()
		let item_price = $('#tbl_tr_inv_itemgrid').find('tbody tr').eq(eq).find('td').eq(5).find('input').eq(0).val()
		let item_disc = $('#tbl_tr_inv_itemgrid').find('tbody tr').eq(eq).find('td').eq(6).find('input').eq(0).val()
		let item_amt = $('#tbl_tr_inv_itemgrid').find('tbody tr').eq(eq).find('td').eq(7).find('input').eq(0)
	...
	function countSubTotal() {
		let jumlahField = totalField('#tbl_tr_inv_itemgrid')
		var countSubTotal = 0;
		
		for (let i = 0; i < jumlahField; i++) {
			let checkDataIndex = $('#tbl_tr_inv_itemgrid').find('tbody tr').eq(i).attr('data-rowindex')
			let item_amt = $('#tbl_tr_inv_itemgrid').find('tbody tr').eq(i).find('td').eq(7).find('input').eq(0)
			// console.log(i + ' + ' + checkDataIndex + ' + ' + item_amt.val())
			if ( checkDataIndex != "$rowindex$" && item_amt.val() != "") {
	...
	function calculateRow(){
		...
		for(let i = 0; i < jumlahField; i++){
            let checkDataIndex = $('#tbl_tr_inv_itemgrid').find('tbody tr').eq(i).attr('data-rowindex')
            let item_id_list = $('#tbl_tr_inv_itemgrid').find('tbody tr').eq(i).find('td').eq(1).find('span > span > span').find('input').eq(1)
			let item_qty_list = $('#tbl_tr_inv_itemgrid').find('tbody tr').eq(i).find('td').eq(4).find('input').eq(0)
			let item_price_list = $('#tbl_tr_inv_itemgrid').find('tbody tr').eq(i).find('td').eq(5).find('input').eq(0)
            let item_disc_list = $('#tbl_tr_inv_itemgrid').find('tbody tr').eq(i).find('td').eq(6).find('input').eq(0)
			if (checkDataIndex != "$rowindex$" ) {
                // this is for change align input to align right
                for (let j = 4; j < 8; j++) {
                    $('#tbl_tr_inv_itemgrid').find('tbody tr').eq(i).find('td').eq(j).find('input').eq(0).css({'text-align': 'right'})
                };
	...
	if (urlLink.slice(-4) == "edit") {
		...
		$('div#btnPrint').on('click', function() {
			...
			window.location.replace(URL + 'phpcetak/tr_inv_master_cetak.php?inv_id=' + result + '&v_print=' + $('#choosePrint').val()  + '&kode_depo=' + localStorage.kode_depo )
		})
	};
});