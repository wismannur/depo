...
$tr_sjc_item_delete = NULL; // Initialize page object first

class ctr_sjc_item_delete extends ctr_sjc_item {
	...
	// Page object name
	var $PageObjName = 'tr_sjc_item_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("tr_sjc_itemlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in tr_sjc_item class, tr_sjc_iteminfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("tr_sjc_itemlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($tr_sjc_item_delete)) $tr_sjc_item_delete = new ctr_sjc_item_delete();

// Page init
$tr_sjc_item_delete->Page_Init();

// Page main
$tr_sjc_item_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tr_sjc_item_delete->Page_Render();
?>
...
var CurrentForm = ftr_sjc_itemdelete = new ew_Form("ftr_sjc_itemdelete", "delete");

// Form_CustomValidate event
ftr_sjc_itemdelete.Form_CustomValidate = 
...
ftr_sjc_itemdelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
ftr_sjc_itemdelete.Lists["x_item_id"] = {"LinkField":"x_product_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_product_name","","",""],"ParentFields":[],"ChildFields":["x_udet_id"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"view_products_unit_price"};
ftr_sjc_itemdelete.Lists["x_item_id"].Data = "<?php echo $tr_sjc_item_delete->item_id->LookupFilterQuery(FALSE, "delete") ?>";
ftr_sjc_itemdelete.AutoSuggests["x_item_id"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $tr_sjc_item_delete->item_id->LookupFilterQuery(TRUE, "delete"))) ?>;
ftr_sjc_itemdelete.Lists["x_udet_id"] = {"LinkField":"x_udet_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_unit_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_unit_detail"};
ftr_sjc_itemdelete.Lists["x_udet_id"].Data = "<?php echo $tr_sjc_item_delete->udet_id->LookupFilterQuery(FALSE, "delete") ?>";
...
<?php $tr_sjc_item_delete->ShowPageHeader(); ?>
<?php
$tr_sjc_item_delete->ShowMessage();
?>
<form name="ftr_sjc_itemdelete" id="ftr_sjc_itemdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tr_sjc_item_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tr_sjc_item_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tr_sjc_item">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($tr_sjc_item_delete->RecKeys as $key) { ?>
...
<?php if ($tr_sjc_item->item_id->Visible) { // item_id ?>
		<th class="<?php echo $tr_sjc_item->item_id->HeaderCellClass() ?>"><span id="elh_tr_sjc_item_item_id" class="tr_sjc_item_item_id"><?php echo $tr_sjc_item->item_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_sjc_item->item_code->Visible) { // item_code ?>
		<th class="<?php echo $tr_sjc_item->item_code->HeaderCellClass() ?>"><span id="elh_tr_sjc_item_item_code" class="tr_sjc_item_item_code"><?php echo $tr_sjc_item->item_code->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_sjc_item->item_name->Visible) { // item_name ?>
		<th class="<?php echo $tr_sjc_item->item_name->HeaderCellClass() ?>"><span id="elh_tr_sjc_item_item_name" class="tr_sjc_item_item_name"><?php echo $tr_sjc_item->item_name->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_sjc_item->udet_id->Visible) { // udet_id ?>
		<th class="<?php echo $tr_sjc_item->udet_id->HeaderCellClass() ?>"><span id="elh_tr_sjc_item_udet_id" class="tr_sjc_item_udet_id"><?php echo $tr_sjc_item->udet_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_sjc_item->item_qty->Visible) { // item_qty ?>
		<th class="<?php echo $tr_sjc_item->item_qty->HeaderCellClass() ?>"><span id="elh_tr_sjc_item_item_qty" class="tr_sjc_item_item_qty"><?php echo $tr_sjc_item->item_qty->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_sjc_item->item_price->Visible) { // item_price ?>
		<th class="<?php echo $tr_sjc_item->item_price->HeaderCellClass() ?>"><span id="elh_tr_sjc_item_item_price" class="tr_sjc_item_item_price"><?php echo $tr_sjc_item->item_price->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_sjc_item->unit_id->Visible) { // unit_id ?>
		<th class="<?php echo $tr_sjc_item->unit_id->HeaderCellClass() ?>"><span id="elh_tr_sjc_item_unit_id" class="tr_sjc_item_unit_id"><?php echo $tr_sjc_item->unit_id->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$tr_sjc_item_delete->RecCnt = 0;
$i = 0;
while (!$tr_sjc_item_delete->Recordset->EOF) {
	$tr_sjc_item_delete->RecCnt++;
	$tr_sjc_item_delete->RowCnt++;

	// Set row properties
	$tr_sjc_item->ResetAttrs();
	$tr_sjc_item->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$tr_sjc_item_delete->LoadRowValues($tr_sjc_item_delete->Recordset);

	// Render row
	$tr_sjc_item_delete->RenderRow();
?>
	<tr<?php echo $tr_sjc_item->RowAttributes() ?>>
<?php if ($tr_sjc_item->item_id->Visible) { // item_id ?>
		<td<?php echo $tr_sjc_item->item_id->CellAttributes() ?>>
<span id="el<?php echo $tr_sjc_item_delete->RowCnt ?>_tr_sjc_item_item_id" class="tr_sjc_item_item_id">
...
<span id="el<?php echo $tr_sjc_item_delete->RowCnt ?>_tr_sjc_item_item_code" class="tr_sjc_item_item_code">
...
<span id="el<?php echo $tr_sjc_item_delete->RowCnt ?>_tr_sjc_item_item_name" class="tr_sjc_item_item_name">
...
<span id="el<?php echo $tr_sjc_item_delete->RowCnt ?>_tr_sjc_item_udet_id" class="tr_sjc_item_udet_id">
...
<span id="el<?php echo $tr_sjc_item_delete->RowCnt ?>_tr_sjc_item_item_qty" class="tr_sjc_item_item_qty">
...
<span id="el<?php echo $tr_sjc_item_delete->RowCnt ?>_tr_sjc_item_item_price" class="tr_sjc_item_item_price">
...
<span id="el<?php echo $tr_sjc_item_delete->RowCnt ?>_tr_sjc_item_unit_id" class="tr_sjc_item_unit_id">
...
<?php
	$tr_sjc_item_delete->Recordset->MoveNext();
}
$tr_sjc_item_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tr_sjc_item_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ftr_sjc_itemdelete.Init();
</script>
<?php
$tr_sjc_item_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tr_sjc_item_delete->Page_Terminate();
?>
