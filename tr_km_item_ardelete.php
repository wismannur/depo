...
$tr_km_item_ar_delete = NULL; // Initialize page object first

class ctr_km_item_ar_delete extends ctr_km_item_ar {
	...
	// Page object name
	var $PageObjName = 'tr_km_item_ar_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("tr_km_item_arlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in tr_km_item_ar class, tr_km_item_arinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("tr_km_item_arlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($tr_km_item_ar_delete)) $tr_km_item_ar_delete = new ctr_km_item_ar_delete();

// Page init
$tr_km_item_ar_delete->Page_Init();

// Page main
$tr_km_item_ar_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tr_km_item_ar_delete->Page_Render();
?>
...
var CurrentForm = ftr_km_item_ardelete = new ew_Form("ftr_km_item_ardelete", "delete");

// Form_CustomValidate event
ftr_km_item_ardelete.Form_CustomValidate = 
...
ftr_km_item_ardelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
ftr_km_item_ardelete.Lists["x_customer_id"] = {"LinkField":"x_customer_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_customer_name","","",""],"ParentFields":[],"ChildFields":["x_inv_number"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_customer"};
ftr_km_item_ardelete.Lists["x_customer_id"].Data = "<?php echo $tr_km_item_ar_delete->customer_id->LookupFilterQuery(FALSE, "delete") ?>";
ftr_km_item_ardelete.Lists["x_inv_number"] = {"LinkField":"x_inv_number","Ajax":true,"AutoFill":false,"DisplayFields":["x_inv_number","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tr_inv_master"};
ftr_km_item_ardelete.Lists["x_inv_number"].Data = "<?php echo $tr_km_item_ar_delete->inv_number->LookupFilterQuery(FALSE, "delete") ?>";
...
<?php $tr_km_item_ar_delete->ShowPageHeader(); ?>
<?php
$tr_km_item_ar_delete->ShowMessage();
?>
<form name="ftr_km_item_ardelete" id="ftr_km_item_ardelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tr_km_item_ar_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tr_km_item_ar_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tr_km_item_ar">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($tr_km_item_ar_delete->RecKeys as $key) { ?>
...
<?php if ($tr_km_item_ar->customer_id->Visible) { // customer_id ?>
		<th class="<?php echo $tr_km_item_ar->customer_id->HeaderCellClass() ?>"><span id="elh_tr_km_item_ar_customer_id" class="tr_km_item_ar_customer_id"><?php echo $tr_km_item_ar->customer_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_km_item_ar->inv_number->Visible) { // inv_number ?>
		<th class="<?php echo $tr_km_item_ar->inv_number->HeaderCellClass() ?>"><span id="elh_tr_km_item_ar_inv_number" class="tr_km_item_ar_inv_number"><?php echo $tr_km_item_ar->inv_number->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_km_item_ar->inv_date->Visible) { // inv_date ?>
		<th class="<?php echo $tr_km_item_ar->inv_date->HeaderCellClass() ?>"><span id="elh_tr_km_item_ar_inv_date" class="tr_km_item_ar_inv_date"><?php echo $tr_km_item_ar->inv_date->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_km_item_ar->inv_amt->Visible) { // inv_amt ?>
		<th class="<?php echo $tr_km_item_ar->inv_amt->HeaderCellClass() ?>"><span id="elh_tr_km_item_ar_inv_amt" class="tr_km_item_ar_inv_amt"><?php echo $tr_km_item_ar->inv_amt->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_km_item_ar->paid_amt->Visible) { // paid_amt ?>
		<th class="<?php echo $tr_km_item_ar->paid_amt->HeaderCellClass() ?>"><span id="elh_tr_km_item_ar_paid_amt" class="tr_km_item_ar_paid_amt"><?php echo $tr_km_item_ar->paid_amt->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$tr_km_item_ar_delete->RecCnt = 0;
$i = 0;
while (!$tr_km_item_ar_delete->Recordset->EOF) {
	$tr_km_item_ar_delete->RecCnt++;
	$tr_km_item_ar_delete->RowCnt++;

	// Set row properties
	$tr_km_item_ar->ResetAttrs();
	$tr_km_item_ar->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$tr_km_item_ar_delete->LoadRowValues($tr_km_item_ar_delete->Recordset);

	// Render row
	$tr_km_item_ar_delete->RenderRow();
?>
	<tr<?php echo $tr_km_item_ar->RowAttributes() ?>>
<?php if ($tr_km_item_ar->customer_id->Visible) { // customer_id ?>
		<td<?php echo $tr_km_item_ar->customer_id->CellAttributes() ?>>
<span id="el<?php echo $tr_km_item_ar_delete->RowCnt ?>_tr_km_item_ar_customer_id" class="tr_km_item_ar_customer_id">
...
<span id="el<?php echo $tr_km_item_ar_delete->RowCnt ?>_tr_km_item_ar_inv_number" class="tr_km_item_ar_inv_number">
...
<span id="el<?php echo $tr_km_item_ar_delete->RowCnt ?>_tr_km_item_ar_inv_date" class="tr_km_item_ar_inv_date">
...
<span id="el<?php echo $tr_km_item_ar_delete->RowCnt ?>_tr_km_item_ar_inv_amt" class="tr_km_item_ar_inv_amt">
...
<span id="el<?php echo $tr_km_item_ar_delete->RowCnt ?>_tr_km_item_ar_paid_amt" class="tr_km_item_ar_paid_amt">
...
<?php
	$tr_km_item_ar_delete->Recordset->MoveNext();
}
$tr_km_item_ar_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tr_km_item_ar_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ftr_km_item_ardelete.Init();
</script>
<?php
$tr_km_item_ar_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tr_km_item_ar_delete->Page_Terminate();
?>
