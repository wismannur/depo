...
//
// Table class for tr_retp_item
//
class ctr_retp_item extends cTable {
	var $row_id;
	var $master_id;
	var $item_id;
	var $item_code;
	...
	function __construct() {
		...
		$this->TableVar = 'tr_retp_item';
		$this->TableName = 'tr_retp_item';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tr_retp_item`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// row_id
		$this->row_id = new cField('tr_retp_item', 'tr_retp_item', 'x_row_id', 'row_id', '`row_id`', '`row_id`', 3, -1, FALSE, '`row_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// master_id
		$this->master_id = new cField('tr_retp_item', 'tr_retp_item', 'x_master_id', 'master_id', '`master_id`', '`master_id`', 3, -1, FALSE, '`master_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// item_id
		$this->item_id = new cField('tr_retp_item', 'tr_retp_item', 'x_item_id', 'item_id', '`item_id`', '`item_id`', 3, -1, FALSE, '`item_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// item_code
		$this->item_code = new cField('tr_retp_item', 'tr_retp_item', 'x_item_code', 'item_code', '`item_code`', '`item_code`', 200, -1, FALSE, '`item_code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->item_code->Sortable = TRUE; // Allow sort
		$this->fields['item_code'] = &$this->item_code;

		// item_name
		$this->item_name = new cField('tr_retp_item', 'tr_retp_item', 'x_item_name', 'item_name', '`item_name`', '`item_name`', 200, -1, FALSE, '`item_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
		$this->item_name->Sortable = TRUE; // Allow sort
		$this->fields['item_name'] = &$this->item_name;

		// udet_id
		$this->udet_id = new cField('tr_retp_item', 'tr_retp_item', 'x_udet_id', 'udet_id', '`udet_id`', '`udet_id`', 3, -1, FALSE, '`udet_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// item_qty
		$this->item_qty = new cField('tr_retp_item', 'tr_retp_item', 'x_item_qty', 'item_qty', '`item_qty`', '`item_qty`', 5, -1, FALSE, '`item_qty`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// rcv_qty
		$this->rcv_qty = new cField('tr_retp_item', 'tr_retp_item', 'x_rcv_qty', 'rcv_qty', '`rcv_qty`', '`rcv_qty`', 5, -1, FALSE, '`rcv_qty`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->rcv_qty->Sortable = TRUE; // Allow sort
		$this->rcv_qty->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['rcv_qty'] = &$this->rcv_qty;

		// is_bs
		$this->is_bs = new cField('tr_retp_item', 'tr_retp_item', 'x_is_bs', 'is_bs', '`is_bs`', '`is_bs`', 202, -1, FALSE, '`is_bs`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
		...
		// unit_id
		$this->unit_id = new cField('tr_retp_item', 'tr_retp_item', 'x_unit_id', 'unit_id', '`unit_id`', '`unit_id`', 200, -1, FALSE, '`unit_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "tr_retp_master") {
			if ($this->master_id->getSessionValue() <> "")
				$sMasterFilter .= "`retp_id`=" . ew_QuotedValue($this->master_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_tr_retp_master() {
		return "`retp_id`=@retp_id@";
	}

	// Detail filter
	function SqlDetailFilter_tr_retp_master() {
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tr_retp_item`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "tr_retp_itemedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "tr_retp_itemadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tr_retp_itemview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tr_retp_itemview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "tr_retp_itemadd.php?" . $this->UrlParm($parm);
		else
			$url = "tr_retp_itemadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("tr_retp_itemedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("tr_retp_itemadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tr_retp_itemdelete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "tr_retp_master" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
			$url .= (strpos($url, "?") !== FALSE ? "&" : "?") . EW_TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_retp_id=" . urlencode($this->master_id->CurrentValue);
		}
		return $url;
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->rcv_qty->setDbValue($rs->fields('rcv_qty'));
	...
	function RenderEditRow() {
		...
		$this->rcv_qty->EditValue = $this->rcv_qty->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->rcv_qty->Exportable) $Doc->ExportCaption($this->rcv_qty);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->rcv_qty->Exportable) $Doc->ExportField($this->rcv_qty);
	...
	function GetAutoFill($id, $val) {
		...
		if (preg_match('/^x(\d)*_item_id$/', $id)) {
			...
			$sSqlWrk = "SELECT `product_code` AS FIELD0, `product_name` AS FIELD1, `udet_id` AS FIELD2, `unit_id` AS FIELD3 FROM `view_products_unit_price`";
			...
			if ($rs = ew_LoadRecordset($sSqlWrk, $conn)) {
				while ($rs && !$rs->EOF) {
					...
					$this->udet_id->setDbValue($rs->fields[2]);
					$this->unit_id->setDbValue($rs->fields[3]);
