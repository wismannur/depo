...
$tr_retp_master_delete = NULL; // Initialize page object first

class ctr_retp_master_delete extends ctr_retp_master {
	...
	// Page object name
	var $PageObjName = 'tr_retp_master_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("tr_retp_masterlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in tr_retp_master class, tr_retp_masterinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("tr_retp_masterlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($tr_retp_master_delete)) $tr_retp_master_delete = new ctr_retp_master_delete();

// Page init
$tr_retp_master_delete->Page_Init();

// Page main
$tr_retp_master_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tr_retp_master_delete->Page_Render();
?>
...
var CurrentForm = ftr_retp_masterdelete = new ew_Form("ftr_retp_masterdelete", "delete");

// Form_CustomValidate event
ftr_retp_masterdelete.Form_CustomValidate = 
...
ftr_retp_masterdelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $tr_retp_master_delete->ShowPageHeader(); ?>
<?php
$tr_retp_master_delete->ShowMessage();
?>
<form name="ftr_retp_masterdelete" id="ftr_retp_masterdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tr_retp_master_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tr_retp_master_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tr_retp_master">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($tr_retp_master_delete->RecKeys as $key) { ?>
...
<?php if ($tr_retp_master->retp_number->Visible) { // retp_number ?>
		<th class="<?php echo $tr_retp_master->retp_number->HeaderCellClass() ?>"><span id="elh_tr_retp_master_retp_number" class="tr_retp_master_retp_number"><?php echo $tr_retp_master->retp_number->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_retp_master->retp_date->Visible) { // retp_date ?>
		<th class="<?php echo $tr_retp_master->retp_date->HeaderCellClass() ?>"><span id="elh_tr_retp_master_retp_date" class="tr_retp_master_retp_date"><?php echo $tr_retp_master->retp_date->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_retp_master->retp_notes->Visible) { // retp_notes ?>
		<th class="<?php echo $tr_retp_master->retp_notes->HeaderCellClass() ?>"><span id="elh_tr_retp_master_retp_notes" class="tr_retp_master_retp_notes"><?php echo $tr_retp_master->retp_notes->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$tr_retp_master_delete->RecCnt = 0;
$i = 0;
while (!$tr_retp_master_delete->Recordset->EOF) {
	$tr_retp_master_delete->RecCnt++;
	$tr_retp_master_delete->RowCnt++;

	// Set row properties
	$tr_retp_master->ResetAttrs();
	$tr_retp_master->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$tr_retp_master_delete->LoadRowValues($tr_retp_master_delete->Recordset);

	// Render row
	$tr_retp_master_delete->RenderRow();
?>
	<tr<?php echo $tr_retp_master->RowAttributes() ?>>
<?php if ($tr_retp_master->retp_number->Visible) { // retp_number ?>
		<td<?php echo $tr_retp_master->retp_number->CellAttributes() ?>>
<span id="el<?php echo $tr_retp_master_delete->RowCnt ?>_tr_retp_master_retp_number" class="tr_retp_master_retp_number">
...
<span id="el<?php echo $tr_retp_master_delete->RowCnt ?>_tr_retp_master_retp_date" class="tr_retp_master_retp_date">
...
<span id="el<?php echo $tr_retp_master_delete->RowCnt ?>_tr_retp_master_retp_notes" class="tr_retp_master_retp_notes">
...
<?php
	$tr_retp_master_delete->Recordset->MoveNext();
}
$tr_retp_master_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tr_retp_master_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ftr_retp_masterdelete.Init();
</script>
<?php
$tr_retp_master_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tr_retp_master_delete->Page_Terminate();
?>
