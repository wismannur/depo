...
//
// Table class for tr_ret_item
//
class ctr_ret_item extends cTable {
	var $row_id;
	var $master_id;
	var $item_id;
	var $item_code;
	...
	function __construct() {
		...
		$this->TableVar = 'tr_ret_item';
		$this->TableName = 'tr_ret_item';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tr_ret_item`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// row_id
		$this->row_id = new cField('tr_ret_item', 'tr_ret_item', 'x_row_id', 'row_id', '`row_id`', '`row_id`', 3, -1, FALSE, '`row_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// master_id
		$this->master_id = new cField('tr_ret_item', 'tr_ret_item', 'x_master_id', 'master_id', '`master_id`', '`master_id`', 3, -1, FALSE, '`master_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// item_id
		$this->item_id = new cField('tr_ret_item', 'tr_ret_item', 'x_item_id', 'item_id', '`item_id`', '`item_id`', 3, -1, FALSE, '`item_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// item_code
		$this->item_code = new cField('tr_ret_item', 'tr_ret_item', 'x_item_code', 'item_code', '`item_code`', '`item_code`', 200, -1, FALSE, '`item_code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->item_code->Sortable = TRUE; // Allow sort
		$this->fields['item_code'] = &$this->item_code;

		// item_name
		$this->item_name = new cField('tr_ret_item', 'tr_ret_item', 'x_item_name', 'item_name', '`item_name`', '`item_name`', 200, -1, FALSE, '`item_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
		$this->item_name->Sortable = TRUE; // Allow sort
		$this->fields['item_name'] = &$this->item_name;

		// udet_id
		$this->udet_id = new cField('tr_ret_item', 'tr_ret_item', 'x_udet_id', 'udet_id', '`udet_id`', '`udet_id`', 3, -1, FALSE, '`udet_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// item_qty
		$this->item_qty = new cField('tr_ret_item', 'tr_ret_item', 'x_item_qty', 'item_qty', '`item_qty`', '`item_qty`', 5, -1, FALSE, '`item_qty`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// item_price
		$this->item_price = new cField('tr_ret_item', 'tr_ret_item', 'x_item_price', 'item_price', '`item_price`', '`item_price`', 5, -1, FALSE, '`item_price`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// item_disc
		$this->item_disc = new cField('tr_ret_item', 'tr_ret_item', 'x_item_disc', 'item_disc', '`item_disc`', '`item_disc`', 2, -1, FALSE, '`item_disc`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// item_amt
		$this->item_amt = new cField('tr_ret_item', 'tr_ret_item', 'x_item_amt', 'item_amt', '\'\'', '\'\'', 201, -1, FALSE, '\'\'', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->item_amt->FldIsCustom = TRUE; // Custom field
		$this->item_amt->Sortable = TRUE; // Allow sort
		$this->fields['item_amt'] = &$this->item_amt;

		// is_bs
		$this->is_bs = new cField('tr_ret_item', 'tr_ret_item', 'x_is_bs', 'is_bs', '`is_bs`', '`is_bs`', 202, -1, FALSE, '`is_bs`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
		...
		// unit_id
		$this->unit_id = new cField('tr_ret_item', 'tr_ret_item', 'x_unit_id', 'unit_id', '`unit_id`', '`unit_id`', 200, -1, FALSE, '`unit_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
	...
	function GetMasterFilter() {
		...
		if ($this->getCurrentMasterTable() == "tr_ret_master") {
			if ($this->master_id->getSessionValue() <> "")
				$sMasterFilter .= "`ret_id`=" . ew_QuotedValue($this->master_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_tr_ret_master() {
		return "`ret_id`=@ret_id@";
	}

	// Detail filter
	function SqlDetailFilter_tr_ret_master() {
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tr_ret_item`";
	}
	...
	function getSqlSelect() { // Select
		return ($this->_SqlSelect <> "") ? $this->_SqlSelect : "SELECT *, '' AS `item_amt` FROM " . $this->getSqlFrom();
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "tr_ret_itemedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "tr_ret_itemadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tr_ret_itemview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tr_ret_itemview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "tr_ret_itemadd.php?" . $this->UrlParm($parm);
		else
			$url = "tr_ret_itemadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("tr_ret_itemedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("tr_ret_itemadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tr_ret_itemdelete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "tr_ret_master" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
			$url .= (strpos($url, "?") !== FALSE ? "&" : "?") . EW_TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_ret_id=" . urlencode($this->master_id->CurrentValue);
		}
		return $url;
	}
	...
	function GetAutoFill($id, $val) {
		...
		if (preg_match('/^x(\d)*_item_id$/', $id)) {
			...
			$sSqlWrk = "SELECT `product_code` AS FIELD0, `product_name` AS FIELD1, `udet_id` AS FIELD2, `unit_price` AS FIELD3, `unit_id` AS FIELD4 FROM `view_products_unit_price`";
			...
			if ($rs = ew_LoadRecordset($sSqlWrk, $conn)) {
				while ($rs && !$rs->EOF) {
					...
					$this->unit_id->setDbValue($rs->fields[4]);
