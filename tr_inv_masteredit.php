...
$tr_inv_master_edit = NULL; // Initialize page object first

class ctr_inv_master_edit extends ctr_inv_master {
	...
	// Page object name
	var $PageObjName = 'tr_inv_master_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			...
			// Load key from Form
			if ($objForm->HasValue("x_inv_id")) {
				$this->inv_id->setFormValue($objForm->GetValue("x_inv_id"));
			}
		} else {
	...
	function RestoreFormValues() {
		global $objForm;
		$this->inv_id->CurrentValue = $this->inv_id->FormValue;
	...
	function ValidateForm() {
		...
		if (in_array("tr_inv_item", $DetailTblVar) && $GLOBALS["tr_inv_item"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// inv_date
			$this->inv_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->inv_date->CurrentValue, 7), NULL, $this->inv_date->ReadOnly);

			// tax_type
			$this->tax_type->SetDbValueDef($rsnew, $this->tax_type->CurrentValue, NULL, $this->tax_type->ReadOnly);

			// due_date
			$this->due_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->due_date->CurrentValue, 0), NULL, $this->due_date->ReadOnly);
			...
			// tax_number
			$this->tax_number->SetDbValueDef($rsnew, $this->tax_number->CurrentValue, NULL, $this->tax_number->ReadOnly);

			// tc_number
			$this->tc_number->SetDbValueDef($rsnew, $this->tc_number->CurrentValue, NULL, $this->tc_number->ReadOnly);

			// inv_amt
			$this->inv_amt->SetDbValueDef($rsnew, $this->inv_amt->CurrentValue, NULL, $this->inv_amt->ReadOnly);

			// total_discount
			$this->total_discount->SetDbValueDef($rsnew, $this->total_discount->CurrentValue, NULL, $this->total_discount->ReadOnly);

			// is_tax
			$tmpBool = $this->is_tax->CurrentValue;
			if ($tmpBool <> "1" && $tmpBool <> "0")
				$tmpBool = (!empty($tmpBool)) ? "1" : "0";
			$this->is_tax->SetDbValueDef($rsnew, $tmpBool, NULL, $this->is_tax->ReadOnly);

			// tax_total
			$this->tax_total->SetDbValueDef($rsnew, $this->tax_total->CurrentValue, NULL, $this->tax_total->ReadOnly);

			// inv_total
			$this->inv_total->SetDbValueDef($rsnew, $this->inv_total->CurrentValue, NULL, $this->inv_total->ReadOnly);
			...
			// tanggal_koreksi
			$this->tanggal_koreksi->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->tanggal_koreksi->CurrentValue, 0), NULL, $this->tanggal_koreksi->ReadOnly);
			...
			if ($tmpBool <> "1" && $tmpBool <> "0")
				$tmpBool = (!empty($tmpBool)) ? "1" : "0";
			$this->paid->SetDbValueDef($rsnew, $tmpBool, NULL, $this->paid->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if (in_array("tr_inv_item", $DetailTblVar) && $GLOBALS["tr_inv_item"]->DetailEdit) {
						...
						$EditRow = $GLOBALS["tr_inv_item_grid"]->GridUpdate();
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("tr_inv_item", $DetailTblVar)) {
				...
				if ($GLOBALS["tr_inv_item_grid"]->DetailEdit) {
					$GLOBALS["tr_inv_item_grid"]->CurrentMode = "edit";
					$GLOBALS["tr_inv_item_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($tr_inv_master_edit)) $tr_inv_master_edit = new ctr_inv_master_edit();

// Page init
$tr_inv_master_edit->Page_Init();

// Page main
$tr_inv_master_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tr_inv_master_edit->Page_Render();
?>
...
var CurrentForm = ftr_inv_masteredit = new ew_Form("ftr_inv_masteredit", "edit");

// Validate form
ftr_inv_masteredit.Validate = function() {
...
ftr_inv_masteredit.Form_CustomValidate = 
...
ftr_inv_masteredit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Multi-Page
ftr_inv_masteredit.MultiPage = new ew_MultiPage("ftr_inv_masteredit");

// Dynamic selection lists
ftr_inv_masteredit.Lists["x_tax_type"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftr_inv_masteredit.Lists["x_tax_type"].Options = <?php echo json_encode($tr_inv_master_edit->tax_type->Options()) ?>;
ftr_inv_masteredit.Lists["x_customer_id"] = {"LinkField":"x_customer_id","Ajax":true,"AutoFill":true,"DisplayFields":["x_customer_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_customer"};
ftr_inv_masteredit.Lists["x_customer_id"].Data = "<?php echo $tr_inv_master_edit->customer_id->LookupFilterQuery(FALSE, "edit") ?>";
ftr_inv_masteredit.AutoSuggests["x_customer_id"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $tr_inv_master_edit->customer_id->LookupFilterQuery(TRUE, "edit"))) ?>;
ftr_inv_masteredit.Lists["x_wilayah_id"] = {"LinkField":"x_wilayah_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama_wilayah","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_wilayah"};
ftr_inv_masteredit.Lists["x_wilayah_id"].Data = "<?php echo $tr_inv_master_edit->wilayah_id->LookupFilterQuery(FALSE, "edit") ?>";
ftr_inv_masteredit.Lists["x_subwil_id"] = {"LinkField":"x_sub_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama_sub_wilayah","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_subwilayah"};
ftr_inv_masteredit.Lists["x_subwil_id"].Data = "<?php echo $tr_inv_master_edit->subwil_id->LookupFilterQuery(FALSE, "edit") ?>";
ftr_inv_masteredit.Lists["x_area_id"] = {"LinkField":"x_area_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_area_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_callarea"};
ftr_inv_masteredit.Lists["x_area_id"].Data = "<?php echo $tr_inv_master_edit->area_id->LookupFilterQuery(FALSE, "edit") ?>";
ftr_inv_masteredit.Lists["x_is_tax[]"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftr_inv_masteredit.Lists["x_is_tax[]"].Options = <?php echo json_encode($tr_inv_master_edit->is_tax->Options()) ?>;
ftr_inv_masteredit.Lists["x_sales_id"] = {"LinkField":"x_sales_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_sales_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_salesman"};
ftr_inv_masteredit.Lists["x_sales_id"].Data = "<?php echo $tr_inv_master_edit->sales_id->LookupFilterQuery(FALSE, "edit") ?>";
ftr_inv_masteredit.Lists["x_paid[]"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftr_inv_masteredit.Lists["x_paid[]"].Options = <?php echo json_encode($tr_inv_master_edit->paid->Options()) ?>;
...
<?php $tr_inv_master_edit->ShowPageHeader(); ?>
<?php
$tr_inv_master_edit->ShowMessage();
?>
<form name="ftr_inv_masteredit" id="ftr_inv_masteredit" class="<?php echo $tr_inv_master_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tr_inv_master_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tr_inv_master_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tr_inv_master">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($tr_inv_master_edit->IsModal) ?>">
<div id="tpd_tr_inv_masteredit" class="ewCustomTemplate"></div>
<script id="tpm_tr_inv_masteredit" type="text/html">
<div id="ct_tr_inv_master_edit"><div class="ewMultiPage"><!-- multi-page -->
...
<?php if (!$tr_inv_master_edit->IsMobileOrModal) { ?>
<div class="ewDesktop"><!-- desktop -->
<?php } ?>
<div class="ewMultiPage hidden"><!-- multi-page -->
<div class="nav-tabs-custom" id="tr_inv_master_edit"><!-- multi-page .nav-tabs-custom -->
	<ul class="nav<?php echo $tr_inv_master_edit->MultiPages->NavStyle() ?>">
		<li<?php echo $tr_inv_master_edit->MultiPages->TabStyle("1") ?>><a href="#tab_tr_inv_master1" data-toggle="tab"><?php echo $tr_inv_master->PageCaption(1) ?></a></li>
		<li<?php echo $tr_inv_master_edit->MultiPages->TabStyle("2") ?>><a href="#tab_tr_inv_master2" data-toggle="tab"><?php echo $tr_inv_master->PageCaption(2) ?></a></li>
	</ul>
	<div class="tab-content"><!-- multi-page .nav-tabs-custom .tab-content -->
		<div class="tab-pane<?php echo $tr_inv_master_edit->MultiPages->PageStyle("1") ?>" id="tab_tr_inv_master1"><!-- multi-page .tab-pane -->
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
<div class="ewEditDiv hidden"><!-- page* -->
<?php } else { ?>
<table id="tbl_tr_inv_masteredit1" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable hidden"><!-- table* -->
<?php } ?>
<?php if ($tr_inv_master->inv_number->Visible) { // inv_number ?>
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_inv_number" class="form-group">
		<label id="elh_tr_inv_master_inv_number" for="x_inv_number" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_inv_number" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->inv_number->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->inv_number->CellAttributes() ?>>
<script id="tpx_tr_inv_master_inv_number" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_inv_number">
		<td class="col-sm-2"><span id="elh_tr_inv_master_inv_number"><script id="tpc_tr_inv_master_inv_number" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->inv_number->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_inv_date" class="form-group">
		<label id="elh_tr_inv_master_inv_date" for="x_inv_date" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_inv_date" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->inv_date->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->inv_date->CellAttributes() ?>>
<script id="tpx_tr_inv_master_inv_date" class="tr_inv_masteredit" type="text/html">
...
<script type="text/html" class="tr_inv_masteredit_js">
ew_CreateDateTimePicker("ftr_inv_masteredit", "x_inv_date", {"ignoreReadonly":true,"useCurrent":false,"format":7});
</script>
<?php echo $tr_inv_master->inv_date->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_inv_date">
		<td class="col-sm-2"><span id="elh_tr_inv_master_inv_date"><script id="tpc_tr_inv_master_inv_date" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->inv_date->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_tax_type" class="form-group">
		<label id="elh_tr_inv_master_tax_type" for="x_tax_type" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_tax_type" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->tax_type->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->tax_type->CellAttributes() ?>>
<script id="tpx_tr_inv_master_tax_type" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_tax_type">
		<td class="col-sm-2"><span id="elh_tr_inv_master_tax_type"><script id="tpc_tr_inv_master_tax_type" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->tax_type->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_due_date" class="form-group">
		<label id="elh_tr_inv_master_due_date" for="x_due_date" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_due_date" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->due_date->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->due_date->CellAttributes() ?>>
<script id="tpx_tr_inv_master_due_date" class="tr_inv_masteredit" type="text/html">
...
<script type="text/html" class="tr_inv_masteredit_js">
ew_CreateDateTimePicker("ftr_inv_masteredit", "x_due_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php echo $tr_inv_master->due_date->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_due_date">
		<td class="col-sm-2"><span id="elh_tr_inv_master_due_date"><script id="tpc_tr_inv_master_due_date" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->due_date->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_customer_id" class="form-group">
		<label id="elh_tr_inv_master_customer_id" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_customer_id" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->customer_id->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->customer_id->CellAttributes() ?>>
<script id="tpx_tr_inv_master_customer_id" class="tr_inv_masteredit" type="text/html">
...
<script type="text/html" class="tr_inv_masteredit_js">
ftr_inv_masteredit.CreateAutoSuggest({"id":"x_customer_id","forceSelect":true});
</script>
<?php echo $tr_inv_master->customer_id->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_customer_id">
		<td class="col-sm-2"><span id="elh_tr_inv_master_customer_id"><script id="tpc_tr_inv_master_customer_id" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->customer_id->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_wilayah_id" class="form-group">
		<label id="elh_tr_inv_master_wilayah_id" for="x_wilayah_id" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_wilayah_id" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->wilayah_id->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->wilayah_id->CellAttributes() ?>>
<script id="tpx_tr_inv_master_wilayah_id" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_wilayah_id">
		<td class="col-sm-2"><span id="elh_tr_inv_master_wilayah_id"><script id="tpc_tr_inv_master_wilayah_id" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->wilayah_id->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_subwil_id" class="form-group">
		<label id="elh_tr_inv_master_subwil_id" for="x_subwil_id" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_subwil_id" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->subwil_id->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->subwil_id->CellAttributes() ?>>
<script id="tpx_tr_inv_master_subwil_id" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_subwil_id">
		<td class="col-sm-2"><span id="elh_tr_inv_master_subwil_id"><script id="tpc_tr_inv_master_subwil_id" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->subwil_id->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_area_id" class="form-group">
		<label id="elh_tr_inv_master_area_id" for="x_area_id" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_area_id" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->area_id->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->area_id->CellAttributes() ?>>
<script id="tpx_tr_inv_master_area_id" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_area_id">
		<td class="col-sm-2"><span id="elh_tr_inv_master_area_id"><script id="tpc_tr_inv_master_area_id" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->area_id->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_inv_amt" class="form-group">
		<label id="elh_tr_inv_master_inv_amt" for="x_inv_amt" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_inv_amt" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->inv_amt->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->inv_amt->CellAttributes() ?>>
<script id="tpx_tr_inv_master_inv_amt" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_inv_amt">
		<td class="col-sm-2"><span id="elh_tr_inv_master_inv_amt"><script id="tpc_tr_inv_master_inv_amt" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->inv_amt->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_total_discount" class="form-group">
		<label id="elh_tr_inv_master_total_discount" for="x_total_discount" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_total_discount" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->total_discount->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->total_discount->CellAttributes() ?>>
<script id="tpx_tr_inv_master_total_discount" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_total_discount">
		<td class="col-sm-2"><span id="elh_tr_inv_master_total_discount"><script id="tpc_tr_inv_master_total_discount" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->total_discount->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_is_tax" class="form-group">
		<label id="elh_tr_inv_master_is_tax" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_is_tax" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->is_tax->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->is_tax->CellAttributes() ?>>
<script id="tpx_tr_inv_master_is_tax" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_is_tax">
		<td class="col-sm-2"><span id="elh_tr_inv_master_is_tax"><script id="tpc_tr_inv_master_is_tax" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->is_tax->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_tax_total" class="form-group">
		<label id="elh_tr_inv_master_tax_total" for="x_tax_total" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_tax_total" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->tax_total->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->tax_total->CellAttributes() ?>>
<script id="tpx_tr_inv_master_tax_total" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_tax_total">
		<td class="col-sm-2"><span id="elh_tr_inv_master_tax_total"><script id="tpc_tr_inv_master_tax_total" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->tax_total->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_inv_total" class="form-group">
		<label id="elh_tr_inv_master_inv_total" for="x_inv_total" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_inv_total" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->inv_total->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->inv_total->CellAttributes() ?>>
<script id="tpx_tr_inv_master_inv_total" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_inv_total">
		<td class="col-sm-2"><span id="elh_tr_inv_master_inv_total"><script id="tpc_tr_inv_master_inv_total" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->inv_total->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_paid_amt" class="form-group">
		<label id="elh_tr_inv_master_paid_amt" for="x_paid_amt" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_paid_amt" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->paid_amt->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->paid_amt->CellAttributes() ?>>
<script id="tpx_tr_inv_master_paid_amt" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_paid_amt">
		<td class="col-sm-2"><span id="elh_tr_inv_master_paid_amt"><script id="tpc_tr_inv_master_paid_amt" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->paid_amt->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_koreksi" class="form-group">
		<label id="elh_tr_inv_master_koreksi" for="x_koreksi" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_koreksi" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->koreksi->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->koreksi->CellAttributes() ?>>
<script id="tpx_tr_inv_master_koreksi" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_koreksi">
		<td class="col-sm-2"><span id="elh_tr_inv_master_koreksi"><script id="tpc_tr_inv_master_koreksi" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->koreksi->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_tanggal_koreksi" class="form-group">
		<label id="elh_tr_inv_master_tanggal_koreksi" for="x_tanggal_koreksi" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_tanggal_koreksi" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->tanggal_koreksi->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->tanggal_koreksi->CellAttributes() ?>>
<script id="tpx_tr_inv_master_tanggal_koreksi" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_tanggal_koreksi">
		<td class="col-sm-2"><span id="elh_tr_inv_master_tanggal_koreksi"><script id="tpc_tr_inv_master_tanggal_koreksi" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->tanggal_koreksi->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_kode_depo" class="form-group">
		<label id="elh_tr_inv_master_kode_depo" for="x_kode_depo" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_kode_depo" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->kode_depo->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->kode_depo->CellAttributes() ?>>
<script id="tpx_tr_inv_master_kode_depo" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_kode_depo">
		<td class="col-sm-2"><span id="elh_tr_inv_master_kode_depo"><script id="tpc_tr_inv_master_kode_depo" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->kode_depo->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_paid" class="form-group">
		<label id="elh_tr_inv_master_paid" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_paid" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->paid->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->paid->CellAttributes() ?>>
<script id="tpx_tr_inv_master_paid" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_paid">
		<td class="col-sm-2"><span id="elh_tr_inv_master_paid"><script id="tpc_tr_inv_master_paid" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->paid->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></script></span></td>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $tr_inv_master_edit->MultiPages->PageStyle("2") ?>" id="tab_tr_inv_master2"><!-- multi-page .tab-pane -->
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
<div class="ewEditDiv hidden"><!-- page* -->
<?php } else { ?>
<table id="tbl_tr_inv_masteredit2" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable hidden"><!-- table* -->
<?php } ?>
<?php if ($tr_inv_master->customer_name->Visible) { // customer_name ?>
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_customer_name" class="form-group">
		<label id="elh_tr_inv_master_customer_name" for="x_customer_name" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_customer_name" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->customer_name->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->customer_name->CellAttributes() ?>>
<script id="tpx_tr_inv_master_customer_name" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_customer_name">
		<td class="col-sm-2"><span id="elh_tr_inv_master_customer_name"><script id="tpc_tr_inv_master_customer_name" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->customer_name->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_address1" class="form-group">
		<label id="elh_tr_inv_master_address1" for="x_address1" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_address1" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->address1->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->address1->CellAttributes() ?>>
<script id="tpx_tr_inv_master_address1" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_address1">
		<td class="col-sm-2"><span id="elh_tr_inv_master_address1"><script id="tpc_tr_inv_master_address1" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->address1->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_address2" class="form-group">
		<label id="elh_tr_inv_master_address2" for="x_address2" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_address2" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->address2->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->address2->CellAttributes() ?>>
<script id="tpx_tr_inv_master_address2" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_address2">
		<td class="col-sm-2"><span id="elh_tr_inv_master_address2"><script id="tpc_tr_inv_master_address2" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->address2->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_address3" class="form-group">
		<label id="elh_tr_inv_master_address3" for="x_address3" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_address3" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->address3->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->address3->CellAttributes() ?>>
<script id="tpx_tr_inv_master_address3" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_address3">
		<td class="col-sm-2"><span id="elh_tr_inv_master_address3"><script id="tpc_tr_inv_master_address3" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->address3->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_tax_number" class="form-group">
		<label id="elh_tr_inv_master_tax_number" for="x_tax_number" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_tax_number" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->tax_number->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->tax_number->CellAttributes() ?>>
<script id="tpx_tr_inv_master_tax_number" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_tax_number">
		<td class="col-sm-2"><span id="elh_tr_inv_master_tax_number"><script id="tpc_tr_inv_master_tax_number" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->tax_number->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_tc_number" class="form-group">
		<label id="elh_tr_inv_master_tc_number" for="x_tc_number" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_tc_number" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->tc_number->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->tc_number->CellAttributes() ?>>
<script id="tpx_tr_inv_master_tc_number" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_tc_number">
		<td class="col-sm-2"><span id="elh_tr_inv_master_tc_number"><script id="tpc_tr_inv_master_tc_number" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->tc_number->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_sales_id" class="form-group">
		<label id="elh_tr_inv_master_sales_id" for="x_sales_id" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_sales_id" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->sales_id->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->sales_id->CellAttributes() ?>>
<script id="tpx_tr_inv_master_sales_id" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_sales_id">
		<td class="col-sm-2"><span id="elh_tr_inv_master_sales_id"><script id="tpc_tr_inv_master_sales_id" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->sales_id->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_sopir" class="form-group">
		<label id="elh_tr_inv_master_sopir" for="x_sopir" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_sopir" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->sopir->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->sopir->CellAttributes() ?>>
<script id="tpx_tr_inv_master_sopir" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_sopir">
		<td class="col-sm-2"><span id="elh_tr_inv_master_sopir"><script id="tpc_tr_inv_master_sopir" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->sopir->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_inv_master_edit->IsMobileOrModal) { ?>
	<div id="r_no_mobil" class="form-group">
		<label id="elh_tr_inv_master_no_mobil" for="x_no_mobil" class="<?php echo $tr_inv_master_edit->LeftColumnClass ?>"><script id="tpc_tr_inv_master_no_mobil" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->no_mobil->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_inv_master_edit->RightColumnClass ?>"><div<?php echo $tr_inv_master->no_mobil->CellAttributes() ?>>
<script id="tpx_tr_inv_master_no_mobil" class="tr_inv_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_no_mobil">
		<td class="col-sm-2"><span id="elh_tr_inv_master_no_mobil"><script id="tpc_tr_inv_master_no_mobil" class="tr_inv_masteredit" type="text/html"><span><?php echo $tr_inv_master->no_mobil->FldCaption() ?></span></script></span></td>
...
<input type="hidden" data-table="tr_inv_master" data-field="x_inv_id" name="x_inv_id" id="x_inv_id" value="<?php echo ew_HtmlEncode($tr_inv_master->inv_id->CurrentValue) ?>">
<?php
	if (in_array("tr_inv_item", explode(",", $tr_inv_master->getCurrentDetailTable())) && $tr_inv_item->DetailEdit) {
?>
...
<?php } ?>
<?php if (!$tr_inv_master_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $tr_inv_master_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tr_inv_master_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
...
ew_ApplyTemplate("tpd_tr_inv_masteredit", "tpm_tr_inv_masteredit", "tr_inv_masteredit", "<?php echo $tr_inv_master->CustomExport ?>", ewVar.templateData.rows[0]);
jQuery("script.tr_inv_masteredit_js").each(function(){ew_AddScript(this.text);});
</script>
<script type="text/javascript">
ftr_inv_masteredit.Init();
</script>
<?php
$tr_inv_master_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tr_inv_master_edit->Page_Terminate();
?>
