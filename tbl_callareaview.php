...
$tbl_callarea_view = NULL; // Initialize page object first

class ctbl_callarea_view extends ctbl_callarea {
	...
	// Page object name
	var $PageObjName = 'tbl_callarea_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["area_id"] <> "") {
			$this->RecKey["area_id"] = $_GET["area_id"];
			$KeyUrl .= "&amp;area_id=" . urlencode($this->RecKey["area_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["area_id"] <> "") {
				...
				$this->RecKey["area_id"] = $this->area_id->QueryStringValue;
			} elseif (@$_POST["area_id"] <> "") {
				$this->area_id->setFormValue($_POST["area_id"]);
				$this->RecKey["area_id"] = $this->area_id->FormValue;
			} else {
				$sReturnUrl = "tbl_callarealist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "tbl_callarealist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "tbl_callarealist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($tbl_callarea_view)) $tbl_callarea_view = new ctbl_callarea_view();

// Page init
$tbl_callarea_view->Page_Init();

// Page main
$tbl_callarea_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tbl_callarea_view->Page_Render();
?>
...
var CurrentForm = ftbl_callareaview = new ew_Form("ftbl_callareaview", "view");

// Form_CustomValidate event
ftbl_callareaview.Form_CustomValidate = 
...
ftbl_callareaview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<div class="ewToolbar">
<?php $tbl_callarea_view->ExportOptions->Render("body") ?>
<?php
	foreach ($tbl_callarea_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php $tbl_callarea_view->ShowPageHeader(); ?>
<?php
$tbl_callarea_view->ShowMessage();
?>
<form name="ftbl_callareaview" id="ftbl_callareaview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tbl_callarea_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tbl_callarea_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tbl_callarea">
<input type="hidden" name="modal" value="<?php echo intval($tbl_callarea_view->IsModal) ?>">
...
<?php echo $tbl_callarea->area_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $tbl_callarea->area_name->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $tbl_callarea->subwil_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $tbl_callarea->wilayah_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
ftbl_callareaview.Init();
</script>
<?php
$tbl_callarea_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tbl_callarea_view->Page_Terminate();
?>
