...
$tr_km_item_ar_list = NULL; // Initialize page object first

class ctr_km_item_ar_list extends ctr_km_item_ar {
	...
	// Page object name
	var $PageObjName = 'tr_km_item_ar_list';

	// Grid form hidden field names
	var $FormName = 'ftr_km_item_arlist';
	...
	function __construct() {
		...
		$this->AddUrl = "tr_km_item_aradd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "tr_km_item_ardelete.php";
		$this->MultiUpdateUrl = "tr_km_item_arupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ftr_km_item_arlistsrch";
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "tr_km_master_ar") {
			global $tr_km_master_ar;
			$rsmaster = $tr_km_master_ar->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("tr_km_master_arlist.php"); // Return to master page
			} else {
				$tr_km_master_ar->LoadListRowValues($rsmaster);
				$tr_km_master_ar->RowType = EW_ROWTYPE_MASTER; // Master row
				$tr_km_master_ar->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ftr_km_item_arlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = FALSE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ftr_km_item_arlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = FALSE;
		...
		if ($this->CurrentAction <> "gridadd" && $this->CurrentAction <> "gridedit") { // Not grid add/edit mode
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.ftr_km_item_arlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function RenderRow() {
		...
		$this->acc_no->ViewCustomAttributes = "";
			...
			$sSqlWrk = "SELECT `inv_number`, `inv_number` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld`, `customer_id` AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `tr_inv_master`";
			$sWhereWrk = "";
			$this->inv_number->LookupFilters = array();
			$lookuptblfilter = (isset($_GET["customer_id"]))? "`customer_id` = '".@$_GET["customer_id"]."' AND `kode_depo` = '".@$_SESSION["KodeDepo"]."'" : "`kode_depo` = '".@$_SESSION["KodeDepo"]."'";
			ew_AddFilter($sWhereWrk, $lookuptblfilter);
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_tr_km_item_ar\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_tr_km_item_ar',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.ftr_km_item_arlist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
	...
	function ExportData() {
		...
		if (EW_EXPORT_MASTER_RECORD && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "tr_km_master_ar") {
			global $tr_km_master_ar;
			if (!isset($tr_km_master_ar)) $tr_km_master_ar = new ctr_km_master_ar;
			$rsmaster = $tr_km_master_ar->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("v"); // Change to vertical
				if ($this->Export <> "csv" || EW_EXPORT_MASTER_RECORD_FOR_CSV) {
					$Doc->Table = &$tr_km_master_ar;
					$tr_km_master_ar->ExportDocument($Doc, $rsmaster, 1, 1);
					$Doc->ExportEmptyRow();
					$Doc->Table = &$this;
				}
	...
	function SetupLookupFilters($fld, $pageId = null) {
		...
		case "x_inv_number":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `inv_number` AS `LinkFld`, `inv_number` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `tr_inv_master`";
			$sWhereWrk = "{filter}";
			$fld->LookupFilters = array();
			$lookuptblfilter = (isset($_GET["customer_id"]))? "`customer_id` = '".@$_GET["customer_id"]."' AND `kode_depo` = '".@$_SESSION["KodeDepo"]."'" : "`kode_depo` = '".@$_SESSION["KodeDepo"]."'";
			ew_AddFilter($sWhereWrk, $lookuptblfilter);
...
// Create page object
if (!isset($tr_km_item_ar_list)) $tr_km_item_ar_list = new ctr_km_item_ar_list();

// Page init
$tr_km_item_ar_list->Page_Init();

// Page main
$tr_km_item_ar_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tr_km_item_ar_list->Page_Render();
?>
...
var CurrentForm = ftr_km_item_arlist = new ew_Form("ftr_km_item_arlist", "list");
ftr_km_item_arlist.FormKeyCountName = '<?php echo $tr_km_item_ar_list->FormKeyCountName ?>';

// Validate form
ftr_km_item_arlist.Validate = function() {
...
// Check empty row
ftr_km_item_arlist.EmptyRow = function(infix) {
	var fobj = this.Form;
...
ftr_km_item_arlist.Form_CustomValidate = 
...
ftr_km_item_arlist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
ftr_km_item_arlist.Lists["x_customer_id"] = {"LinkField":"x_customer_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_customer_name","","",""],"ParentFields":[],"ChildFields":["x_inv_number"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_customer"};
ftr_km_item_arlist.Lists["x_customer_id"].Data = "<?php echo $tr_km_item_ar_list->customer_id->LookupFilterQuery(FALSE, "list") ?>";
ftr_km_item_arlist.Lists["x_inv_number"] = {"LinkField":"x_inv_number","Ajax":true,"AutoFill":true,"DisplayFields":["x_inv_number","","",""],"ParentFields":["x_customer_id"],"ChildFields":[],"FilterFields":["x_customer_id"],"Options":[],"Template":"","LinkTable":"tr_inv_master"};
ftr_km_item_arlist.Lists["x_inv_number"].Data = "<?php echo $tr_km_item_ar_list->inv_number->LookupFilterQuery(FALSE, "list") ?>";
...
<?php if ($tr_km_item_ar->Export == "") { ?>
<div class="ewToolbar">
<?php if ($tr_km_item_ar_list->TotalRecs > 0 && $tr_km_item_ar_list->ExportOptions->Visible()) { ?>
<?php $tr_km_item_ar_list->ExportOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php if (($tr_km_item_ar->Export == "") || (EW_EXPORT_MASTER_RECORD && $tr_km_item_ar->Export == "print")) { ?>
<?php
if ($tr_km_item_ar_list->DbMasterFilter <> "" && $tr_km_item_ar->getCurrentMasterTable() == "tr_km_master_ar") {
	if ($tr_km_item_ar_list->MasterRecordExists) {
?>
<?php include_once "tr_km_master_armaster.php" ?>
<?php
	}
}
?>
...
if ($tr_km_item_ar->CurrentAction == "gridadd") {
	...
	$tr_km_item_ar_list->StartRec = 1;
	$tr_km_item_ar_list->DisplayRecs = $tr_km_item_ar->GridAddRowCount;
	$tr_km_item_ar_list->TotalRecs = $tr_km_item_ar_list->DisplayRecs;
	$tr_km_item_ar_list->StopRec = $tr_km_item_ar_list->DisplayRecs;
} else {
	$bSelectLimit = $tr_km_item_ar_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($tr_km_item_ar_list->TotalRecs <= 0)
			$tr_km_item_ar_list->TotalRecs = $tr_km_item_ar->ListRecordCount();
	} else {
		if (!$tr_km_item_ar_list->Recordset && ($tr_km_item_ar_list->Recordset = $tr_km_item_ar_list->LoadRecordset()))
			$tr_km_item_ar_list->TotalRecs = $tr_km_item_ar_list->Recordset->RecordCount();
	}
	$tr_km_item_ar_list->StartRec = 1;
	if ($tr_km_item_ar_list->DisplayRecs <= 0 || ($tr_km_item_ar->Export <> "" && $tr_km_item_ar->ExportAll)) // Display all records
		$tr_km_item_ar_list->DisplayRecs = $tr_km_item_ar_list->TotalRecs;
	if (!($tr_km_item_ar->Export <> "" && $tr_km_item_ar->ExportAll))
		$tr_km_item_ar_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$tr_km_item_ar_list->Recordset = $tr_km_item_ar_list->LoadRecordset($tr_km_item_ar_list->StartRec-1, $tr_km_item_ar_list->DisplayRecs);

	// Set no record found message
	if ($tr_km_item_ar->CurrentAction == "" && $tr_km_item_ar_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$tr_km_item_ar_list->setWarningMessage(ew_DeniedMsg());
		if ($tr_km_item_ar_list->SearchWhere == "0=101")
			$tr_km_item_ar_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$tr_km_item_ar_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
}
$tr_km_item_ar_list->RenderOtherOptions();
?>
<?php $tr_km_item_ar_list->ShowPageHeader(); ?>
<?php
$tr_km_item_ar_list->ShowMessage();
?>
<?php if ($tr_km_item_ar_list->TotalRecs > 0 || $tr_km_item_ar->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($tr_km_item_ar_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> tr_km_item_ar">
<?php if ($tr_km_item_ar->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($tr_km_item_ar->CurrentAction <> "gridadd" && $tr_km_item_ar->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($tr_km_item_ar_list->Pager)) $tr_km_item_ar_list->Pager = new cPrevNextPager($tr_km_item_ar_list->StartRec, $tr_km_item_ar_list->DisplayRecs, $tr_km_item_ar_list->TotalRecs, $tr_km_item_ar_list->AutoHidePager) ?>
<?php if ($tr_km_item_ar_list->Pager->RecordCount > 0 && $tr_km_item_ar_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($tr_km_item_ar_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $tr_km_item_ar_list->PageUrl() ?>start=<?php echo $tr_km_item_ar_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($tr_km_item_ar_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $tr_km_item_ar_list->PageUrl() ?>start=<?php echo $tr_km_item_ar_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $tr_km_item_ar_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($tr_km_item_ar_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $tr_km_item_ar_list->PageUrl() ?>start=<?php echo $tr_km_item_ar_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($tr_km_item_ar_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $tr_km_item_ar_list->PageUrl() ?>start=<?php echo $tr_km_item_ar_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $tr_km_item_ar_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($tr_km_item_ar_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $tr_km_item_ar_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $tr_km_item_ar_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $tr_km_item_ar_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($tr_km_item_ar_list->TotalRecs > 0 && (!$tr_km_item_ar_list->AutoHidePageSizeSelector || $tr_km_item_ar_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="tr_km_item_ar">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="10"<?php if ($tr_km_item_ar_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($tr_km_item_ar_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($tr_km_item_ar_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($tr_km_item_ar_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
<option value="ALL"<?php if ($tr_km_item_ar->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($tr_km_item_ar_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="ftr_km_item_arlist" id="ftr_km_item_arlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tr_km_item_ar_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tr_km_item_ar_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tr_km_item_ar">
<?php if ($tr_km_item_ar->getCurrentMasterTable() == "tr_km_master_ar" && $tr_km_item_ar->CurrentAction <> "") { ?>
<input type="hidden" name="<?php echo EW_TABLE_SHOW_MASTER ?>" value="tr_km_master_ar">
<input type="hidden" name="fk_row_id" value="<?php echo $tr_km_item_ar->master_id->getSessionValue() ?>">
<?php } ?>
<div id="gmp_tr_km_item_ar" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($tr_km_item_ar_list->TotalRecs > 0 || $tr_km_item_ar->CurrentAction == "gridedit") { ?>
<table id="tbl_tr_km_item_arlist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$tr_km_item_ar_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$tr_km_item_ar_list->RenderListOptions();

// Render list options (header, left)
$tr_km_item_ar_list->ListOptions->Render("header", "left");
?>
...
$tr_km_item_ar_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($tr_km_item_ar->ExportAll && $tr_km_item_ar->Export <> "") {
	$tr_km_item_ar_list->StopRec = $tr_km_item_ar_list->TotalRecs;
} else {

	// Set the last record to display
	if ($tr_km_item_ar_list->TotalRecs > $tr_km_item_ar_list->StartRec + $tr_km_item_ar_list->DisplayRecs - 1)
		$tr_km_item_ar_list->StopRec = $tr_km_item_ar_list->StartRec + $tr_km_item_ar_list->DisplayRecs - 1;
	else
		$tr_km_item_ar_list->StopRec = $tr_km_item_ar_list->TotalRecs;
}
...
if ($objForm) {
	$objForm->Index = -1;
	if ($objForm->HasValue($tr_km_item_ar_list->FormKeyCountName) && ($tr_km_item_ar->CurrentAction == "gridadd" || $tr_km_item_ar->CurrentAction == "gridedit" || $tr_km_item_ar->CurrentAction == "F")) {
		$tr_km_item_ar_list->KeyCount = $objForm->GetValue($tr_km_item_ar_list->FormKeyCountName);
		$tr_km_item_ar_list->StopRec = $tr_km_item_ar_list->StartRec + $tr_km_item_ar_list->KeyCount - 1;
	}
}
$tr_km_item_ar_list->RecCnt = $tr_km_item_ar_list->StartRec - 1;
if ($tr_km_item_ar_list->Recordset && !$tr_km_item_ar_list->Recordset->EOF) {
	$tr_km_item_ar_list->Recordset->MoveFirst();
	$bSelectLimit = $tr_km_item_ar_list->UseSelectLimit;
	if (!$bSelectLimit && $tr_km_item_ar_list->StartRec > 1)
		$tr_km_item_ar_list->Recordset->Move($tr_km_item_ar_list->StartRec - 1);
} elseif (!$tr_km_item_ar->AllowAddDeleteRow && $tr_km_item_ar_list->StopRec == 0) {
	$tr_km_item_ar_list->StopRec = $tr_km_item_ar->GridAddRowCount;
}

// Initialize aggregate
$tr_km_item_ar->RowType = EW_ROWTYPE_AGGREGATEINIT;
$tr_km_item_ar->ResetAttrs();
$tr_km_item_ar_list->RenderRow();
if ($tr_km_item_ar->CurrentAction == "gridadd")
	$tr_km_item_ar_list->RowIndex = 0;
if ($tr_km_item_ar->CurrentAction == "gridedit")
	$tr_km_item_ar_list->RowIndex = 0;
while ($tr_km_item_ar_list->RecCnt < $tr_km_item_ar_list->StopRec) {
	$tr_km_item_ar_list->RecCnt++;
	if (intval($tr_km_item_ar_list->RecCnt) >= intval($tr_km_item_ar_list->StartRec)) {
		$tr_km_item_ar_list->RowCnt++;
		if ($tr_km_item_ar->CurrentAction == "gridadd" || $tr_km_item_ar->CurrentAction == "gridedit" || $tr_km_item_ar->CurrentAction == "F") {
			$tr_km_item_ar_list->RowIndex++;
			$objForm->Index = $tr_km_item_ar_list->RowIndex;
			if ($objForm->HasValue($tr_km_item_ar_list->FormActionName))
				$tr_km_item_ar_list->RowAction = strval($objForm->GetValue($tr_km_item_ar_list->FormActionName));
			elseif ($tr_km_item_ar->CurrentAction == "gridadd")
				$tr_km_item_ar_list->RowAction = "insert";
			else
				$tr_km_item_ar_list->RowAction = "";
		}

		// Set up key count
		$tr_km_item_ar_list->KeyCount = $tr_km_item_ar_list->RowIndex;

		// Init row class and style
		$tr_km_item_ar->ResetAttrs();
		$tr_km_item_ar->CssClass = "";
		if ($tr_km_item_ar->CurrentAction == "gridadd") {
			$tr_km_item_ar_list->LoadRowValues(); // Load default values
		} else {
			$tr_km_item_ar_list->LoadRowValues($tr_km_item_ar_list->Recordset); // Load row values
		}
		...
		if ($tr_km_item_ar->CurrentAction == "gridadd" && $tr_km_item_ar->EventCancelled && !$objForm->HasValue("k_blankrow")) // Insert failed
			$tr_km_item_ar_list->RestoreCurrentRowFormValues($tr_km_item_ar_list->RowIndex); // Restore form values
		if ($tr_km_item_ar->CurrentAction == "gridedit") { // Grid edit
			if ($tr_km_item_ar->EventCancelled) {
				$tr_km_item_ar_list->RestoreCurrentRowFormValues($tr_km_item_ar_list->RowIndex); // Restore form values
			}
			if ($tr_km_item_ar_list->RowAction == "insert")
				$tr_km_item_ar->RowType = EW_ROWTYPE_ADD; // Render add
			else
		...
		if ($tr_km_item_ar->RowType == EW_ROWTYPE_EDIT) // Edit row
			$tr_km_item_ar_list->EditRowCnt++;

		// Set up row id / data-rowindex
		$tr_km_item_ar->RowAttrs = array_merge($tr_km_item_ar->RowAttrs, array('data-rowindex'=>$tr_km_item_ar_list->RowCnt, 'id'=>'r' . $tr_km_item_ar_list->RowCnt . '_tr_km_item_ar', 'data-rowtype'=>$tr_km_item_ar->RowType));
		...
		if ($tr_km_item_ar_list->RowAction <> "delete" && $tr_km_item_ar_list->RowAction <> "insertdelete" && !($tr_km_item_ar_list->RowAction == "insert" && $tr_km_item_ar->CurrentAction == "F" && $tr_km_item_ar_list->EmptyRow())) {
?>
	<tr<?php echo $tr_km_item_ar->RowAttributes() ?>>
<?php

// Render list options (body, left)
$tr_km_item_ar_list->ListOptions->Render("body", "left", $tr_km_item_ar_list->RowCnt);
?>
	<?php if ($tr_km_item_ar->customer_id->Visible) { // customer_id ?>
...
<span id="el<?php echo $tr_km_item_ar_list->RowCnt ?>_tr_km_item_ar_customer_id" class="form-group tr_km_item_ar_customer_id">
...
<div class="ewDropdownList has-feedback">
	...
	<?php } ?>
	<div id="dsl_x<?php echo $tr_km_item_ar_list->RowIndex ?>_customer_id" data-repeatcolumn="1" class="dropdown-menu">
		<div class="ewItems" style="position: relative; overflow-x: hidden;">
<?php echo $tr_km_item_ar->customer_id->RadioButtonListHtml(TRUE, "x{$tr_km_item_ar_list->RowIndex}_customer_id") ?>
		</div>
	</div>
	<div id="tp_x<?php echo $tr_km_item_ar_list->RowIndex ?>_customer_id" class="ewTemplate"><input type="radio" data-table="tr_km_item_ar" data-field="x_customer_id" data-value-separator="<?php echo $tr_km_item_ar->customer_id->DisplayValueSeparatorAttribute() ?>" name="x<?php echo $tr_km_item_ar_list->RowIndex ?>_customer_id" id="x<?php echo $tr_km_item_ar_list->RowIndex ?>_customer_id" value="{value}"<?php echo $tr_km_item_ar->customer_id->EditAttributes() ?>></div>
</div>
</span>
<input type="hidden" data-table="tr_km_item_ar" data-field="x_customer_id" name="o<?php echo $tr_km_item_ar_list->RowIndex ?>_customer_id" id="o<?php echo $tr_km_item_ar_list->RowIndex ?>_customer_id" value="<?php echo ew_HtmlEncode($tr_km_item_ar->customer_id->OldValue) ?>">
<?php } ?>
...
<span id="el<?php echo $tr_km_item_ar_list->RowCnt ?>_tr_km_item_ar_customer_id" class="tr_km_item_ar_customer_id">
...
<?php } ?>
	...
	<?php } ?>
<?php if ($tr_km_item_ar->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<input type="hidden" data-table="tr_km_item_ar" data-field="x_row_id" name="x<?php echo $tr_km_item_ar_list->RowIndex ?>_row_id" id="x<?php echo $tr_km_item_ar_list->RowIndex ?>_row_id" value="<?php echo ew_HtmlEncode($tr_km_item_ar->row_id->CurrentValue) ?>">
<input type="hidden" data-table="tr_km_item_ar" data-field="x_row_id" name="o<?php echo $tr_km_item_ar_list->RowIndex ?>_row_id" id="o<?php echo $tr_km_item_ar_list->RowIndex ?>_row_id" value="<?php echo ew_HtmlEncode($tr_km_item_ar->row_id->OldValue) ?>">
<?php } ?>
...
<?php if ($tr_km_item_ar->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $tr_km_item_ar_list->RowCnt ?>_tr_km_item_ar_inv_number" class="form-group tr_km_item_ar_inv_number">
...
<div class="ewDropdownList has-feedback">
	...
	<span class="form-control-feedback"><span class="caret"></span></span>
	<?php } ?>
	<div id="dsl_x<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_number" data-repeatcolumn="1" class="dropdown-menu">
		<div class="ewItems" style="position: relative; overflow-x: hidden;">
<?php echo $tr_km_item_ar->inv_number->RadioButtonListHtml(TRUE, "x{$tr_km_item_ar_list->RowIndex}_inv_number") ?>
		</div>
	</div>
	<div id="tp_x<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_number" class="ewTemplate"><input type="radio" data-table="tr_km_item_ar" data-field="x_inv_number" data-value-separator="<?php echo $tr_km_item_ar->inv_number->DisplayValueSeparatorAttribute() ?>" name="x<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_number" id="x<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_number" value="{value}"<?php echo $tr_km_item_ar->inv_number->EditAttributes() ?>></div>
</div>
<input type="hidden" name="ln_x<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_number" id="ln_x<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_number" value="x<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_date,x<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_amt,x<?php echo $tr_km_item_ar_list->RowIndex ?>_paid_amt">
</span>
<input type="hidden" data-table="tr_km_item_ar" data-field="x_inv_number" name="o<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_number" id="o<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_number" value="<?php echo ew_HtmlEncode($tr_km_item_ar->inv_number->OldValue) ?>">
<?php } ?>
<?php if ($tr_km_item_ar->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $tr_km_item_ar_list->RowCnt ?>_tr_km_item_ar_inv_number" class="form-group tr_km_item_ar_inv_number">
<?php $tr_km_item_ar->inv_number->EditAttrs["onclick"] = "ew_AutoFill(this); " . @$tr_km_item_ar->inv_number->EditAttrs["onclick"]; ?>
...
<?php } ?>
<?php if ($tr_km_item_ar->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $tr_km_item_ar_list->RowCnt ?>_tr_km_item_ar_inv_number" class="tr_km_item_ar_inv_number">
...
<?php if ($tr_km_item_ar->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $tr_km_item_ar_list->RowCnt ?>_tr_km_item_ar_inv_date" class="form-group tr_km_item_ar_inv_date">
<input type="text" data-table="tr_km_item_ar" data-field="x_inv_date" name="x<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_date" id="x<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_date" size="10" placeholder="<?php echo ew_HtmlEncode($tr_km_item_ar->inv_date->getPlaceHolder()) ?>" value="<?php echo $tr_km_item_ar->inv_date->EditValue ?>"<?php echo $tr_km_item_ar->inv_date->EditAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("ftr_km_item_arlist", "x<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php } ?>
</span>
<input type="hidden" data-table="tr_km_item_ar" data-field="x_inv_date" name="o<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_date" id="o<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_date" value="<?php echo ew_HtmlEncode($tr_km_item_ar->inv_date->OldValue) ?>">
<?php } ?>
<?php if ($tr_km_item_ar->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $tr_km_item_ar_list->RowCnt ?>_tr_km_item_ar_inv_date" class="form-group tr_km_item_ar_inv_date">
<input type="text" data-table="tr_km_item_ar" data-field="x_inv_date" name="x<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_date" id="x<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_date" size="10" placeholder="<?php echo ew_HtmlEncode($tr_km_item_ar->inv_date->getPlaceHolder()) ?>" value="<?php echo $tr_km_item_ar->inv_date->EditValue ?>"<?php echo $tr_km_item_ar->inv_date->EditAttributes() ?>>
...
<?php } ?>
<?php if ($tr_km_item_ar->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $tr_km_item_ar_list->RowCnt ?>_tr_km_item_ar_inv_date" class="tr_km_item_ar_inv_date">
...
<?php if ($tr_km_item_ar->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $tr_km_item_ar_list->RowCnt ?>_tr_km_item_ar_inv_amt" class="form-group tr_km_item_ar_inv_amt">
<input type="text" data-table="tr_km_item_ar" data-field="x_inv_amt" name="x<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_amt" id="x<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_amt" size="30" placeholder="<?php echo ew_HtmlEncode($tr_km_item_ar->inv_amt->getPlaceHolder()) ?>" value="<?php echo $tr_km_item_ar->inv_amt->EditValue ?>"<?php echo $tr_km_item_ar->inv_amt->EditAttributes() ?>>
</span>
<input type="hidden" data-table="tr_km_item_ar" data-field="x_inv_amt" name="o<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_amt" id="o<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_amt" value="<?php echo ew_HtmlEncode($tr_km_item_ar->inv_amt->OldValue) ?>">
<?php } ?>
<?php if ($tr_km_item_ar->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $tr_km_item_ar_list->RowCnt ?>_tr_km_item_ar_inv_amt" class="form-group tr_km_item_ar_inv_amt">
<input type="text" data-table="tr_km_item_ar" data-field="x_inv_amt" name="x<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_amt" id="x<?php echo $tr_km_item_ar_list->RowIndex ?>_inv_amt" size="30" placeholder="<?php echo ew_HtmlEncode($tr_km_item_ar->inv_amt->getPlaceHolder()) ?>" value="<?php echo $tr_km_item_ar->inv_amt->EditValue ?>"<?php echo $tr_km_item_ar->inv_amt->EditAttributes() ?>>
...
<?php } ?>
<?php if ($tr_km_item_ar->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $tr_km_item_ar_list->RowCnt ?>_tr_km_item_ar_inv_amt" class="tr_km_item_ar_inv_amt">
...
<?php if ($tr_km_item_ar->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $tr_km_item_ar_list->RowCnt ?>_tr_km_item_ar_paid_amt" class="form-group tr_km_item_ar_paid_amt">
<input type="text" data-table="tr_km_item_ar" data-field="x_paid_amt" name="x<?php echo $tr_km_item_ar_list->RowIndex ?>_paid_amt" id="x<?php echo $tr_km_item_ar_list->RowIndex ?>_paid_amt" size="30" placeholder="<?php echo ew_HtmlEncode($tr_km_item_ar->paid_amt->getPlaceHolder()) ?>" value="<?php echo $tr_km_item_ar->paid_amt->EditValue ?>"<?php echo $tr_km_item_ar->paid_amt->EditAttributes() ?>>
</span>
<input type="hidden" data-table="tr_km_item_ar" data-field="x_paid_amt" name="o<?php echo $tr_km_item_ar_list->RowIndex ?>_paid_amt" id="o<?php echo $tr_km_item_ar_list->RowIndex ?>_paid_amt" value="<?php echo ew_HtmlEncode($tr_km_item_ar->paid_amt->OldValue) ?>">
<?php } ?>
<?php if ($tr_km_item_ar->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $tr_km_item_ar_list->RowCnt ?>_tr_km_item_ar_paid_amt" class="form-group tr_km_item_ar_paid_amt">
<input type="text" data-table="tr_km_item_ar" data-field="x_paid_amt" name="x<?php echo $tr_km_item_ar_list->RowIndex ?>_paid_amt" id="x<?php echo $tr_km_item_ar_list->RowIndex ?>_paid_amt" size="30" placeholder="<?php echo ew_HtmlEncode($tr_km_item_ar->paid_amt->getPlaceHolder()) ?>" value="<?php echo $tr_km_item_ar->paid_amt->EditValue ?>"<?php echo $tr_km_item_ar->paid_amt->EditAttributes() ?>>
...
<?php } ?>
<?php if ($tr_km_item_ar->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $tr_km_item_ar_list->RowCnt ?>_tr_km_item_ar_paid_amt" class="tr_km_item_ar_paid_amt">
...
$tr_km_item_ar_list->ListOptions->Render("body", "right", $tr_km_item_ar_list->RowCnt);
?>
	</tr>
<?php if ($tr_km_item_ar->RowType == EW_ROWTYPE_ADD || $tr_km_item_ar->RowType == EW_ROWTYPE_EDIT) { ?>
<script type="text/javascript">
ftr_km_item_arlist.UpdateOpts(<?php echo $tr_km_item_ar_list->RowIndex ?>);
</script>
<?php } ?>
<?php
	}
	...
	if ($tr_km_item_ar->CurrentAction <> "gridadd")
		if (!$tr_km_item_ar_list->Recordset->EOF) $tr_km_item_ar_list->Recordset->MoveNext();
}
?>
<?php
	if ($tr_km_item_ar->CurrentAction == "gridadd" || $tr_km_item_ar->CurrentAction == "gridedit") {
		$tr_km_item_ar_list->RowIndex = '$rowindex$';
		$tr_km_item_ar_list->LoadRowValues();

		// Set row properties
		$tr_km_item_ar->ResetAttrs();
		$tr_km_item_ar->RowAttrs = array_merge($tr_km_item_ar->RowAttrs, array('data-rowindex'=>$tr_km_item_ar_list->RowIndex, 'id'=>'r0_tr_km_item_ar', 'data-rowtype'=>EW_ROWTYPE_ADD));
		...
		// Render list options
		$tr_km_item_ar_list->RenderListOptions();
		$tr_km_item_ar_list->StartRowCnt = 0;
?>
	<tr<?php echo $tr_km_item_ar->RowAttributes() ?>>
<?php

// Render list options (body, left)
$tr_km_item_ar_list->ListOptions->Render("body", "left", $tr_km_item_ar_list->RowIndex);
?>
	<?php if ($tr_km_item_ar->customer_id->Visible) { // customer_id ?>
...
// Render list options (body, right)
$tr_km_item_ar_list->ListOptions->Render("body", "right", $tr_km_item_ar_list->RowIndex);
?>
<script type="text/javascript">
ftr_km_item_arlist.UpdateOpts(<?php echo $tr_km_item_ar_list->RowIndex ?>);
</script>
...
<?php } ?>
<?php if ($tr_km_item_ar->CurrentAction == "gridadd") { ?>
<input type="hidden" name="a_list" id="a_list" value="gridinsert">
<input type="hidden" name="<?php echo $tr_km_item_ar_list->FormKeyCountName ?>" id="<?php echo $tr_km_item_ar_list->FormKeyCountName ?>" value="<?php echo $tr_km_item_ar_list->KeyCount ?>">
<?php echo $tr_km_item_ar_list->MultiSelectKey ?>
<?php } ?>
<?php if ($tr_km_item_ar->CurrentAction == "gridedit") { ?>
<input type="hidden" name="a_list" id="a_list" value="gridupdate">
<input type="hidden" name="<?php echo $tr_km_item_ar_list->FormKeyCountName ?>" id="<?php echo $tr_km_item_ar_list->FormKeyCountName ?>" value="<?php echo $tr_km_item_ar_list->KeyCount ?>">
<?php echo $tr_km_item_ar_list->MultiSelectKey ?>
<?php } ?>
<?php if ($tr_km_item_ar->CurrentAction == "") { ?>
...
// Close recordset
if ($tr_km_item_ar_list->Recordset)
	$tr_km_item_ar_list->Recordset->Close();
?>
<?php if ($tr_km_item_ar->Export == "") { ?>
...
<?php } ?>
<?php if ($tr_km_item_ar_list->TotalRecs == 0 && $tr_km_item_ar->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($tr_km_item_ar_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($tr_km_item_ar->Export == "") { ?>
<script type="text/javascript">
ftr_km_item_arlist.Init();
</script>
<?php } ?>
<?php
$tr_km_item_ar_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($tr_km_item_ar->Export == "") { ?>
...
<?php
$tr_km_item_ar_list->Page_Terminate();
?>
