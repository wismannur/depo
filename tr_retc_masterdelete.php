...
$tr_retc_master_delete = NULL; // Initialize page object first

class ctr_retc_master_delete extends ctr_retc_master {
	...
	// Page object name
	var $PageObjName = 'tr_retc_master_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("tr_retc_masterlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in tr_retc_master class, tr_retc_masterinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("tr_retc_masterlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($tr_retc_master_delete)) $tr_retc_master_delete = new ctr_retc_master_delete();

// Page init
$tr_retc_master_delete->Page_Init();

// Page main
$tr_retc_master_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tr_retc_master_delete->Page_Render();
?>
...
var CurrentForm = ftr_retc_masterdelete = new ew_Form("ftr_retc_masterdelete", "delete");

// Form_CustomValidate event
ftr_retc_masterdelete.Form_CustomValidate = 
...
ftr_retc_masterdelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
ftr_retc_masterdelete.Lists["x_sjc_number"] = {"LinkField":"x_sjc_number","Ajax":true,"AutoFill":false,"DisplayFields":["x_sjc_number","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tr_sjc_master"};
ftr_retc_masterdelete.Lists["x_sjc_number"].Data = "<?php echo $tr_retc_master_delete->sjc_number->LookupFilterQuery(FALSE, "delete") ?>";
ftr_retc_masterdelete.Lists["x_sales_id"] = {"LinkField":"x_sales_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_sales_name","","",""],"ParentFields":[],"ChildFields":["x_sjc_number"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_salesman"};
ftr_retc_masterdelete.Lists["x_sales_id"].Data = "<?php echo $tr_retc_master_delete->sales_id->LookupFilterQuery(FALSE, "delete") ?>";
...
<?php $tr_retc_master_delete->ShowPageHeader(); ?>
<?php
$tr_retc_master_delete->ShowMessage();
?>
<form name="ftr_retc_masterdelete" id="ftr_retc_masterdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tr_retc_master_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tr_retc_master_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tr_retc_master">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($tr_retc_master_delete->RecKeys as $key) { ?>
...
<?php if ($tr_retc_master->retc_number->Visible) { // retc_number ?>
		<th class="<?php echo $tr_retc_master->retc_number->HeaderCellClass() ?>"><span id="elh_tr_retc_master_retc_number" class="tr_retc_master_retc_number"><?php echo $tr_retc_master->retc_number->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_retc_master->retc_date->Visible) { // retc_date ?>
		<th class="<?php echo $tr_retc_master->retc_date->HeaderCellClass() ?>"><span id="elh_tr_retc_master_retc_date" class="tr_retc_master_retc_date"><?php echo $tr_retc_master->retc_date->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_retc_master->sjc_number->Visible) { // sjc_number ?>
		<th class="<?php echo $tr_retc_master->sjc_number->HeaderCellClass() ?>"><span id="elh_tr_retc_master_sjc_number" class="tr_retc_master_sjc_number"><?php echo $tr_retc_master->sjc_number->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_retc_master->retc_notes->Visible) { // retc_notes ?>
		<th class="<?php echo $tr_retc_master->retc_notes->HeaderCellClass() ?>"><span id="elh_tr_retc_master_retc_notes" class="tr_retc_master_retc_notes"><?php echo $tr_retc_master->retc_notes->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_retc_master->sales_id->Visible) { // sales_id ?>
		<th class="<?php echo $tr_retc_master->sales_id->HeaderCellClass() ?>"><span id="elh_tr_retc_master_sales_id" class="tr_retc_master_sales_id"><?php echo $tr_retc_master->sales_id->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$tr_retc_master_delete->RecCnt = 0;
$i = 0;
while (!$tr_retc_master_delete->Recordset->EOF) {
	$tr_retc_master_delete->RecCnt++;
	$tr_retc_master_delete->RowCnt++;

	// Set row properties
	$tr_retc_master->ResetAttrs();
	$tr_retc_master->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$tr_retc_master_delete->LoadRowValues($tr_retc_master_delete->Recordset);

	// Render row
	$tr_retc_master_delete->RenderRow();
?>
	<tr<?php echo $tr_retc_master->RowAttributes() ?>>
<?php if ($tr_retc_master->retc_number->Visible) { // retc_number ?>
		<td<?php echo $tr_retc_master->retc_number->CellAttributes() ?>>
<span id="el<?php echo $tr_retc_master_delete->RowCnt ?>_tr_retc_master_retc_number" class="tr_retc_master_retc_number">
...
<span id="el<?php echo $tr_retc_master_delete->RowCnt ?>_tr_retc_master_retc_date" class="tr_retc_master_retc_date">
...
<span id="el<?php echo $tr_retc_master_delete->RowCnt ?>_tr_retc_master_sjc_number" class="tr_retc_master_sjc_number">
...
<span id="el<?php echo $tr_retc_master_delete->RowCnt ?>_tr_retc_master_retc_notes" class="tr_retc_master_retc_notes">
...
<span id="el<?php echo $tr_retc_master_delete->RowCnt ?>_tr_retc_master_sales_id" class="tr_retc_master_sales_id">
...
<?php
	$tr_retc_master_delete->Recordset->MoveNext();
}
$tr_retc_master_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tr_retc_master_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ftr_retc_masterdelete.Init();
</script>
<?php
$tr_retc_master_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tr_retc_master_delete->Page_Terminate();
?>
