...
$tr_ret_master_add = NULL; // Initialize page object first

class ctr_ret_master_add extends ctr_ret_master {
	...
	// Page object name
	var $PageObjName = 'tr_ret_master_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["ret_id"] != "") {
				...
				$this->setKey("ret_id", $this->ret_id->CurrentValue); // Set up key
			} else {
				$this->setKey("ret_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "tr_ret_masterview.php")
	...
	function LoadDefaultValues() {
		...
		$this->ret_id->OldValue = $this->ret_id->CurrentValue;
		$this->ret_number->CurrentValue = NULL;
		$this->ret_number->OldValue = $this->ret_number->CurrentValue;
		$this->ret_date->CurrentValue = ew_CurrentDate();
		...
		$this->ret_amt->CurrentValue = 0;
		$this->disc_total->CurrentValue = 0;
		$this->ret_total->CurrentValue = 0;
	...
	function NewRow() {
		...
		$row = array();
		$row['ret_id'] = $this->ret_id->CurrentValue;
		$row['ret_number'] = $this->ret_number->CurrentValue;
		$row['ret_date'] = $this->ret_date->CurrentValue;
		...
		$row['ret_amt'] = $this->ret_amt->CurrentValue;
		$row['disc_total'] = $this->disc_total->CurrentValue;
		$row['ret_total'] = $this->ret_total->CurrentValue;
	...
	function ValidateForm() {
		...
		if (in_array("tr_ret_item", $DetailTblVar) && $GLOBALS["tr_ret_item"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		// ret_number
		$this->ret_number->SetDbValueDef($rsnew, $this->ret_number->CurrentValue, NULL, FALSE);

		// ret_date
		$this->ret_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->ret_date->CurrentValue, 7), NULL, FALSE);
		...
		// ret_amt
		$this->ret_amt->SetDbValueDef($rsnew, $this->ret_amt->CurrentValue, NULL, FALSE);

		// disc_total
		$this->disc_total->SetDbValueDef($rsnew, $this->disc_total->CurrentValue, NULL, FALSE);

		// ret_total
		$this->ret_total->SetDbValueDef($rsnew, $this->ret_total->CurrentValue, NULL, FALSE);
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("tr_ret_item", $DetailTblVar) && $GLOBALS["tr_ret_item"]->DetailAdd) {
				$GLOBALS["tr_ret_item"]->master_id->setSessionValue($this->ret_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["tr_ret_item_grid"])) $GLOBALS["tr_ret_item_grid"] = new ctr_ret_item_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "tr_ret_item"); // Load user level of detail table
				$AddRow = $GLOBALS["tr_ret_item_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["tr_ret_item"]->master_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("tr_ret_item", $DetailTblVar)) {
				...
				if ($GLOBALS["tr_ret_item_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["tr_ret_item_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["tr_ret_item_grid"]->CurrentMode = "add";
					$GLOBALS["tr_ret_item_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($tr_ret_master_add)) $tr_ret_master_add = new ctr_ret_master_add();

// Page init
$tr_ret_master_add->Page_Init();

// Page main
$tr_ret_master_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tr_ret_master_add->Page_Render();
?>
...
var CurrentForm = ftr_ret_masteradd = new ew_Form("ftr_ret_masteradd", "add");

// Validate form
ftr_ret_masteradd.Validate = function() {
...
ftr_ret_masteradd.Form_CustomValidate = 
...
ftr_ret_masteradd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
ftr_ret_masteradd.Lists["x_customer_id"] = {"LinkField":"x_customer_id","Ajax":true,"AutoFill":true,"DisplayFields":["x_customer_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_customer"};
ftr_ret_masteradd.Lists["x_customer_id"].Data = "<?php echo $tr_ret_master_add->customer_id->LookupFilterQuery(FALSE, "add") ?>";
ftr_ret_masteradd.AutoSuggests["x_customer_id"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $tr_ret_master_add->customer_id->LookupFilterQuery(TRUE, "add"))) ?>;
...
<?php $tr_ret_master_add->ShowPageHeader(); ?>
<?php
$tr_ret_master_add->ShowMessage();
?>
<form name="ftr_ret_masteradd" id="ftr_ret_masteradd" class="<?php echo $tr_ret_master_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tr_ret_master_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tr_ret_master_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tr_ret_master">
<input type="hidden" name="a_add" id="a_add" value="A">
<input type="hidden" name="modal" value="<?php echo intval($tr_ret_master_add->IsModal) ?>">
<div id="tpd_tr_ret_masteradd" class="ewCustomTemplate"></div>
<script id="tpm_tr_ret_masteradd" type="text/html">
<div id="ct_tr_ret_master_add"><div class="col-sm-12 panel-custom" style="">
	<div class="row">
...
<?php if (!$tr_ret_master_add->IsMobileOrModal) { ?>
<div class="ewDesktop"><!-- desktop -->
<?php } ?>
<?php if ($tr_ret_master_add->IsMobileOrModal) { ?>
<div class="ewAddDiv hidden"><!-- page* -->
<?php } else { ?>
<table id="tbl_tr_ret_masteradd" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable hidden"><!-- table* -->
<?php } ?>
<?php if ($tr_ret_master->ret_number->Visible) { // ret_number ?>
<?php if ($tr_ret_master_add->IsMobileOrModal) { ?>
	<div id="r_ret_number" class="form-group">
		<label id="elh_tr_ret_master_ret_number" for="x_ret_number" class="<?php echo $tr_ret_master_add->LeftColumnClass ?>"><script id="tpc_tr_ret_master_ret_number" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->ret_number->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_ret_master_add->RightColumnClass ?>"><div<?php echo $tr_ret_master->ret_number->CellAttributes() ?>>
<script id="tpx_tr_ret_master_ret_number" class="tr_ret_masteradd" type="text/html">
...
<?php } else { ?>
	<tr id="r_ret_number">
		<td class="col-sm-2"><span id="elh_tr_ret_master_ret_number"><script id="tpc_tr_ret_master_ret_number" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->ret_number->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_ret_master_add->IsMobileOrModal) { ?>
	<div id="r_ret_date" class="form-group">
		<label id="elh_tr_ret_master_ret_date" for="x_ret_date" class="<?php echo $tr_ret_master_add->LeftColumnClass ?>"><script id="tpc_tr_ret_master_ret_date" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->ret_date->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_ret_master_add->RightColumnClass ?>"><div<?php echo $tr_ret_master->ret_date->CellAttributes() ?>>
<script id="tpx_tr_ret_master_ret_date" class="tr_ret_masteradd" type="text/html">
...
<script type="text/html" class="tr_ret_masteradd_js">
ew_CreateDateTimePicker("ftr_ret_masteradd", "x_ret_date", {"ignoreReadonly":true,"useCurrent":false,"format":7});
</script>
<?php echo $tr_ret_master->ret_date->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_ret_date">
		<td class="col-sm-2"><span id="elh_tr_ret_master_ret_date"><script id="tpc_tr_ret_master_ret_date" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->ret_date->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_ret_master_add->IsMobileOrModal) { ?>
	<div id="r_customer_id" class="form-group">
		<label id="elh_tr_ret_master_customer_id" class="<?php echo $tr_ret_master_add->LeftColumnClass ?>"><script id="tpc_tr_ret_master_customer_id" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->customer_id->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_ret_master_add->RightColumnClass ?>"><div<?php echo $tr_ret_master->customer_id->CellAttributes() ?>>
<script id="tpx_tr_ret_master_customer_id" class="tr_ret_masteradd" type="text/html">
...
<script type="text/html" class="tr_ret_masteradd_js">
ftr_ret_masteradd.CreateAutoSuggest({"id":"x_customer_id","forceSelect":true});
</script>
<?php echo $tr_ret_master->customer_id->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_customer_id">
		<td class="col-sm-2"><span id="elh_tr_ret_master_customer_id"><script id="tpc_tr_ret_master_customer_id" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->customer_id->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_ret_master_add->IsMobileOrModal) { ?>
	<div id="r_address1" class="form-group">
		<label id="elh_tr_ret_master_address1" for="x_address1" class="<?php echo $tr_ret_master_add->LeftColumnClass ?>"><script id="tpc_tr_ret_master_address1" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->address1->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_ret_master_add->RightColumnClass ?>"><div<?php echo $tr_ret_master->address1->CellAttributes() ?>>
<script id="tpx_tr_ret_master_address1" class="tr_ret_masteradd" type="text/html">
...
<?php } else { ?>
	<tr id="r_address1">
		<td class="col-sm-2"><span id="elh_tr_ret_master_address1"><script id="tpc_tr_ret_master_address1" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->address1->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_ret_master_add->IsMobileOrModal) { ?>
	<div id="r_address2" class="form-group">
		<label id="elh_tr_ret_master_address2" for="x_address2" class="<?php echo $tr_ret_master_add->LeftColumnClass ?>"><script id="tpc_tr_ret_master_address2" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->address2->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_ret_master_add->RightColumnClass ?>"><div<?php echo $tr_ret_master->address2->CellAttributes() ?>>
<script id="tpx_tr_ret_master_address2" class="tr_ret_masteradd" type="text/html">
...
<?php } else { ?>
	<tr id="r_address2">
		<td class="col-sm-2"><span id="elh_tr_ret_master_address2"><script id="tpc_tr_ret_master_address2" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->address2->FldCaption() ?></span></script></span></td>
...
<?php } ?>
<?php } ?>
<?php if ($tr_ret_master->address3->Visible) { // address3 ?>
<?php if ($tr_ret_master_add->IsMobileOrModal) { ?>
	<div id="r_address3" class="form-group">
		<label id="elh_tr_ret_master_address3" class="<?php echo $tr_ret_master_add->LeftColumnClass ?>"><script id="tpc_tr_ret_master_address3" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->address3->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_ret_master_add->RightColumnClass ?>"><div<?php echo $tr_ret_master->address3->CellAttributes() ?>>
<script id="tpx_tr_ret_master_address3" class="tr_ret_masteradd" type="text/html">
<span id="el_tr_ret_master_address3">
<input type="text" data-table="tr_ret_master" data-field="x_address3" name="x_address3" id="x_address3" size="50" maxlength="50" placeholder="<?php echo ew_HtmlEncode($tr_ret_master->address3->getPlaceHolder()) ?>" value="<?php echo $tr_ret_master->address3->EditValue ?>"<?php echo $tr_ret_master->address3->EditAttributes() ?>>
</span>
</script>
<?php echo $tr_ret_master->address3->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_address3">
		<td class="col-sm-2"><span id="elh_tr_ret_master_address3"><script id="tpc_tr_ret_master_address3" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->address3->FldCaption() ?></span></script></span></td>
		<td<?php echo $tr_ret_master->address3->CellAttributes() ?>>
<script id="tpx_tr_ret_master_address3" class="tr_ret_masteradd" type="text/html">
<span id="el_tr_ret_master_address3">
<input type="text" data-table="tr_ret_master" data-field="x_address3" name="x_address3" id="x_address3" size="50" maxlength="50" placeholder="<?php echo ew_HtmlEncode($tr_ret_master->address3->getPlaceHolder()) ?>" value="<?php echo $tr_ret_master->address3->EditValue ?>"<?php echo $tr_ret_master->address3->EditAttributes() ?>>
</span>
</script>
<?php echo $tr_ret_master->address3->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php } ?>
...
<?php if ($tr_ret_master_add->IsMobileOrModal) { ?>
	<div id="r_ret_amt" class="form-group">
		<label id="elh_tr_ret_master_ret_amt" for="x_ret_amt" class="<?php echo $tr_ret_master_add->LeftColumnClass ?>"><script id="tpc_tr_ret_master_ret_amt" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->ret_amt->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_ret_master_add->RightColumnClass ?>"><div<?php echo $tr_ret_master->ret_amt->CellAttributes() ?>>
<script id="tpx_tr_ret_master_ret_amt" class="tr_ret_masteradd" type="text/html">
...
<?php } else { ?>
	<tr id="r_ret_amt">
		<td class="col-sm-2"><span id="elh_tr_ret_master_ret_amt"><script id="tpc_tr_ret_master_ret_amt" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->ret_amt->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_ret_master_add->IsMobileOrModal) { ?>
	<div id="r_disc_total" class="form-group">
		<label id="elh_tr_ret_master_disc_total" for="x_disc_total" class="<?php echo $tr_ret_master_add->LeftColumnClass ?>"><script id="tpc_tr_ret_master_disc_total" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->disc_total->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_ret_master_add->RightColumnClass ?>"><div<?php echo $tr_ret_master->disc_total->CellAttributes() ?>>
<script id="tpx_tr_ret_master_disc_total" class="tr_ret_masteradd" type="text/html">
...
<?php } else { ?>
	<tr id="r_disc_total">
		<td class="col-sm-2"><span id="elh_tr_ret_master_disc_total"><script id="tpc_tr_ret_master_disc_total" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->disc_total->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_ret_master_add->IsMobileOrModal) { ?>
	<div id="r_ret_total" class="form-group">
		<label id="elh_tr_ret_master_ret_total" for="x_ret_total" class="<?php echo $tr_ret_master_add->LeftColumnClass ?>"><script id="tpc_tr_ret_master_ret_total" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->ret_total->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_ret_master_add->RightColumnClass ?>"><div<?php echo $tr_ret_master->ret_total->CellAttributes() ?>>
<script id="tpx_tr_ret_master_ret_total" class="tr_ret_masteradd" type="text/html">
...
<?php } else { ?>
	<tr id="r_ret_total">
		<td class="col-sm-2"><span id="elh_tr_ret_master_ret_total"><script id="tpc_tr_ret_master_ret_total" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->ret_total->FldCaption() ?></span></script></span></td>
...
<?php } ?>
<?php } ?>
<?php if ($tr_ret_master->kode_depo->Visible) { // kode_depo ?>
<?php if ($tr_ret_master_add->IsMobileOrModal) { ?>
	<div id="r_kode_depo" class="form-group">
		<label id="elh_tr_ret_master_kode_depo" class="<?php echo $tr_ret_master_add->LeftColumnClass ?>"><script id="tpc_tr_ret_master_kode_depo" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->kode_depo->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_ret_master_add->RightColumnClass ?>"><div<?php echo $tr_ret_master->kode_depo->CellAttributes() ?>>
<script id="tpx_tr_ret_master_kode_depo" class="tr_ret_masteradd" type="text/html">
<span id="el_tr_ret_master_kode_depo">
<input type="text" data-table="tr_ret_master" data-field="x_kode_depo" name="x_kode_depo" id="x_kode_depo" size="30" maxlength="3" placeholder="<?php echo ew_HtmlEncode($tr_ret_master->kode_depo->getPlaceHolder()) ?>" value="<?php echo $tr_ret_master->kode_depo->EditValue ?>"<?php echo $tr_ret_master->kode_depo->EditAttributes() ?>>
</span>
</script>
<?php echo $tr_ret_master->kode_depo->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_kode_depo">
		<td class="col-sm-2"><span id="elh_tr_ret_master_kode_depo"><script id="tpc_tr_ret_master_kode_depo" class="tr_ret_masteradd" type="text/html"><span><?php echo $tr_ret_master->kode_depo->FldCaption() ?></span></script></span></td>
		<td<?php echo $tr_ret_master->kode_depo->CellAttributes() ?>>
<script id="tpx_tr_ret_master_kode_depo" class="tr_ret_masteradd" type="text/html">
<span id="el_tr_ret_master_kode_depo">
<input type="text" data-table="tr_ret_master" data-field="x_kode_depo" name="x_kode_depo" id="x_kode_depo" size="30" maxlength="3" placeholder="<?php echo ew_HtmlEncode($tr_ret_master->kode_depo->getPlaceHolder()) ?>" value="<?php echo $tr_ret_master->kode_depo->EditValue ?>"<?php echo $tr_ret_master->kode_depo->EditAttributes() ?>>
</span>
</script>
<?php echo $tr_ret_master->kode_depo->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php } ?>
<?php if ($tr_ret_master_add->IsMobileOrModal) { ?>
...
<?php } ?>
<?php
	if (in_array("tr_ret_item", explode(",", $tr_ret_master->getCurrentDetailTable())) && $tr_ret_item->DetailAdd) {
?>
...
<?php } ?>
<?php if (!$tr_ret_master_add->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $tr_ret_master_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tr_ret_master_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
...
ew_ApplyTemplate("tpd_tr_ret_masteradd", "tpm_tr_ret_masteradd", "tr_ret_masteradd", "<?php echo $tr_ret_master->CustomExport ?>", ewVar.templateData.rows[0]);
jQuery("script.tr_ret_masteradd_js").each(function(){ew_AddScript(this.text);});
</script>
<script type="text/javascript">
ftr_ret_masteradd.Init();
</script>
<?php
$tr_ret_master_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tr_ret_master_add->Page_Terminate();
?>
