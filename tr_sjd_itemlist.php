...
$tr_sjd_item_list = NULL; // Initialize page object first

class ctr_sjd_item_list extends ctr_sjd_item {
	...
	// Page object name
	var $PageObjName = 'tr_sjd_item_list';

	// Grid form hidden field names
	var $FormName = 'ftr_sjd_itemlist';
	...
	function __construct() {
		...
		$this->AddUrl = "tr_sjd_itemadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "tr_sjd_itemdelete.php";
		$this->MultiUpdateUrl = "tr_sjd_itemupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ftr_sjd_itemlistsrch";
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "tr_sjd_master") {
			global $tr_sjd_master;
			$rsmaster = $tr_sjd_master->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("tr_sjd_masterlist.php"); // Return to master page
			} else {
				$tr_sjd_master->LoadListRowValues($rsmaster);
				$tr_sjd_master->RowType = EW_ROWTYPE_MASTER; // Master row
				$tr_sjd_master->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function ClearInlineMode() {
		...
		$this->qty_rcv->FormValue = ""; // Clear form value
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_item_qty") && $objForm->HasValue("o_item_qty") && $this->item_qty->CurrentValue <> $this->item_qty->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_qty_rcv") && $objForm->HasValue("o_qty_rcv") && $this->qty_rcv->CurrentValue <> $this->qty_rcv->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_cek_sjd") && $objForm->HasValue("o_cek_sjd") && $this->cek_sjd->CurrentValue <> $this->cek_sjd->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ftr_sjd_itemlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = FALSE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ftr_sjd_itemlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = FALSE;
		...
		if ($this->CurrentAction <> "gridadd" && $this->CurrentAction <> "gridedit") { // Not grid add/edit mode
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.ftr_sjd_itemlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function LoadFormValues() {
		...
		$this->qty_rcv->setOldValue($objForm->GetValue("o_qty_rcv"));
		if (!$this->cek_sjd->FldIsDetailKey) {
			$this->cek_sjd->setFormValue($objForm->GetValue("x_cek_sjd"));
		}
		$this->cek_sjd->setOldValue($objForm->GetValue("o_cek_sjd"));
	...
	function RenderRow() {
		...
		$this->cek_sjd->ViewCustomAttributes = "";
			...
			if (strval($this->qty_rcv->EditValue) <> "" && is_numeric($this->qty_rcv->EditValue)) {
			$this->qty_rcv->EditValue = ew_FormatNumber($this->qty_rcv->EditValue, -2, -2, -2, -2);
			$this->qty_rcv->OldValue = $this->qty_rcv->EditValue;
			}
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_tr_sjd_item\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_tr_sjd_item',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.ftr_sjd_itemlist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
	...
	function ExportData() {
		...
		if (EW_EXPORT_MASTER_RECORD && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "tr_sjd_master") {
			global $tr_sjd_master;
			if (!isset($tr_sjd_master)) $tr_sjd_master = new ctr_sjd_master;
			$rsmaster = $tr_sjd_master->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("v"); // Change to vertical
				if ($this->Export <> "csv" || EW_EXPORT_MASTER_RECORD_FOR_CSV) {
					$Doc->Table = &$tr_sjd_master;
					$tr_sjd_master->ExportDocument($Doc, $rsmaster, 1, 1);
					$Doc->ExportEmptyRow();
					$Doc->Table = &$this;
				}
...
// Create page object
if (!isset($tr_sjd_item_list)) $tr_sjd_item_list = new ctr_sjd_item_list();

// Page init
$tr_sjd_item_list->Page_Init();

// Page main
$tr_sjd_item_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tr_sjd_item_list->Page_Render();
?>
...
var CurrentForm = ftr_sjd_itemlist = new ew_Form("ftr_sjd_itemlist", "list");
ftr_sjd_itemlist.FormKeyCountName = '<?php echo $tr_sjd_item_list->FormKeyCountName ?>';

// Validate form
ftr_sjd_itemlist.Validate = function() {
...
// Check empty row
ftr_sjd_itemlist.EmptyRow = function(infix) {
	var fobj = this.Form;
...
ftr_sjd_itemlist.Form_CustomValidate = 
...
ftr_sjd_itemlist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
ftr_sjd_itemlist.Lists["x_item_id"] = {"LinkField":"x_product_id","Ajax":true,"AutoFill":true,"DisplayFields":["x_product_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_products"};
ftr_sjd_itemlist.Lists["x_item_id"].Data = "<?php echo $tr_sjd_item_list->item_id->LookupFilterQuery(FALSE, "list") ?>";
ftr_sjd_itemlist.AutoSuggests["x_item_id"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $tr_sjd_item_list->item_id->LookupFilterQuery(TRUE, "list"))) ?>;
ftr_sjd_itemlist.Lists["x_unit_id"] = {"LinkField":"x_unit_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_unit_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_unit_detail"};
ftr_sjd_itemlist.Lists["x_unit_id"].Data = "<?php echo $tr_sjd_item_list->unit_id->LookupFilterQuery(FALSE, "list") ?>";
...
<?php if ($tr_sjd_item->Export == "") { ?>
<div class="ewToolbar">
<?php if ($tr_sjd_item_list->TotalRecs > 0 && $tr_sjd_item_list->ExportOptions->Visible()) { ?>
<?php $tr_sjd_item_list->ExportOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php if (($tr_sjd_item->Export == "") || (EW_EXPORT_MASTER_RECORD && $tr_sjd_item->Export == "print")) { ?>
<?php
if ($tr_sjd_item_list->DbMasterFilter <> "" && $tr_sjd_item->getCurrentMasterTable() == "tr_sjd_master") {
	if ($tr_sjd_item_list->MasterRecordExists) {
?>
<?php include_once "tr_sjd_mastermaster.php" ?>
<?php
	}
}
?>
...
if ($tr_sjd_item->CurrentAction == "gridadd") {
	...
	$tr_sjd_item_list->StartRec = 1;
	$tr_sjd_item_list->DisplayRecs = $tr_sjd_item->GridAddRowCount;
	$tr_sjd_item_list->TotalRecs = $tr_sjd_item_list->DisplayRecs;
	$tr_sjd_item_list->StopRec = $tr_sjd_item_list->DisplayRecs;
} else {
	$bSelectLimit = $tr_sjd_item_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($tr_sjd_item_list->TotalRecs <= 0)
			$tr_sjd_item_list->TotalRecs = $tr_sjd_item->ListRecordCount();
	} else {
		if (!$tr_sjd_item_list->Recordset && ($tr_sjd_item_list->Recordset = $tr_sjd_item_list->LoadRecordset()))
			$tr_sjd_item_list->TotalRecs = $tr_sjd_item_list->Recordset->RecordCount();
	}
	$tr_sjd_item_list->StartRec = 1;
	if ($tr_sjd_item_list->DisplayRecs <= 0 || ($tr_sjd_item->Export <> "" && $tr_sjd_item->ExportAll)) // Display all records
		$tr_sjd_item_list->DisplayRecs = $tr_sjd_item_list->TotalRecs;
	if (!($tr_sjd_item->Export <> "" && $tr_sjd_item->ExportAll))
		$tr_sjd_item_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$tr_sjd_item_list->Recordset = $tr_sjd_item_list->LoadRecordset($tr_sjd_item_list->StartRec-1, $tr_sjd_item_list->DisplayRecs);

	// Set no record found message
	if ($tr_sjd_item->CurrentAction == "" && $tr_sjd_item_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$tr_sjd_item_list->setWarningMessage(ew_DeniedMsg());
		if ($tr_sjd_item_list->SearchWhere == "0=101")
			$tr_sjd_item_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$tr_sjd_item_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
}
$tr_sjd_item_list->RenderOtherOptions();
?>
<?php $tr_sjd_item_list->ShowPageHeader(); ?>
<?php
$tr_sjd_item_list->ShowMessage();
?>
<?php if ($tr_sjd_item_list->TotalRecs > 0 || $tr_sjd_item->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($tr_sjd_item_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> tr_sjd_item">
<?php if ($tr_sjd_item->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($tr_sjd_item->CurrentAction <> "gridadd" && $tr_sjd_item->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($tr_sjd_item_list->Pager)) $tr_sjd_item_list->Pager = new cPrevNextPager($tr_sjd_item_list->StartRec, $tr_sjd_item_list->DisplayRecs, $tr_sjd_item_list->TotalRecs, $tr_sjd_item_list->AutoHidePager) ?>
<?php if ($tr_sjd_item_list->Pager->RecordCount > 0 && $tr_sjd_item_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($tr_sjd_item_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $tr_sjd_item_list->PageUrl() ?>start=<?php echo $tr_sjd_item_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($tr_sjd_item_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $tr_sjd_item_list->PageUrl() ?>start=<?php echo $tr_sjd_item_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $tr_sjd_item_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($tr_sjd_item_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $tr_sjd_item_list->PageUrl() ?>start=<?php echo $tr_sjd_item_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($tr_sjd_item_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $tr_sjd_item_list->PageUrl() ?>start=<?php echo $tr_sjd_item_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $tr_sjd_item_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($tr_sjd_item_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $tr_sjd_item_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $tr_sjd_item_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $tr_sjd_item_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($tr_sjd_item_list->TotalRecs > 0 && (!$tr_sjd_item_list->AutoHidePageSizeSelector || $tr_sjd_item_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="tr_sjd_item">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="10"<?php if ($tr_sjd_item_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($tr_sjd_item_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($tr_sjd_item_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($tr_sjd_item_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
<option value="ALL"<?php if ($tr_sjd_item->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($tr_sjd_item_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="ftr_sjd_itemlist" id="ftr_sjd_itemlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tr_sjd_item_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tr_sjd_item_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tr_sjd_item">
<?php if ($tr_sjd_item->getCurrentMasterTable() == "tr_sjd_master" && $tr_sjd_item->CurrentAction <> "") { ?>
...
<?php if ($tr_sjd_item_list->TotalRecs > 0 || $tr_sjd_item->CurrentAction == "gridedit") { ?>
<table id="tbl_tr_sjd_itemlist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$tr_sjd_item_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$tr_sjd_item_list->RenderListOptions();

// Render list options (header, left)
$tr_sjd_item_list->ListOptions->Render("header", "left");
?>
...
$tr_sjd_item_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($tr_sjd_item->ExportAll && $tr_sjd_item->Export <> "") {
	$tr_sjd_item_list->StopRec = $tr_sjd_item_list->TotalRecs;
} else {

	// Set the last record to display
	if ($tr_sjd_item_list->TotalRecs > $tr_sjd_item_list->StartRec + $tr_sjd_item_list->DisplayRecs - 1)
		$tr_sjd_item_list->StopRec = $tr_sjd_item_list->StartRec + $tr_sjd_item_list->DisplayRecs - 1;
	else
		$tr_sjd_item_list->StopRec = $tr_sjd_item_list->TotalRecs;
}
...
if ($objForm) {
	$objForm->Index = -1;
	if ($objForm->HasValue($tr_sjd_item_list->FormKeyCountName) && ($tr_sjd_item->CurrentAction == "gridadd" || $tr_sjd_item->CurrentAction == "gridedit" || $tr_sjd_item->CurrentAction == "F")) {
		$tr_sjd_item_list->KeyCount = $objForm->GetValue($tr_sjd_item_list->FormKeyCountName);
		$tr_sjd_item_list->StopRec = $tr_sjd_item_list->StartRec + $tr_sjd_item_list->KeyCount - 1;
	}
}
$tr_sjd_item_list->RecCnt = $tr_sjd_item_list->StartRec - 1;
if ($tr_sjd_item_list->Recordset && !$tr_sjd_item_list->Recordset->EOF) {
	$tr_sjd_item_list->Recordset->MoveFirst();
	$bSelectLimit = $tr_sjd_item_list->UseSelectLimit;
	if (!$bSelectLimit && $tr_sjd_item_list->StartRec > 1)
		$tr_sjd_item_list->Recordset->Move($tr_sjd_item_list->StartRec - 1);
} elseif (!$tr_sjd_item->AllowAddDeleteRow && $tr_sjd_item_list->StopRec == 0) {
	$tr_sjd_item_list->StopRec = $tr_sjd_item->GridAddRowCount;
}

// Initialize aggregate
$tr_sjd_item->RowType = EW_ROWTYPE_AGGREGATEINIT;
$tr_sjd_item->ResetAttrs();
$tr_sjd_item_list->RenderRow();
if ($tr_sjd_item->CurrentAction == "gridadd")
	$tr_sjd_item_list->RowIndex = 0;
if ($tr_sjd_item->CurrentAction == "gridedit")
	$tr_sjd_item_list->RowIndex = 0;
while ($tr_sjd_item_list->RecCnt < $tr_sjd_item_list->StopRec) {
	$tr_sjd_item_list->RecCnt++;
	if (intval($tr_sjd_item_list->RecCnt) >= intval($tr_sjd_item_list->StartRec)) {
		$tr_sjd_item_list->RowCnt++;
		if ($tr_sjd_item->CurrentAction == "gridadd" || $tr_sjd_item->CurrentAction == "gridedit" || $tr_sjd_item->CurrentAction == "F") {
			$tr_sjd_item_list->RowIndex++;
			$objForm->Index = $tr_sjd_item_list->RowIndex;
			if ($objForm->HasValue($tr_sjd_item_list->FormActionName))
				$tr_sjd_item_list->RowAction = strval($objForm->GetValue($tr_sjd_item_list->FormActionName));
			elseif ($tr_sjd_item->CurrentAction == "gridadd")
				$tr_sjd_item_list->RowAction = "insert";
			else
				$tr_sjd_item_list->RowAction = "";
		}

		// Set up key count
		$tr_sjd_item_list->KeyCount = $tr_sjd_item_list->RowIndex;

		// Init row class and style
		$tr_sjd_item->ResetAttrs();
		$tr_sjd_item->CssClass = "";
		if ($tr_sjd_item->CurrentAction == "gridadd") {
			$tr_sjd_item_list->LoadRowValues(); // Load default values
		} else {
			$tr_sjd_item_list->LoadRowValues($tr_sjd_item_list->Recordset); // Load row values
		}
		...
		if ($tr_sjd_item->CurrentAction == "gridadd" && $tr_sjd_item->EventCancelled && !$objForm->HasValue("k_blankrow")) // Insert failed
			$tr_sjd_item_list->RestoreCurrentRowFormValues($tr_sjd_item_list->RowIndex); // Restore form values
		if ($tr_sjd_item->CurrentAction == "gridedit") { // Grid edit
			if ($tr_sjd_item->EventCancelled) {
				$tr_sjd_item_list->RestoreCurrentRowFormValues($tr_sjd_item_list->RowIndex); // Restore form values
			}
			if ($tr_sjd_item_list->RowAction == "insert")
				$tr_sjd_item->RowType = EW_ROWTYPE_ADD; // Render add
			else
		...
		if ($tr_sjd_item->RowType == EW_ROWTYPE_EDIT) // Edit row
			$tr_sjd_item_list->EditRowCnt++;

		// Set up row id / data-rowindex
		$tr_sjd_item->RowAttrs = array_merge($tr_sjd_item->RowAttrs, array('data-rowindex'=>$tr_sjd_item_list->RowCnt, 'id'=>'r' . $tr_sjd_item_list->RowCnt . '_tr_sjd_item', 'data-rowtype'=>$tr_sjd_item->RowType));
		...
		if ($tr_sjd_item_list->RowAction <> "delete" && $tr_sjd_item_list->RowAction <> "insertdelete" && !($tr_sjd_item_list->RowAction == "insert" && $tr_sjd_item->CurrentAction == "F" && $tr_sjd_item_list->EmptyRow())) {
?>
	<tr<?php echo $tr_sjd_item->RowAttributes() ?>>
<?php

// Render list options (body, left)
$tr_sjd_item_list->ListOptions->Render("body", "left", $tr_sjd_item_list->RowCnt);
?>
	<?php if ($tr_sjd_item->item_id->Visible) { // item_id ?>
		<td data-name="item_id"<?php echo $tr_sjd_item->item_id->CellAttributes() ?>>
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_item_id" class="form-group tr_sjd_item_item_id">
<?php
...
?>
<span id="as_x<?php echo $tr_sjd_item_list->RowIndex ?>_item_id" style="white-space: nowrap; z-index: <?php echo (9000 - $tr_sjd_item_list->RowCnt * 10) ?>">
	<input type="text" name="sv_x<?php echo $tr_sjd_item_list->RowIndex ?>_item_id" id="sv_x<?php echo $tr_sjd_item_list->RowIndex ?>_item_id" value="<?php echo $tr_sjd_item->item_id->EditValue ?>" size="40" placeholder="<?php echo ew_HtmlEncode($tr_sjd_item->item_id->getPlaceHolder()) ?>" data-placeholder="<?php echo ew_HtmlEncode($tr_sjd_item->item_id->getPlaceHolder()) ?>"<?php echo $tr_sjd_item->item_id->EditAttributes() ?>>
</span>
<input type="hidden" data-table="tr_sjd_item" data-field="x_item_id" data-value-separator="<?php echo $tr_sjd_item->item_id->DisplayValueSeparatorAttribute() ?>" name="x<?php echo $tr_sjd_item_list->RowIndex ?>_item_id" id="x<?php echo $tr_sjd_item_list->RowIndex ?>_item_id" value="<?php echo ew_HtmlEncode($tr_sjd_item->item_id->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<script type="text/javascript">
ftr_sjd_itemlist.CreateAutoSuggest({"id":"x<?php echo $tr_sjd_item_list->RowIndex ?>_item_id","forceSelect":true});
</script>
<input type="hidden" name="ln_x<?php echo $tr_sjd_item_list->RowIndex ?>_item_id" id="ln_x<?php echo $tr_sjd_item_list->RowIndex ?>_item_id" value="x<?php echo $tr_sjd_item_list->RowIndex ?>_item_code,x<?php echo $tr_sjd_item_list->RowIndex ?>_item_price,x<?php echo $tr_sjd_item_list->RowIndex ?>_unit_id">
</span>
<input type="hidden" data-table="tr_sjd_item" data-field="x_item_id" name="o<?php echo $tr_sjd_item_list->RowIndex ?>_item_id" id="o<?php echo $tr_sjd_item_list->RowIndex ?>_item_id" value="<?php echo ew_HtmlEncode($tr_sjd_item->item_id->OldValue) ?>">
<?php } ?>
...
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_item_id" class="tr_sjd_item_item_id">
...
<?php } ?>
	...
	<?php } ?>
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<input type="hidden" data-table="tr_sjd_item" data-field="x_row_id" name="x<?php echo $tr_sjd_item_list->RowIndex ?>_row_id" id="x<?php echo $tr_sjd_item_list->RowIndex ?>_row_id" value="<?php echo ew_HtmlEncode($tr_sjd_item->row_id->CurrentValue) ?>">
<input type="hidden" data-table="tr_sjd_item" data-field="x_row_id" name="o<?php echo $tr_sjd_item_list->RowIndex ?>_row_id" id="o<?php echo $tr_sjd_item_list->RowIndex ?>_row_id" value="<?php echo ew_HtmlEncode($tr_sjd_item->row_id->OldValue) ?>">
<?php } ?>
...
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_item_code" class="form-group tr_sjd_item_item_code">
<input type="text" data-table="tr_sjd_item" data-field="x_item_code" name="x<?php echo $tr_sjd_item_list->RowIndex ?>_item_code" id="x<?php echo $tr_sjd_item_list->RowIndex ?>_item_code" size="10" maxlength="10" placeholder="<?php echo ew_HtmlEncode($tr_sjd_item->item_code->getPlaceHolder()) ?>" value="<?php echo $tr_sjd_item->item_code->EditValue ?>"<?php echo $tr_sjd_item->item_code->EditAttributes() ?>>
</span>
<input type="hidden" data-table="tr_sjd_item" data-field="x_item_code" name="o<?php echo $tr_sjd_item_list->RowIndex ?>_item_code" id="o<?php echo $tr_sjd_item_list->RowIndex ?>_item_code" value="<?php echo ew_HtmlEncode($tr_sjd_item->item_code->OldValue) ?>">
<?php } ?>
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_item_code" class="form-group tr_sjd_item_item_code">
<input type="text" data-table="tr_sjd_item" data-field="x_item_code" name="x<?php echo $tr_sjd_item_list->RowIndex ?>_item_code" id="x<?php echo $tr_sjd_item_list->RowIndex ?>_item_code" size="10" maxlength="10" placeholder="<?php echo ew_HtmlEncode($tr_sjd_item->item_code->getPlaceHolder()) ?>" value="<?php echo $tr_sjd_item->item_code->EditValue ?>"<?php echo $tr_sjd_item->item_code->EditAttributes() ?>>
...
<?php } ?>
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_item_code" class="tr_sjd_item_item_code">
...
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_unit_id" class="form-group tr_sjd_item_unit_id">
<div class="ewDropdownList has-feedback">
	...
	<?php } ?>
	<div id="dsl_x<?php echo $tr_sjd_item_list->RowIndex ?>_unit_id" data-repeatcolumn="1" class="dropdown-menu">
		<div class="ewItems" style="position: relative; overflow-x: hidden;">
<?php echo $tr_sjd_item->unit_id->RadioButtonListHtml(TRUE, "x{$tr_sjd_item_list->RowIndex}_unit_id") ?>
		</div>
	</div>
	<div id="tp_x<?php echo $tr_sjd_item_list->RowIndex ?>_unit_id" class="ewTemplate"><input type="radio" data-table="tr_sjd_item" data-field="x_unit_id" data-value-separator="<?php echo $tr_sjd_item->unit_id->DisplayValueSeparatorAttribute() ?>" name="x<?php echo $tr_sjd_item_list->RowIndex ?>_unit_id" id="x<?php echo $tr_sjd_item_list->RowIndex ?>_unit_id" value="{value}"<?php echo $tr_sjd_item->unit_id->EditAttributes() ?>></div>
</div>
</span>
<input type="hidden" data-table="tr_sjd_item" data-field="x_unit_id" name="o<?php echo $tr_sjd_item_list->RowIndex ?>_unit_id" id="o<?php echo $tr_sjd_item_list->RowIndex ?>_unit_id" value="<?php echo ew_HtmlEncode($tr_sjd_item->unit_id->OldValue) ?>">
<?php } ?>
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_unit_id" class="form-group tr_sjd_item_unit_id">
<div class="ewDropdownList has-feedback">
...
<?php } ?>
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_unit_id" class="tr_sjd_item_unit_id">
...
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_item_price" class="form-group tr_sjd_item_item_price">
<input type="text" data-table="tr_sjd_item" data-field="x_item_price" name="x<?php echo $tr_sjd_item_list->RowIndex ?>_item_price" id="x<?php echo $tr_sjd_item_list->RowIndex ?>_item_price" size="15" placeholder="<?php echo ew_HtmlEncode($tr_sjd_item->item_price->getPlaceHolder()) ?>" value="<?php echo $tr_sjd_item->item_price->EditValue ?>"<?php echo $tr_sjd_item->item_price->EditAttributes() ?>>
</span>
<input type="hidden" data-table="tr_sjd_item" data-field="x_item_price" name="o<?php echo $tr_sjd_item_list->RowIndex ?>_item_price" id="o<?php echo $tr_sjd_item_list->RowIndex ?>_item_price" value="<?php echo ew_HtmlEncode($tr_sjd_item->item_price->OldValue) ?>">
<?php } ?>
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_item_price" class="form-group tr_sjd_item_item_price">
<input type="text" data-table="tr_sjd_item" data-field="x_item_price" name="x<?php echo $tr_sjd_item_list->RowIndex ?>_item_price" id="x<?php echo $tr_sjd_item_list->RowIndex ?>_item_price" size="15" placeholder="<?php echo ew_HtmlEncode($tr_sjd_item->item_price->getPlaceHolder()) ?>" value="<?php echo $tr_sjd_item->item_price->EditValue ?>"<?php echo $tr_sjd_item->item_price->EditAttributes() ?>>
...
<?php } ?>
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_item_price" class="tr_sjd_item_item_price">
...
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_item_qty" class="form-group tr_sjd_item_item_qty">
<input type="text" data-table="tr_sjd_item" data-field="x_item_qty" name="x<?php echo $tr_sjd_item_list->RowIndex ?>_item_qty" id="x<?php echo $tr_sjd_item_list->RowIndex ?>_item_qty" size="10" placeholder="<?php echo ew_HtmlEncode($tr_sjd_item->item_qty->getPlaceHolder()) ?>" value="<?php echo $tr_sjd_item->item_qty->EditValue ?>"<?php echo $tr_sjd_item->item_qty->EditAttributes() ?>>
</span>
<input type="hidden" data-table="tr_sjd_item" data-field="x_item_qty" name="o<?php echo $tr_sjd_item_list->RowIndex ?>_item_qty" id="o<?php echo $tr_sjd_item_list->RowIndex ?>_item_qty" value="<?php echo ew_HtmlEncode($tr_sjd_item->item_qty->OldValue) ?>">
<?php } ?>
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_item_qty" class="form-group tr_sjd_item_item_qty">
<input type="text" data-table="tr_sjd_item" data-field="x_item_qty" name="x<?php echo $tr_sjd_item_list->RowIndex ?>_item_qty" id="x<?php echo $tr_sjd_item_list->RowIndex ?>_item_qty" size="10" placeholder="<?php echo ew_HtmlEncode($tr_sjd_item->item_qty->getPlaceHolder()) ?>" value="<?php echo $tr_sjd_item->item_qty->EditValue ?>"<?php echo $tr_sjd_item->item_qty->EditAttributes() ?>>
...
<?php } ?>
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_item_qty" class="tr_sjd_item_item_qty">
...
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_qty_rcv" class="form-group tr_sjd_item_qty_rcv">
<input type="text" data-table="tr_sjd_item" data-field="x_qty_rcv" name="x<?php echo $tr_sjd_item_list->RowIndex ?>_qty_rcv" id="x<?php echo $tr_sjd_item_list->RowIndex ?>_qty_rcv" size="10" placeholder="<?php echo ew_HtmlEncode($tr_sjd_item->qty_rcv->getPlaceHolder()) ?>" value="<?php echo $tr_sjd_item->qty_rcv->EditValue ?>"<?php echo $tr_sjd_item->qty_rcv->EditAttributes() ?>>
</span>
<input type="hidden" data-table="tr_sjd_item" data-field="x_qty_rcv" name="o<?php echo $tr_sjd_item_list->RowIndex ?>_qty_rcv" id="o<?php echo $tr_sjd_item_list->RowIndex ?>_qty_rcv" value="<?php echo ew_HtmlEncode($tr_sjd_item->qty_rcv->OldValue) ?>">
<?php } ?>
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_qty_rcv" class="form-group tr_sjd_item_qty_rcv">
<input type="text" data-table="tr_sjd_item" data-field="x_qty_rcv" name="x<?php echo $tr_sjd_item_list->RowIndex ?>_qty_rcv" id="x<?php echo $tr_sjd_item_list->RowIndex ?>_qty_rcv" size="10" placeholder="<?php echo ew_HtmlEncode($tr_sjd_item->qty_rcv->getPlaceHolder()) ?>" value="<?php echo $tr_sjd_item->qty_rcv->EditValue ?>"<?php echo $tr_sjd_item->qty_rcv->EditAttributes() ?>>
...
<?php } ?>
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_qty_rcv" class="tr_sjd_item_qty_rcv">
...
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_cek_sjd" class="form-group tr_sjd_item_cek_sjd">
<div id="tp_x<?php echo $tr_sjd_item_list->RowIndex ?>_cek_sjd" class="ewTemplate"><input type="checkbox" data-table="tr_sjd_item" data-field="x_cek_sjd" data-value-separator="<?php echo $tr_sjd_item->cek_sjd->DisplayValueSeparatorAttribute() ?>" name="x<?php echo $tr_sjd_item_list->RowIndex ?>_cek_sjd[]" id="x<?php echo $tr_sjd_item_list->RowIndex ?>_cek_sjd[]" value="{value}"<?php echo $tr_sjd_item->cek_sjd->EditAttributes() ?>></div>
<div id="dsl_x<?php echo $tr_sjd_item_list->RowIndex ?>_cek_sjd" data-repeatcolumn="5" class="ewItemList" style="display: none;"><div>
<?php echo $tr_sjd_item->cek_sjd->CheckBoxListHtml(FALSE, "x{$tr_sjd_item_list->RowIndex}_cek_sjd[]") ?>
</div></div>
</span>
<input type="hidden" data-table="tr_sjd_item" data-field="x_cek_sjd" name="o<?php echo $tr_sjd_item_list->RowIndex ?>_cek_sjd[]" id="o<?php echo $tr_sjd_item_list->RowIndex ?>_cek_sjd[]" value="<?php echo ew_HtmlEncode($tr_sjd_item->cek_sjd->OldValue) ?>">
<?php } ?>
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_cek_sjd" class="form-group tr_sjd_item_cek_sjd">
<div id="tp_x<?php echo $tr_sjd_item_list->RowIndex ?>_cek_sjd" class="ewTemplate"><input type="checkbox" data-table="tr_sjd_item" data-field="x_cek_sjd" data-value-separator="<?php echo $tr_sjd_item->cek_sjd->DisplayValueSeparatorAttribute() ?>" name="x<?php echo $tr_sjd_item_list->RowIndex ?>_cek_sjd[]" id="x<?php echo $tr_sjd_item_list->RowIndex ?>_cek_sjd[]" value="{value}"<?php echo $tr_sjd_item->cek_sjd->EditAttributes() ?>></div>
...
<?php } ?>
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $tr_sjd_item_list->RowCnt ?>_tr_sjd_item_cek_sjd" class="tr_sjd_item_cek_sjd">
...
$tr_sjd_item_list->ListOptions->Render("body", "right", $tr_sjd_item_list->RowCnt);
?>
	</tr>
<?php if ($tr_sjd_item->RowType == EW_ROWTYPE_ADD || $tr_sjd_item->RowType == EW_ROWTYPE_EDIT) { ?>
<script type="text/javascript">
ftr_sjd_itemlist.UpdateOpts(<?php echo $tr_sjd_item_list->RowIndex ?>);
</script>
<?php } ?>
<?php
	}
	...
	if ($tr_sjd_item->CurrentAction <> "gridadd")
		if (!$tr_sjd_item_list->Recordset->EOF) $tr_sjd_item_list->Recordset->MoveNext();
}
?>
<?php
	if ($tr_sjd_item->CurrentAction == "gridadd" || $tr_sjd_item->CurrentAction == "gridedit") {
		$tr_sjd_item_list->RowIndex = '$rowindex$';
		$tr_sjd_item_list->LoadRowValues();

		// Set row properties
		$tr_sjd_item->ResetAttrs();
		$tr_sjd_item->RowAttrs = array_merge($tr_sjd_item->RowAttrs, array('data-rowindex'=>$tr_sjd_item_list->RowIndex, 'id'=>'r0_tr_sjd_item', 'data-rowtype'=>EW_ROWTYPE_ADD));
		...
		// Render list options
		$tr_sjd_item_list->RenderListOptions();
		$tr_sjd_item_list->StartRowCnt = 0;
?>
	<tr<?php echo $tr_sjd_item->RowAttributes() ?>>
<?php

// Render list options (body, left)
$tr_sjd_item_list->ListOptions->Render("body", "left", $tr_sjd_item_list->RowIndex);
?>
	<?php if ($tr_sjd_item->item_id->Visible) { // item_id ?>
...
// Render list options (body, right)
$tr_sjd_item_list->ListOptions->Render("body", "right", $tr_sjd_item_list->RowIndex);
?>
<script type="text/javascript">
ftr_sjd_itemlist.UpdateOpts(<?php echo $tr_sjd_item_list->RowIndex ?>);
</script>
...
<?php } ?>
<?php if ($tr_sjd_item->CurrentAction == "gridadd") { ?>
<input type="hidden" name="a_list" id="a_list" value="gridinsert">
<input type="hidden" name="<?php echo $tr_sjd_item_list->FormKeyCountName ?>" id="<?php echo $tr_sjd_item_list->FormKeyCountName ?>" value="<?php echo $tr_sjd_item_list->KeyCount ?>">
<?php echo $tr_sjd_item_list->MultiSelectKey ?>
<?php } ?>
<?php if ($tr_sjd_item->CurrentAction == "gridedit") { ?>
<input type="hidden" name="a_list" id="a_list" value="gridupdate">
<input type="hidden" name="<?php echo $tr_sjd_item_list->FormKeyCountName ?>" id="<?php echo $tr_sjd_item_list->FormKeyCountName ?>" value="<?php echo $tr_sjd_item_list->KeyCount ?>">
<?php echo $tr_sjd_item_list->MultiSelectKey ?>
<?php } ?>
<?php if ($tr_sjd_item->CurrentAction == "") { ?>
...
// Close recordset
if ($tr_sjd_item_list->Recordset)
	$tr_sjd_item_list->Recordset->Close();
?>
</div>
<?php } ?>
<?php if ($tr_sjd_item_list->TotalRecs == 0 && $tr_sjd_item->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($tr_sjd_item_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($tr_sjd_item->Export == "") { ?>
<script type="text/javascript">
ftr_sjd_itemlist.Init();
</script>
<?php } ?>
<?php
$tr_sjd_item_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($tr_sjd_item->Export == "") { ?>
...
<?php
$tr_sjd_item_list->Page_Terminate();
?>
