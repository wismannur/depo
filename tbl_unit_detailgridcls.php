...
$tbl_unit_detail_grid = NULL; // Initialize page object first

class ctbl_unit_detail_grid extends ctbl_unit_detail {
	...
	// Page object name
	var $PageObjName = 'tbl_unit_detail_grid';

	// Grid form hidden field names
	var $FormName = 'ftbl_unit_detailgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["tbl_unit_detail"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function ClearInlineMode() {
		$this->unit_cost->FormValue = ""; // Clear form value
		$this->unit_price->FormValue = ""; // Clear form value
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->udet_id->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_unit_name") && $objForm->HasValue("o_unit_name") && $this->unit_name->CurrentValue <> $this->unit_name->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_qty_in_unit") && $objForm->HasValue("o_qty_in_unit") && $this->qty_in_unit->CurrentValue <> $this->qty_in_unit->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_unit_cost") && $objForm->HasValue("o_unit_cost") && $this->unit_cost->CurrentValue <> $this->unit_cost->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_unit_price") && $objForm->HasValue("o_unit_price") && $this->unit_price->CurrentValue <> $this->unit_price->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->udet_id->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->qty_in_unit->CurrentValue = 1;
		$this->qty_in_unit->OldValue = $this->qty_in_unit->CurrentValue;
	...
	function LoadFormValues() {
		...
		$this->unit_name->setOldValue($objForm->GetValue("o_unit_name"));
		if (!$this->qty_in_unit->FldIsDetailKey) {
			$this->qty_in_unit->setFormValue($objForm->GetValue("x_qty_in_unit"));
		}
		$this->qty_in_unit->setOldValue($objForm->GetValue("o_qty_in_unit"));
		if (!$this->unit_cost->FldIsDetailKey) {
			$this->unit_cost->setFormValue($objForm->GetValue("x_unit_cost"));
		}
		$this->unit_cost->setOldValue($objForm->GetValue("o_unit_cost"));
		if (!$this->unit_price->FldIsDetailKey) {
			$this->unit_price->setFormValue($objForm->GetValue("x_unit_price"));
		}
		$this->unit_price->setOldValue($objForm->GetValue("o_unit_price"));
		...
		if (!$this->udet_id->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function RestoreFormValues() {
		...
		$this->qty_in_unit->CurrentValue = $this->qty_in_unit->FormValue;
	...
	function NewRow() {
		...
		$row['unit_name'] = $this->unit_name->CurrentValue;
		$row['qty_in_unit'] = $this->qty_in_unit->CurrentValue;
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->udet_id->CurrentValue = strval($arKeys[0]); // udet_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		$this->product_code->ViewCustomAttributes = "";
			...
			$this->qty_in_unit->EditValue = ew_HtmlEncode($this->qty_in_unit->CurrentValue);
			...
			$this->unit_cost->EditValue = ew_FormatNumber($this->unit_cost->EditValue, -2, -2, -2, -2);
			$this->unit_cost->OldValue = $this->unit_cost->EditValue;
			}
			...
			$this->unit_price->EditValue = ew_FormatNumber($this->unit_price->EditValue, -2, -2, -2, -2);
			$this->unit_price->OldValue = $this->unit_price->EditValue;
			}
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!ew_CheckInteger($this->qty_in_unit->FormValue)) {
			ew_AddMessage($gsFormError, $this->qty_in_unit->FldErrMsg());
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// unit_name
			$this->unit_name->SetDbValueDef($rsnew, $this->unit_name->CurrentValue, NULL, $this->unit_name->ReadOnly);

			// qty_in_unit
			$this->qty_in_unit->SetDbValueDef($rsnew, $this->qty_in_unit->CurrentValue, NULL, $this->qty_in_unit->ReadOnly);

			// unit_cost
			$this->unit_cost->SetDbValueDef($rsnew, $this->unit_cost->CurrentValue, NULL, $this->unit_cost->ReadOnly);
			...
			// Check referential integrity for master table 'tbl_products'
			$bValidMasterRecord = TRUE;
			$sMasterFilter = $this->SqlMasterFilter_tbl_products();
			...
			if ($bValidMasterRecord) {
				...
				$rsmaster = $GLOBALS["tbl_products"]->LoadRs($sMasterFilter);
				$bValidMasterRecord = ($rsmaster && !$rsmaster->EOF);
				$rsmaster->Close();
			}
			if (!$bValidMasterRecord) {
				$sRelatedRecordMsg = str_replace("%t", "tbl_products", $Language->Phrase("RelatedRecordRequired"));
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->product_id->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@product_id@", ew_AdjustSql($this->product_id->getSessionValue(), "db_inventory_pusat"), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
		...
		// unit_name
		$this->unit_name->SetDbValueDef($rsnew, $this->unit_name->CurrentValue, NULL, FALSE);

		// qty_in_unit
		$this->qty_in_unit->SetDbValueDef($rsnew, $this->qty_in_unit->CurrentValue, NULL, strval($this->qty_in_unit->CurrentValue) == "");
		...
		// product_id
		if ($this->product_id->getSessionValue() <> "") {
			$rsnew['product_id'] = $this->product_id->getSessionValue();
		}
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "tbl_products") {
			...
			if ($GLOBALS["tbl_products"]->EventCancelled) $this->EventCancelled = TRUE;
		}
