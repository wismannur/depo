...
//
// Table class for tr_km_item_ar2
//
class ctr_km_item_ar2 extends cTable {
	var $row_id;
	var $master_id;
	var $customer_id;
	var $inv_number;
	...
	function __construct() {
		...
		$this->TableVar = 'tr_km_item_ar2';
		$this->TableName = 'tr_km_item_ar2';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tr_km_item_ar2`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// row_id
		$this->row_id = new cField('tr_km_item_ar2', 'tr_km_item_ar2', 'x_row_id', 'row_id', '`row_id`', '`row_id`', 3, -1, FALSE, '`row_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// master_id
		$this->master_id = new cField('tr_km_item_ar2', 'tr_km_item_ar2', 'x_master_id', 'master_id', '`master_id`', '`master_id`', 3, -1, FALSE, '`master_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// customer_id
		$this->customer_id = new cField('tr_km_item_ar2', 'tr_km_item_ar2', 'x_customer_id', 'customer_id', '`customer_id`', '`customer_id`', 3, -1, FALSE, '`customer_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// inv_number
		$this->inv_number = new cField('tr_km_item_ar2', 'tr_km_item_ar2', 'x_inv_number', 'inv_number', '`inv_number`', '`inv_number`', 200, -1, FALSE, '`inv_number`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// inv_date
		$this->inv_date = new cField('tr_km_item_ar2', 'tr_km_item_ar2', 'x_inv_date', 'inv_date', '`inv_date`', ew_CastDateFieldForLike('`inv_date`', 0, "DB"), 133, 0, FALSE, '`inv_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// inv_amt
		$this->inv_amt = new cField('tr_km_item_ar2', 'tr_km_item_ar2', 'x_inv_amt', 'inv_amt', '`inv_amt`', '`inv_amt`', 5, -1, FALSE, '`inv_amt`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// paid_amt
		$this->paid_amt = new cField('tr_km_item_ar2', 'tr_km_item_ar2', 'x_paid_amt', 'paid_amt', '`paid_amt`', '`paid_amt`', 5, -1, FALSE, '`paid_amt`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// acc_no
		$this->acc_no = new cField('tr_km_item_ar2', 'tr_km_item_ar2', 'x_acc_no', 'acc_no', '`acc_no`', '`acc_no`', 200, -1, FALSE, '`acc_no`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "tr_km_master_ar2") {
	...
	// Master filter
	function SqlMasterFilter_tr_km_master_ar2() {
		return "`row_id`=@row_id@";
	}

	// Detail filter
	function SqlDetailFilter_tr_km_master_ar2() {
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tr_km_item_ar2`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "tr_km_item_ar2edit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "tr_km_item_ar2add.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tr_km_item_ar2view.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tr_km_item_ar2view.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "tr_km_item_ar2add.php?" . $this->UrlParm($parm);
		else
			$url = "tr_km_item_ar2add.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("tr_km_item_ar2edit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("tr_km_item_ar2add.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tr_km_item_ar2delete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "tr_km_master_ar2" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
	...
	function GetAutoFill($id, $val) {
		...
		if (preg_match('/^x(\d)*_inv_number$/', $id)) {
			$conn = &$this->Connection();
			$sSqlWrk = "SELECT `inv_date` AS FIELD0, `inv_total` AS FIELD1, `inv_total` AS FIELD2 FROM `tr_inv_canvas_master`";
