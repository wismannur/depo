...
$tr_sjc_item_add = NULL; // Initialize page object first

class ctr_sjc_item_add extends ctr_sjc_item {
	...
	// Page object name
	var $PageObjName = 'tr_sjc_item_add';
	...
	function Page_Init() {
		...
		if (!$Security->CanAdd()) {
			...
			if ($Security->CanList())
				$this->Page_Terminate(ew_GetUrl("tr_sjc_itemlist.php"));
			else
		...
		if ($url <> "") {
			...
			if ($this->IsModal) { // Show as modal
				...
				if ($pageName != $this->GetListUrl()) { // Not List page
					...
					if ($pageName == "tr_sjc_itemview.php")
						$row["view"] = "1";
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$loaded) { // Record not loaded
					...
					$this->Page_Terminate("tr_sjc_itemlist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "tr_sjc_itemlist.php")
						$sReturnUrl = $this->AddMasterUrl($sReturnUrl); // List page, return to List page with correct master key if necessary
					elseif (ew_GetPageName($sReturnUrl) == "tr_sjc_itemview.php")
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, $this->AddMasterUrl("tr_sjc_itemlist.php"), "", $this->TableVar, TRUE);
...
// Create page object
if (!isset($tr_sjc_item_add)) $tr_sjc_item_add = new ctr_sjc_item_add();

// Page init
$tr_sjc_item_add->Page_Init();

// Page main
$tr_sjc_item_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tr_sjc_item_add->Page_Render();
?>
...
var CurrentForm = ftr_sjc_itemadd = new ew_Form("ftr_sjc_itemadd", "add");

// Validate form
ftr_sjc_itemadd.Validate = function() {
...
ftr_sjc_itemadd.Form_CustomValidate = 
...
ftr_sjc_itemadd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
ftr_sjc_itemadd.Lists["x_item_id"] = {"LinkField":"x_product_id","Ajax":true,"AutoFill":true,"DisplayFields":["x_product_name","","",""],"ParentFields":[],"ChildFields":["x_udet_id"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"view_products_unit_price"};
ftr_sjc_itemadd.Lists["x_item_id"].Data = "<?php echo $tr_sjc_item_add->item_id->LookupFilterQuery(FALSE, "add") ?>";
ftr_sjc_itemadd.AutoSuggests["x_item_id"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $tr_sjc_item_add->item_id->LookupFilterQuery(TRUE, "add"))) ?>;
ftr_sjc_itemadd.Lists["x_udet_id"] = {"LinkField":"x_udet_id","Ajax":true,"AutoFill":true,"DisplayFields":["x_unit_name","","",""],"ParentFields":["x_item_id"],"ChildFields":[],"FilterFields":["x_product_id"],"Options":[],"Template":"","LinkTable":"tbl_unit_detail"};
ftr_sjc_itemadd.Lists["x_udet_id"].Data = "<?php echo $tr_sjc_item_add->udet_id->LookupFilterQuery(FALSE, "add") ?>";
...
<?php $tr_sjc_item_add->ShowPageHeader(); ?>
<?php
$tr_sjc_item_add->ShowMessage();
?>
<form name="ftr_sjc_itemadd" id="ftr_sjc_itemadd" class="<?php echo $tr_sjc_item_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tr_sjc_item_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tr_sjc_item_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tr_sjc_item">
<input type="hidden" name="a_add" id="a_add" value="A">
<input type="hidden" name="modal" value="<?php echo intval($tr_sjc_item_add->IsModal) ?>">
<?php if ($tr_sjc_item->getCurrentMasterTable() == "tr_sjc_master") { ?>
...
<?php } ?>
<?php if (!$tr_sjc_item_add->IsMobileOrModal) { ?>
<div class="ewDesktop"><!-- desktop -->
<?php } ?>
<?php if ($tr_sjc_item_add->IsMobileOrModal) { ?>
<div class="ewAddDiv"><!-- page* -->
<?php } else { ?>
<table id="tbl_tr_sjc_itemadd" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable"><!-- table* -->
<?php } ?>
<?php if ($tr_sjc_item->item_id->Visible) { // item_id ?>
<?php if ($tr_sjc_item_add->IsMobileOrModal) { ?>
	<div id="r_item_id" class="form-group">
		<label id="elh_tr_sjc_item_item_id" class="<?php echo $tr_sjc_item_add->LeftColumnClass ?>"><?php echo $tr_sjc_item->item_id->FldCaption() ?></label>
		<div class="<?php echo $tr_sjc_item_add->RightColumnClass ?>"><div<?php echo $tr_sjc_item->item_id->CellAttributes() ?>>
<span id="el_tr_sjc_item_item_id">
<?php
...
<span id="as_x_item_id" style="white-space: nowrap; z-index: 8970">
	<input type="text" name="sv_x_item_id" id="sv_x_item_id" value="<?php echo $tr_sjc_item->item_id->EditValue ?>" size="50" placeholder="<?php echo ew_HtmlEncode($tr_sjc_item->item_id->getPlaceHolder()) ?>" data-placeholder="<?php echo ew_HtmlEncode($tr_sjc_item->item_id->getPlaceHolder()) ?>"<?php echo $tr_sjc_item->item_id->EditAttributes() ?>>
</span>
<input type="hidden" data-table="tr_sjc_item" data-field="x_item_id" data-value-separator="<?php echo $tr_sjc_item->item_id->DisplayValueSeparatorAttribute() ?>" name="x_item_id" id="x_item_id" value="<?php echo ew_HtmlEncode($tr_sjc_item->item_id->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<script type="text/javascript">
ftr_sjc_itemadd.CreateAutoSuggest({"id":"x_item_id","forceSelect":true});
</script>
<input type="hidden" name="ln_x_item_id" id="ln_x_item_id" value="x_item_code,x_item_name,x_udet_id,x_item_price,x_unit_id">
</span>
<?php echo $tr_sjc_item->item_id->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_item_id">
		<td class="col-sm-2"><span id="elh_tr_sjc_item_item_id"><?php echo $tr_sjc_item->item_id->FldCaption() ?></span></td>
...
<?php echo $tr_sjc_item->item_id->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php } ?>
...
<?php if ($tr_sjc_item_add->IsMobileOrModal) { ?>
	<div id="r_item_code" class="form-group">
		<label id="elh_tr_sjc_item_item_code" for="x_item_code" class="<?php echo $tr_sjc_item_add->LeftColumnClass ?>"><?php echo $tr_sjc_item->item_code->FldCaption() ?></label>
		<div class="<?php echo $tr_sjc_item_add->RightColumnClass ?>"><div<?php echo $tr_sjc_item->item_code->CellAttributes() ?>>
<span id="el_tr_sjc_item_item_code">
<input type="text" data-table="tr_sjc_item" data-field="x_item_code" name="x_item_code" id="x_item_code" size="30" maxlength="10" placeholder="<?php echo ew_HtmlEncode($tr_sjc_item->item_code->getPlaceHolder()) ?>" value="<?php echo $tr_sjc_item->item_code->EditValue ?>"<?php echo $tr_sjc_item->item_code->EditAttributes() ?>>
</span>
<?php echo $tr_sjc_item->item_code->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_item_code">
		<td class="col-sm-2"><span id="elh_tr_sjc_item_item_code"><?php echo $tr_sjc_item->item_code->FldCaption() ?></span></td>
		<td<?php echo $tr_sjc_item->item_code->CellAttributes() ?>>
<span id="el_tr_sjc_item_item_code">
<input type="text" data-table="tr_sjc_item" data-field="x_item_code" name="x_item_code" id="x_item_code" size="30" maxlength="10" placeholder="<?php echo ew_HtmlEncode($tr_sjc_item->item_code->getPlaceHolder()) ?>" value="<?php echo $tr_sjc_item->item_code->EditValue ?>"<?php echo $tr_sjc_item->item_code->EditAttributes() ?>>
</span>
<?php echo $tr_sjc_item->item_code->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php } ?>
...
<?php if ($tr_sjc_item_add->IsMobileOrModal) { ?>
	<div id="r_item_name" class="form-group">
		<label id="elh_tr_sjc_item_item_name" class="<?php echo $tr_sjc_item_add->LeftColumnClass ?>"><?php echo $tr_sjc_item->item_name->FldCaption() ?></label>
		<div class="<?php echo $tr_sjc_item_add->RightColumnClass ?>"><div<?php echo $tr_sjc_item->item_name->CellAttributes() ?>>
<span id="el_tr_sjc_item_item_name">
<input type="text" data-table="tr_sjc_item" data-field="x_item_name" name="x_item_name" id="x_item_name" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($tr_sjc_item->item_name->getPlaceHolder()) ?>" value="<?php echo $tr_sjc_item->item_name->EditValue ?>"<?php echo $tr_sjc_item->item_name->EditAttributes() ?>>
</span>
<?php echo $tr_sjc_item->item_name->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_item_name">
		<td class="col-sm-2"><span id="elh_tr_sjc_item_item_name"><?php echo $tr_sjc_item->item_name->FldCaption() ?></span></td>
		<td<?php echo $tr_sjc_item->item_name->CellAttributes() ?>>
<span id="el_tr_sjc_item_item_name">
<input type="text" data-table="tr_sjc_item" data-field="x_item_name" name="x_item_name" id="x_item_name" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($tr_sjc_item->item_name->getPlaceHolder()) ?>" value="<?php echo $tr_sjc_item->item_name->EditValue ?>"<?php echo $tr_sjc_item->item_name->EditAttributes() ?>>
</span>
<?php echo $tr_sjc_item->item_name->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php } ?>
...
<?php if ($tr_sjc_item_add->IsMobileOrModal) { ?>
	<div id="r_udet_id" class="form-group">
		<label id="elh_tr_sjc_item_udet_id" for="x_udet_id" class="<?php echo $tr_sjc_item_add->LeftColumnClass ?>"><?php echo $tr_sjc_item->udet_id->FldCaption() ?></label>
		<div class="<?php echo $tr_sjc_item_add->RightColumnClass ?>"><div<?php echo $tr_sjc_item->udet_id->CellAttributes() ?>>
<span id="el_tr_sjc_item_udet_id">
...
<?php echo $tr_sjc_item->udet_id->RadioButtonListHtml(TRUE, "x_udet_id") ?>
		</div>
	</div>
	<div id="tp_x_udet_id" class="ewTemplate"><input type="radio" data-table="tr_sjc_item" data-field="x_udet_id" data-value-separator="<?php echo $tr_sjc_item->udet_id->DisplayValueSeparatorAttribute() ?>" name="x_udet_id" id="x_udet_id" value="{value}"<?php echo $tr_sjc_item->udet_id->EditAttributes() ?>></div>
</div>
<input type="hidden" name="ln_x_udet_id" id="ln_x_udet_id" value="x_item_price,x_unit_id">
</span>
<?php echo $tr_sjc_item->udet_id->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_udet_id">
		<td class="col-sm-2"><span id="elh_tr_sjc_item_udet_id"><?php echo $tr_sjc_item->udet_id->FldCaption() ?></span></td>
...
<?php echo $tr_sjc_item->udet_id->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php } ?>
...
<?php if ($tr_sjc_item_add->IsMobileOrModal) { ?>
	<div id="r_item_qty" class="form-group">
		<label id="elh_tr_sjc_item_item_qty" for="x_item_qty" class="<?php echo $tr_sjc_item_add->LeftColumnClass ?>"><?php echo $tr_sjc_item->item_qty->FldCaption() ?></label>
		<div class="<?php echo $tr_sjc_item_add->RightColumnClass ?>"><div<?php echo $tr_sjc_item->item_qty->CellAttributes() ?>>
<span id="el_tr_sjc_item_item_qty">
<input type="text" data-table="tr_sjc_item" data-field="x_item_qty" name="x_item_qty" id="x_item_qty" size="30" placeholder="<?php echo ew_HtmlEncode($tr_sjc_item->item_qty->getPlaceHolder()) ?>" value="<?php echo $tr_sjc_item->item_qty->EditValue ?>"<?php echo $tr_sjc_item->item_qty->EditAttributes() ?>>
</span>
<?php echo $tr_sjc_item->item_qty->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_item_qty">
		<td class="col-sm-2"><span id="elh_tr_sjc_item_item_qty"><?php echo $tr_sjc_item->item_qty->FldCaption() ?></span></td>
		<td<?php echo $tr_sjc_item->item_qty->CellAttributes() ?>>
<span id="el_tr_sjc_item_item_qty">
<input type="text" data-table="tr_sjc_item" data-field="x_item_qty" name="x_item_qty" id="x_item_qty" size="30" placeholder="<?php echo ew_HtmlEncode($tr_sjc_item->item_qty->getPlaceHolder()) ?>" value="<?php echo $tr_sjc_item->item_qty->EditValue ?>"<?php echo $tr_sjc_item->item_qty->EditAttributes() ?>>
</span>
<?php echo $tr_sjc_item->item_qty->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php } ?>
...
<?php if ($tr_sjc_item_add->IsMobileOrModal) { ?>
	<div id="r_item_price" class="form-group">
		<label id="elh_tr_sjc_item_item_price" for="x_item_price" class="<?php echo $tr_sjc_item_add->LeftColumnClass ?>"><?php echo $tr_sjc_item->item_price->FldCaption() ?></label>
		<div class="<?php echo $tr_sjc_item_add->RightColumnClass ?>"><div<?php echo $tr_sjc_item->item_price->CellAttributes() ?>>
<span id="el_tr_sjc_item_item_price">
<input type="text" data-table="tr_sjc_item" data-field="x_item_price" name="x_item_price" id="x_item_price" size="30" placeholder="<?php echo ew_HtmlEncode($tr_sjc_item->item_price->getPlaceHolder()) ?>" value="<?php echo $tr_sjc_item->item_price->EditValue ?>"<?php echo $tr_sjc_item->item_price->EditAttributes() ?>>
</span>
<?php echo $tr_sjc_item->item_price->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_item_price">
		<td class="col-sm-2"><span id="elh_tr_sjc_item_item_price"><?php echo $tr_sjc_item->item_price->FldCaption() ?></span></td>
		<td<?php echo $tr_sjc_item->item_price->CellAttributes() ?>>
<span id="el_tr_sjc_item_item_price">
<input type="text" data-table="tr_sjc_item" data-field="x_item_price" name="x_item_price" id="x_item_price" size="30" placeholder="<?php echo ew_HtmlEncode($tr_sjc_item->item_price->getPlaceHolder()) ?>" value="<?php echo $tr_sjc_item->item_price->EditValue ?>"<?php echo $tr_sjc_item->item_price->EditAttributes() ?>>
</span>
<?php echo $tr_sjc_item->item_price->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php } ?>
...
<?php if ($tr_sjc_item_add->IsMobileOrModal) { ?>
	<div id="r_unit_id" class="form-group">
		<label id="elh_tr_sjc_item_unit_id" class="<?php echo $tr_sjc_item_add->LeftColumnClass ?>"><?php echo $tr_sjc_item->unit_id->FldCaption() ?></label>
		<div class="<?php echo $tr_sjc_item_add->RightColumnClass ?>"><div<?php echo $tr_sjc_item->unit_id->CellAttributes() ?>>
<span id="el_tr_sjc_item_unit_id">
<input type="text" data-table="tr_sjc_item" data-field="x_unit_id" name="x_unit_id" id="x_unit_id" size="30" placeholder="<?php echo ew_HtmlEncode($tr_sjc_item->unit_id->getPlaceHolder()) ?>" value="<?php echo $tr_sjc_item->unit_id->EditValue ?>"<?php echo $tr_sjc_item->unit_id->EditAttributes() ?>>
</span>
<?php echo $tr_sjc_item->unit_id->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_unit_id">
		<td class="col-sm-2"><span id="elh_tr_sjc_item_unit_id"><?php echo $tr_sjc_item->unit_id->FldCaption() ?></span></td>
		<td<?php echo $tr_sjc_item->unit_id->CellAttributes() ?>>
<span id="el_tr_sjc_item_unit_id">
<input type="text" data-table="tr_sjc_item" data-field="x_unit_id" name="x_unit_id" id="x_unit_id" size="30" placeholder="<?php echo ew_HtmlEncode($tr_sjc_item->unit_id->getPlaceHolder()) ?>" value="<?php echo $tr_sjc_item->unit_id->EditValue ?>"<?php echo $tr_sjc_item->unit_id->EditAttributes() ?>>
</span>
<?php echo $tr_sjc_item->unit_id->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php } ?>
<?php if ($tr_sjc_item_add->IsMobileOrModal) { ?>
...
<?php } ?>
<?php if (strval($tr_sjc_item->master_id->getSessionValue()) <> "") { ?>
<input type="hidden" name="x_master_id" id="x_master_id" value="<?php echo ew_HtmlEncode(strval($tr_sjc_item->master_id->getSessionValue())) ?>">
<?php } ?>
<?php if (!$tr_sjc_item_add->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $tr_sjc_item_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tr_sjc_item_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
...
<script type="text/javascript">
ftr_sjc_itemadd.Init();
</script>
<?php
$tr_sjc_item_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tr_sjc_item_add->Page_Terminate();
?>
