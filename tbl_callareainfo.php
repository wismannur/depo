...
//
// Table class for tbl_callarea
//
class ctbl_callarea extends cTable {
	var $area_id;
	var $area_name;
	var $subwil_id;
	var $wilayah_id;
	...
	function __construct() {
		...
		$this->TableVar = 'tbl_callarea';
		$this->TableName = 'tbl_callarea';
		$this->TableType = 'LINKTABLE';

		// Update Table
		$this->UpdateTable = "`tbl_callarea`";
		...
		// area_id
		$this->area_id = new cField('tbl_callarea', 'tbl_callarea', 'x_area_id', 'area_id', '`area_id`', '`area_id`', 3, -1, FALSE, '`area_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// area_name
		$this->area_name = new cField('tbl_callarea', 'tbl_callarea', 'x_area_name', 'area_name', '`area_name`', '`area_name`', 200, -1, FALSE, '`area_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->area_name->Sortable = TRUE; // Allow sort
		$this->fields['area_name'] = &$this->area_name;

		// subwil_id
		$this->subwil_id = new cField('tbl_callarea', 'tbl_callarea', 'x_subwil_id', 'subwil_id', '`subwil_id`', '`subwil_id`', 3, -1, FALSE, '`subwil_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// wilayah_id
		$this->wilayah_id = new cField('tbl_callarea', 'tbl_callarea', 'x_wilayah_id', 'wilayah_id', '`wilayah_id`', '`wilayah_id`', 3, -1, FALSE, '`wilayah_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tbl_callarea`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->area_id->setDbValue($conn->Insert_ID());
			$rs['area_id'] = $this->area_id->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('area_id', $rs))
				ew_AddFilter($where, ew_QuotedName('area_id', $this->DBID) . '=' . ew_QuotedValue($rs['area_id'], $this->area_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->area_id->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->area_id->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@area_id@", ew_AdjustSql($this->area_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "tbl_callareaedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "tbl_callareaadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tbl_callareaview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tbl_callareaview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "tbl_callareaadd.php?" . $this->UrlParm($parm);
		else
			$url = "tbl_callareaadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("tbl_callareaedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("tbl_callareaadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tbl_callareadelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "area_id:" . ew_VarToJson($this->area_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->area_id->CurrentValue)) {
			$sUrl .= "area_id=" . urlencode($this->area_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["area_id"]))
				$arKeys[] = $_POST["area_id"];
	...
	function RenderEditRow() {
		...
		$this->subwil_id->EditValue = $this->subwil_id->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->area_name->Exportable) $Doc->ExportCaption($this->area_name);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->area_name->Exportable) $Doc->ExportField($this->area_name);
