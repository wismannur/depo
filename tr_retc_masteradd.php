...
$tr_retc_master_add = NULL; // Initialize page object first

class ctr_retc_master_add extends ctr_retc_master {
	...
	// Page object name
	var $PageObjName = 'tr_retc_master_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["retc_id"] != "") {
				...
				$this->setKey("retc_id", $this->retc_id->CurrentValue); // Set up key
			} else {
				$this->setKey("retc_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "tr_retc_masterview.php")
	...
	function LoadDefaultValues() {
		...
		$this->retc_id->OldValue = $this->retc_id->CurrentValue;
		$this->retc_number->CurrentValue = NULL;
		$this->retc_number->OldValue = $this->retc_number->CurrentValue;
		$this->retc_date->CurrentValue = ew_CurrentDate();
		...
		$this->retc_notes->CurrentValue = @$_SESSION["KodeDepo"];
	...
	function NewRow() {
		...
		$row = array();
		$row['retc_id'] = $this->retc_id->CurrentValue;
		$row['retc_number'] = $this->retc_number->CurrentValue;
		$row['retc_date'] = $this->retc_date->CurrentValue;
		$row['sjc_number'] = $this->sjc_number->CurrentValue;
		$row['kode_depo'] = $this->kode_depo->CurrentValue;
		$row['retc_notes'] = $this->retc_notes->CurrentValue;
	...
	function ValidateForm() {
		...
		if (in_array("tr_retc_item", $DetailTblVar) && $GLOBALS["tr_retc_item"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		// retc_number
		$this->retc_number->SetDbValueDef($rsnew, $this->retc_number->CurrentValue, NULL, FALSE);

		// retc_date
		$this->retc_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->retc_date->CurrentValue, 0), NULL, FALSE);
		...
		// retc_notes
		$this->retc_notes->SetDbValueDef($rsnew, $this->retc_notes->CurrentValue, NULL, FALSE);
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("tr_retc_item", $DetailTblVar) && $GLOBALS["tr_retc_item"]->DetailAdd) {
				$GLOBALS["tr_retc_item"]->master_id->setSessionValue($this->retc_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["tr_retc_item_grid"])) $GLOBALS["tr_retc_item_grid"] = new ctr_retc_item_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "tr_retc_item"); // Load user level of detail table
				$AddRow = $GLOBALS["tr_retc_item_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["tr_retc_item"]->master_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("tr_retc_item", $DetailTblVar)) {
				...
				if ($GLOBALS["tr_retc_item_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["tr_retc_item_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["tr_retc_item_grid"]->CurrentMode = "add";
					$GLOBALS["tr_retc_item_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($tr_retc_master_add)) $tr_retc_master_add = new ctr_retc_master_add();

// Page init
$tr_retc_master_add->Page_Init();

// Page main
$tr_retc_master_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tr_retc_master_add->Page_Render();
?>
...
var CurrentForm = ftr_retc_masteradd = new ew_Form("ftr_retc_masteradd", "add");

// Validate form
ftr_retc_masteradd.Validate = function() {
...
ftr_retc_masteradd.Form_CustomValidate = 
...
ftr_retc_masteradd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
ftr_retc_masteradd.Lists["x_sjc_number"] = {"LinkField":"x_sjc_number","Ajax":true,"AutoFill":false,"DisplayFields":["x_sjc_number","","",""],"ParentFields":["x_sales_id"],"ChildFields":[],"FilterFields":["x_sales_id"],"Options":[],"Template":"","LinkTable":"tr_sjc_master"};
ftr_retc_masteradd.Lists["x_sjc_number"].Data = "<?php echo $tr_retc_master_add->sjc_number->LookupFilterQuery(FALSE, "add") ?>";
ftr_retc_masteradd.Lists["x_sales_id"] = {"LinkField":"x_sales_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_sales_name","","",""],"ParentFields":[],"ChildFields":["x_sjc_number"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_salesman"};
ftr_retc_masteradd.Lists["x_sales_id"].Data = "<?php echo $tr_retc_master_add->sales_id->LookupFilterQuery(FALSE, "add") ?>";
...
<?php $tr_retc_master_add->ShowPageHeader(); ?>
<?php
$tr_retc_master_add->ShowMessage();
?>
<form name="ftr_retc_masteradd" id="ftr_retc_masteradd" class="<?php echo $tr_retc_master_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tr_retc_master_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tr_retc_master_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tr_retc_master">
<input type="hidden" name="a_add" id="a_add" value="A">
<input type="hidden" name="modal" value="<?php echo intval($tr_retc_master_add->IsModal) ?>">
<div id="tpd_tr_retc_masteradd" class="ewCustomTemplate"></div>
<script id="tpm_tr_retc_masteradd" type="text/html">
<div id="ct_tr_retc_master_add"><div class="col-sm-12 panel-custom" style="">
	<div class="col-sm-5">
...
<?php if (!$tr_retc_master_add->IsMobileOrModal) { ?>
<div class="ewDesktop"><!-- desktop -->
<?php } ?>
<?php if ($tr_retc_master_add->IsMobileOrModal) { ?>
<div class="ewAddDiv hidden"><!-- page* -->
<?php } else { ?>
<table id="tbl_tr_retc_masteradd" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable hidden"><!-- table* -->
<?php } ?>
<?php if ($tr_retc_master->retc_number->Visible) { // retc_number ?>
<?php if ($tr_retc_master_add->IsMobileOrModal) { ?>
	<div id="r_retc_number" class="form-group">
		<label id="elh_tr_retc_master_retc_number" for="x_retc_number" class="<?php echo $tr_retc_master_add->LeftColumnClass ?>"><script id="tpc_tr_retc_master_retc_number" class="tr_retc_masteradd" type="text/html"><span><?php echo $tr_retc_master->retc_number->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_retc_master_add->RightColumnClass ?>"><div<?php echo $tr_retc_master->retc_number->CellAttributes() ?>>
<script id="tpx_tr_retc_master_retc_number" class="tr_retc_masteradd" type="text/html">
...
<?php } else { ?>
	<tr id="r_retc_number">
		<td class="col-sm-2"><span id="elh_tr_retc_master_retc_number"><script id="tpc_tr_retc_master_retc_number" class="tr_retc_masteradd" type="text/html"><span><?php echo $tr_retc_master->retc_number->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_retc_master_add->IsMobileOrModal) { ?>
	<div id="r_retc_date" class="form-group">
		<label id="elh_tr_retc_master_retc_date" for="x_retc_date" class="<?php echo $tr_retc_master_add->LeftColumnClass ?>"><script id="tpc_tr_retc_master_retc_date" class="tr_retc_masteradd" type="text/html"><span><?php echo $tr_retc_master->retc_date->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_retc_master_add->RightColumnClass ?>"><div<?php echo $tr_retc_master->retc_date->CellAttributes() ?>>
<script id="tpx_tr_retc_master_retc_date" class="tr_retc_masteradd" type="text/html">
...
<script type="text/html" class="tr_retc_masteradd_js">
ew_CreateDateTimePicker("ftr_retc_masteradd", "x_retc_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php echo $tr_retc_master->retc_date->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_retc_date">
		<td class="col-sm-2"><span id="elh_tr_retc_master_retc_date"><script id="tpc_tr_retc_master_retc_date" class="tr_retc_masteradd" type="text/html"><span><?php echo $tr_retc_master->retc_date->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_retc_master_add->IsMobileOrModal) { ?>
	<div id="r_sjc_number" class="form-group">
		<label id="elh_tr_retc_master_sjc_number" for="x_sjc_number" class="<?php echo $tr_retc_master_add->LeftColumnClass ?>"><script id="tpc_tr_retc_master_sjc_number" class="tr_retc_masteradd" type="text/html"><span><?php echo $tr_retc_master->sjc_number->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_retc_master_add->RightColumnClass ?>"><div<?php echo $tr_retc_master->sjc_number->CellAttributes() ?>>
<script id="tpx_tr_retc_master_sjc_number" class="tr_retc_masteradd" type="text/html">
...
<?php } else { ?>
	<tr id="r_sjc_number">
		<td class="col-sm-2"><span id="elh_tr_retc_master_sjc_number"><script id="tpc_tr_retc_master_sjc_number" class="tr_retc_masteradd" type="text/html"><span><?php echo $tr_retc_master->sjc_number->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_retc_master_add->IsMobileOrModal) { ?>
	<div id="r_kode_depo" class="form-group">
		<label id="elh_tr_retc_master_kode_depo" for="x_kode_depo" class="<?php echo $tr_retc_master_add->LeftColumnClass ?>"><script id="tpc_tr_retc_master_kode_depo" class="tr_retc_masteradd" type="text/html"><span><?php echo $tr_retc_master->kode_depo->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_retc_master_add->RightColumnClass ?>"><div<?php echo $tr_retc_master->kode_depo->CellAttributes() ?>>
<script id="tpx_tr_retc_master_kode_depo" class="tr_retc_masteradd" type="text/html">
...
<?php } else { ?>
	<tr id="r_kode_depo">
		<td class="col-sm-2"><span id="elh_tr_retc_master_kode_depo"><script id="tpc_tr_retc_master_kode_depo" class="tr_retc_masteradd" type="text/html"><span><?php echo $tr_retc_master->kode_depo->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_retc_master_add->IsMobileOrModal) { ?>
	<div id="r_retc_notes" class="form-group">
		<label id="elh_tr_retc_master_retc_notes" for="x_retc_notes" class="<?php echo $tr_retc_master_add->LeftColumnClass ?>"><script id="tpc_tr_retc_master_retc_notes" class="tr_retc_masteradd" type="text/html"><span><?php echo $tr_retc_master->retc_notes->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_retc_master_add->RightColumnClass ?>"><div<?php echo $tr_retc_master->retc_notes->CellAttributes() ?>>
<script id="tpx_tr_retc_master_retc_notes" class="tr_retc_masteradd" type="text/html">
...
<?php } else { ?>
	<tr id="r_retc_notes">
		<td class="col-sm-2"><span id="elh_tr_retc_master_retc_notes"><script id="tpc_tr_retc_master_retc_notes" class="tr_retc_masteradd" type="text/html"><span><?php echo $tr_retc_master->retc_notes->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_retc_master_add->IsMobileOrModal) { ?>
	<div id="r_sales_id" class="form-group">
		<label id="elh_tr_retc_master_sales_id" for="x_sales_id" class="<?php echo $tr_retc_master_add->LeftColumnClass ?>"><script id="tpc_tr_retc_master_sales_id" class="tr_retc_masteradd" type="text/html"><span><?php echo $tr_retc_master->sales_id->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_retc_master_add->RightColumnClass ?>"><div<?php echo $tr_retc_master->sales_id->CellAttributes() ?>>
<script id="tpx_tr_retc_master_sales_id" class="tr_retc_masteradd" type="text/html">
...
<?php } else { ?>
	<tr id="r_sales_id">
		<td class="col-sm-2"><span id="elh_tr_retc_master_sales_id"><script id="tpc_tr_retc_master_sales_id" class="tr_retc_masteradd" type="text/html"><span><?php echo $tr_retc_master->sales_id->FldCaption() ?></span></script></span></td>
...
<?php } ?>
<?php
	if (in_array("tr_retc_item", explode(",", $tr_retc_master->getCurrentDetailTable())) && $tr_retc_item->DetailAdd) {
?>
...
<?php } ?>
<?php if (!$tr_retc_master_add->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $tr_retc_master_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tr_retc_master_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
...
ew_ApplyTemplate("tpd_tr_retc_masteradd", "tpm_tr_retc_masteradd", "tr_retc_masteradd", "<?php echo $tr_retc_master->CustomExport ?>", ewVar.templateData.rows[0]);
jQuery("script.tr_retc_masteradd_js").each(function(){ew_AddScript(this.text);});
</script>
<script type="text/javascript">
ftr_retc_masteradd.Init();
</script>
<?php
$tr_retc_master_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tr_retc_master_add->Page_Terminate();
?>
