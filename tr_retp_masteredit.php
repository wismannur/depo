...
$tr_retp_master_edit = NULL; // Initialize page object first

class ctr_retp_master_edit extends ctr_retp_master {
	...
	// Page object name
	var $PageObjName = 'tr_retp_master_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			...
			// Load key from Form
			if ($objForm->HasValue("x_retp_id")) {
				$this->retp_id->setFormValue($objForm->GetValue("x_retp_id"));
			}
		} else {
	...
	function LoadFormValues() {
		...
		if (!$this->retp_id->FldIsDetailKey)
			$this->retp_id->setFormValue($objForm->GetValue("x_retp_id"));
	}
	...
	function RestoreFormValues() {
		global $objForm;
		$this->retp_id->CurrentValue = $this->retp_id->FormValue;
	...
	function ValidateForm() {
		...
		if (in_array("tr_retp_item", $DetailTblVar) && $GLOBALS["tr_retp_item"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// retp_number
			$this->retp_number->SetDbValueDef($rsnew, $this->retp_number->CurrentValue, NULL, $this->retp_number->ReadOnly);

			// retp_date
			$this->retp_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->retp_date->CurrentValue, 0), NULL, $this->retp_date->ReadOnly);

			// kode_depo
			$this->kode_depo->SetDbValueDef($rsnew, $this->kode_depo->CurrentValue, NULL, $this->kode_depo->ReadOnly);

			// retp_notes
			$this->retp_notes->SetDbValueDef($rsnew, $this->retp_notes->CurrentValue, NULL, $this->retp_notes->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if (in_array("tr_retp_item", $DetailTblVar) && $GLOBALS["tr_retp_item"]->DetailEdit) {
						...
						$EditRow = $GLOBALS["tr_retp_item_grid"]->GridUpdate();
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("tr_retp_item", $DetailTblVar)) {
				...
				if ($GLOBALS["tr_retp_item_grid"]->DetailEdit) {
					$GLOBALS["tr_retp_item_grid"]->CurrentMode = "edit";
					$GLOBALS["tr_retp_item_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($tr_retp_master_edit)) $tr_retp_master_edit = new ctr_retp_master_edit();

// Page init
$tr_retp_master_edit->Page_Init();

// Page main
$tr_retp_master_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tr_retp_master_edit->Page_Render();
?>
...
var CurrentForm = ftr_retp_masteredit = new ew_Form("ftr_retp_masteredit", "edit");

// Validate form
ftr_retp_masteredit.Validate = function() {
...
ftr_retp_masteredit.Form_CustomValidate = 
...
ftr_retp_masteredit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $tr_retp_master_edit->ShowPageHeader(); ?>
<?php
$tr_retp_master_edit->ShowMessage();
?>
<form name="ftr_retp_masteredit" id="ftr_retp_masteredit" class="<?php echo $tr_retp_master_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tr_retp_master_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tr_retp_master_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tr_retp_master">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($tr_retp_master_edit->IsModal) ?>">
<div id="tpd_tr_retp_masteredit" class="ewCustomTemplate"></div>
<script id="tpm_tr_retp_masteredit" type="text/html">
<div id="ct_tr_retp_master_edit"><div class="col-sm-12 panel-custom" style="">
	<div class="col-sm-5">
...
<?php if (!$tr_retp_master_edit->IsMobileOrModal) { ?>
<div class="ewDesktop"><!-- desktop -->
<?php } ?>
<?php if ($tr_retp_master_edit->IsMobileOrModal) { ?>
<div class="ewEditDiv hidden"><!-- page* -->
<?php } else { ?>
<table id="tbl_tr_retp_masteredit" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable hidden"><!-- table* -->
<?php } ?>
<?php if ($tr_retp_master->retp_number->Visible) { // retp_number ?>
<?php if ($tr_retp_master_edit->IsMobileOrModal) { ?>
	<div id="r_retp_number" class="form-group">
		<label id="elh_tr_retp_master_retp_number" for="x_retp_number" class="<?php echo $tr_retp_master_edit->LeftColumnClass ?>"><script id="tpc_tr_retp_master_retp_number" class="tr_retp_masteredit" type="text/html"><span><?php echo $tr_retp_master->retp_number->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_retp_master_edit->RightColumnClass ?>"><div<?php echo $tr_retp_master->retp_number->CellAttributes() ?>>
<script id="tpx_tr_retp_master_retp_number" class="tr_retp_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_retp_number">
		<td class="col-sm-2"><span id="elh_tr_retp_master_retp_number"><script id="tpc_tr_retp_master_retp_number" class="tr_retp_masteredit" type="text/html"><span><?php echo $tr_retp_master->retp_number->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_retp_master_edit->IsMobileOrModal) { ?>
	<div id="r_retp_date" class="form-group">
		<label id="elh_tr_retp_master_retp_date" for="x_retp_date" class="<?php echo $tr_retp_master_edit->LeftColumnClass ?>"><script id="tpc_tr_retp_master_retp_date" class="tr_retp_masteredit" type="text/html"><span><?php echo $tr_retp_master->retp_date->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_retp_master_edit->RightColumnClass ?>"><div<?php echo $tr_retp_master->retp_date->CellAttributes() ?>>
<script id="tpx_tr_retp_master_retp_date" class="tr_retp_masteredit" type="text/html">
...
<script type="text/html" class="tr_retp_masteredit_js">
ew_CreateDateTimePicker("ftr_retp_masteredit", "x_retp_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php echo $tr_retp_master->retp_date->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_retp_date">
		<td class="col-sm-2"><span id="elh_tr_retp_master_retp_date"><script id="tpc_tr_retp_master_retp_date" class="tr_retp_masteredit" type="text/html"><span><?php echo $tr_retp_master->retp_date->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_retp_master_edit->IsMobileOrModal) { ?>
	<div id="r_kode_depo" class="form-group">
		<label id="elh_tr_retp_master_kode_depo" for="x_kode_depo" class="<?php echo $tr_retp_master_edit->LeftColumnClass ?>"><script id="tpc_tr_retp_master_kode_depo" class="tr_retp_masteredit" type="text/html"><span><?php echo $tr_retp_master->kode_depo->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_retp_master_edit->RightColumnClass ?>"><div<?php echo $tr_retp_master->kode_depo->CellAttributes() ?>>
<script id="tpx_tr_retp_master_kode_depo" class="tr_retp_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_kode_depo">
		<td class="col-sm-2"><span id="elh_tr_retp_master_kode_depo"><script id="tpc_tr_retp_master_kode_depo" class="tr_retp_masteredit" type="text/html"><span><?php echo $tr_retp_master->kode_depo->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_retp_master_edit->IsMobileOrModal) { ?>
	<div id="r_retp_notes" class="form-group">
		<label id="elh_tr_retp_master_retp_notes" for="x_retp_notes" class="<?php echo $tr_retp_master_edit->LeftColumnClass ?>"><script id="tpc_tr_retp_master_retp_notes" class="tr_retp_masteredit" type="text/html"><span><?php echo $tr_retp_master->retp_notes->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_retp_master_edit->RightColumnClass ?>"><div<?php echo $tr_retp_master->retp_notes->CellAttributes() ?>>
<script id="tpx_tr_retp_master_retp_notes" class="tr_retp_masteredit" type="text/html">
...
<?php } else { ?>
	<tr id="r_retp_notes">
		<td class="col-sm-2"><span id="elh_tr_retp_master_retp_notes"><script id="tpc_tr_retp_master_retp_notes" class="tr_retp_masteredit" type="text/html"><span><?php echo $tr_retp_master->retp_notes->FldCaption() ?></span></script></span></td>
...
<?php } ?>
<input type="hidden" data-table="tr_retp_master" data-field="x_retp_id" name="x_retp_id" id="x_retp_id" value="<?php echo ew_HtmlEncode($tr_retp_master->retp_id->CurrentValue) ?>">
<?php
	if (in_array("tr_retp_item", explode(",", $tr_retp_master->getCurrentDetailTable())) && $tr_retp_item->DetailEdit) {
?>
...
<?php } ?>
<?php if (!$tr_retp_master_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $tr_retp_master_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tr_retp_master_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
...
ew_ApplyTemplate("tpd_tr_retp_masteredit", "tpm_tr_retp_masteredit", "tr_retp_masteredit", "<?php echo $tr_retp_master->CustomExport ?>", ewVar.templateData.rows[0]);
jQuery("script.tr_retp_masteredit_js").each(function(){ew_AddScript(this.text);});
</script>
<script type="text/javascript">
ftr_retp_masteredit.Init();
</script>
<?php
$tr_retp_master_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tr_retp_master_edit->Page_Terminate();
?>
