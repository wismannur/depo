...
$tr_km_item_ar_grid = NULL; // Initialize page object first

class ctr_km_item_ar_grid extends ctr_km_item_ar {
	...
	// Page object name
	var $PageObjName = 'tr_km_item_ar_grid';

	// Grid form hidden field names
	var $FormName = 'ftr_km_item_argrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["tr_km_item_ar"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "tr_km_master_ar") {
			...
			if ($GLOBALS["tr_km_master_ar"]->EventCancelled) $this->EventCancelled = TRUE;
		}
	...
	function SetupLookupFilters($fld, $pageId = null) {
		...
		case "x_customer_id":
			...
			$lookuptblfilter = (isset($_GET["customer_id"]))? "`customer_id` = '".@$_GET["customer_id"]."' AND `kode_depo` = '".@$_SESSION["KodeDepo"]."'" : "`kode_depo` = '".@$_SESSION["KodeDepo"]."'";
			ew_AddFilter($sWhereWrk, $lookuptblfilter);
			$fld->LookupFilters += array("s" => $sSqlWrk, "d" => "", "f0" => '`customer_id` IN ({filter_value})', "t0" => "3", "fn0" => "", "f1" => '`customer_id` IN ({filter_value})', "t1" => "3", "fn1" => "");
			$sSqlWrk = "";
			$this->Lookup_Selecting($this->customer_id, $sWhereWrk); // Call Lookup Selecting
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
			if ($sSqlWrk <> "")
