...
$tr_km_master_ar2_delete = NULL; // Initialize page object first

class ctr_km_master_ar2_delete extends ctr_km_master_ar2 {
	...
	// Page object name
	var $PageObjName = 'tr_km_master_ar2_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("tr_km_master_ar2list.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in tr_km_master_ar2 class, tr_km_master_ar2info.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("tr_km_master_ar2list.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($tr_km_master_ar2_delete)) $tr_km_master_ar2_delete = new ctr_km_master_ar2_delete();

// Page init
$tr_km_master_ar2_delete->Page_Init();

// Page main
$tr_km_master_ar2_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tr_km_master_ar2_delete->Page_Render();
?>
...
var CurrentForm = ftr_km_master_ar2delete = new ew_Form("ftr_km_master_ar2delete", "delete");

// Form_CustomValidate event
ftr_km_master_ar2delete.Form_CustomValidate = 
...
ftr_km_master_ar2delete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
ftr_km_master_ar2delete.Lists["x_customer_id"] = {"LinkField":"x_customer_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_customer_name","","",""],"ParentFields":[],"ChildFields":["tr_km_item_ar2 x_customer_id"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_customer"};
ftr_km_master_ar2delete.Lists["x_customer_id"].Data = "<?php echo $tr_km_master_ar2_delete->customer_id->LookupFilterQuery(FALSE, "delete") ?>";
ftr_km_master_ar2delete.AutoSuggests["x_customer_id"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $tr_km_master_ar2_delete->customer_id->LookupFilterQuery(TRUE, "delete"))) ?>;
...
<?php $tr_km_master_ar2_delete->ShowPageHeader(); ?>
<?php
$tr_km_master_ar2_delete->ShowMessage();
?>
<form name="ftr_km_master_ar2delete" id="ftr_km_master_ar2delete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tr_km_master_ar2_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tr_km_master_ar2_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tr_km_master_ar2">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($tr_km_master_ar2_delete->RecKeys as $key) { ?>
...
<?php if ($tr_km_master_ar2->km_nomor->Visible) { // km_nomor ?>
		<th class="<?php echo $tr_km_master_ar2->km_nomor->HeaderCellClass() ?>"><span id="elh_tr_km_master_ar2_km_nomor" class="tr_km_master_ar2_km_nomor"><?php echo $tr_km_master_ar2->km_nomor->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_km_master_ar2->km_tanggal->Visible) { // km_tanggal ?>
		<th class="<?php echo $tr_km_master_ar2->km_tanggal->HeaderCellClass() ?>"><span id="elh_tr_km_master_ar2_km_tanggal" class="tr_km_master_ar2_km_tanggal"><?php echo $tr_km_master_ar2->km_tanggal->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_km_master_ar2->customer_id->Visible) { // customer_id ?>
		<th class="<?php echo $tr_km_master_ar2->customer_id->HeaderCellClass() ?>"><span id="elh_tr_km_master_ar2_customer_id" class="tr_km_master_ar2_customer_id"><?php echo $tr_km_master_ar2->customer_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_km_master_ar2->cek_no->Visible) { // cek_no ?>
		<th class="<?php echo $tr_km_master_ar2->cek_no->HeaderCellClass() ?>"><span id="elh_tr_km_master_ar2_cek_no" class="tr_km_master_ar2_cek_no"><?php echo $tr_km_master_ar2->cek_no->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_km_master_ar2->tgl_jt->Visible) { // tgl_jt ?>
		<th class="<?php echo $tr_km_master_ar2->tgl_jt->HeaderCellClass() ?>"><span id="elh_tr_km_master_ar2_tgl_jt" class="tr_km_master_ar2_tgl_jt"><?php echo $tr_km_master_ar2->tgl_jt->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_km_master_ar2->km_amt->Visible) { // km_amt ?>
		<th class="<?php echo $tr_km_master_ar2->km_amt->HeaderCellClass() ?>"><span id="elh_tr_km_master_ar2_km_amt" class="tr_km_master_ar2_km_amt"><?php echo $tr_km_master_ar2->km_amt->FldCaption() ?></span></th>
<?php } ?>
<?php if ($tr_km_master_ar2->km_notes->Visible) { // km_notes ?>
		<th class="<?php echo $tr_km_master_ar2->km_notes->HeaderCellClass() ?>"><span id="elh_tr_km_master_ar2_km_notes" class="tr_km_master_ar2_km_notes"><?php echo $tr_km_master_ar2->km_notes->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$tr_km_master_ar2_delete->RecCnt = 0;
$i = 0;
while (!$tr_km_master_ar2_delete->Recordset->EOF) {
	$tr_km_master_ar2_delete->RecCnt++;
	$tr_km_master_ar2_delete->RowCnt++;

	// Set row properties
	$tr_km_master_ar2->ResetAttrs();
	$tr_km_master_ar2->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$tr_km_master_ar2_delete->LoadRowValues($tr_km_master_ar2_delete->Recordset);

	// Render row
	$tr_km_master_ar2_delete->RenderRow();
?>
	<tr<?php echo $tr_km_master_ar2->RowAttributes() ?>>
<?php if ($tr_km_master_ar2->km_nomor->Visible) { // km_nomor ?>
		<td<?php echo $tr_km_master_ar2->km_nomor->CellAttributes() ?>>
<span id="el<?php echo $tr_km_master_ar2_delete->RowCnt ?>_tr_km_master_ar2_km_nomor" class="tr_km_master_ar2_km_nomor">
...
<span id="el<?php echo $tr_km_master_ar2_delete->RowCnt ?>_tr_km_master_ar2_km_tanggal" class="tr_km_master_ar2_km_tanggal">
...
<span id="el<?php echo $tr_km_master_ar2_delete->RowCnt ?>_tr_km_master_ar2_customer_id" class="tr_km_master_ar2_customer_id">
...
<span id="el<?php echo $tr_km_master_ar2_delete->RowCnt ?>_tr_km_master_ar2_cek_no" class="tr_km_master_ar2_cek_no">
...
<span id="el<?php echo $tr_km_master_ar2_delete->RowCnt ?>_tr_km_master_ar2_tgl_jt" class="tr_km_master_ar2_tgl_jt">
...
<span id="el<?php echo $tr_km_master_ar2_delete->RowCnt ?>_tr_km_master_ar2_km_amt" class="tr_km_master_ar2_km_amt">
...
<span id="el<?php echo $tr_km_master_ar2_delete->RowCnt ?>_tr_km_master_ar2_km_notes" class="tr_km_master_ar2_km_notes">
...
<?php
	$tr_km_master_ar2_delete->Recordset->MoveNext();
}
$tr_km_master_ar2_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tr_km_master_ar2_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ftr_km_master_ar2delete.Init();
</script>
<?php
$tr_km_master_ar2_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tr_km_master_ar2_delete->Page_Terminate();
?>
