...
$tr_km_master_ar2_add = NULL; // Initialize page object first

class ctr_km_master_ar2_add extends ctr_km_master_ar2 {
	...
	// Page object name
	var $PageObjName = 'tr_km_master_ar2_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "tr_km_master_ar2view.php")
	...
	function ValidateForm() {
		...
		if (in_array("tr_km_item_ar2", $DetailTblVar) && $GLOBALS["tr_km_item_ar2"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("tr_km_item_ar2", $DetailTblVar) && $GLOBALS["tr_km_item_ar2"]->DetailAdd) {
				$GLOBALS["tr_km_item_ar2"]->master_id->setSessionValue($this->row_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["tr_km_item_ar2_grid"])) $GLOBALS["tr_km_item_ar2_grid"] = new ctr_km_item_ar2_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "tr_km_item_ar2"); // Load user level of detail table
				$AddRow = $GLOBALS["tr_km_item_ar2_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["tr_km_item_ar2"]->master_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("tr_km_item_ar2", $DetailTblVar)) {
				...
				if ($GLOBALS["tr_km_item_ar2_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["tr_km_item_ar2_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["tr_km_item_ar2_grid"]->CurrentMode = "add";
					$GLOBALS["tr_km_item_ar2_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($tr_km_master_ar2_add)) $tr_km_master_ar2_add = new ctr_km_master_ar2_add();

// Page init
$tr_km_master_ar2_add->Page_Init();

// Page main
$tr_km_master_ar2_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tr_km_master_ar2_add->Page_Render();
?>
...
var CurrentForm = ftr_km_master_ar2add = new ew_Form("ftr_km_master_ar2add", "add");

// Validate form
ftr_km_master_ar2add.Validate = function() {
...
ftr_km_master_ar2add.Form_CustomValidate = 
...
ftr_km_master_ar2add.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
ftr_km_master_ar2add.Lists["x_customer_id"] = {"LinkField":"x_customer_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_customer_name","","",""],"ParentFields":[],"ChildFields":["tr_km_item_ar2 x_customer_id"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tbl_customer"};
ftr_km_master_ar2add.Lists["x_customer_id"].Data = "<?php echo $tr_km_master_ar2_add->customer_id->LookupFilterQuery(FALSE, "add") ?>";
ftr_km_master_ar2add.AutoSuggests["x_customer_id"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $tr_km_master_ar2_add->customer_id->LookupFilterQuery(TRUE, "add"))) ?>;
ftr_km_master_ar2add.Lists["x_km_type"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftr_km_master_ar2add.Lists["x_km_type"].Options = <?php echo json_encode($tr_km_master_ar2_add->km_type->Options()) ?>;
...
<?php $tr_km_master_ar2_add->ShowPageHeader(); ?>
<?php
$tr_km_master_ar2_add->ShowMessage();
?>
<form name="ftr_km_master_ar2add" id="ftr_km_master_ar2add" class="<?php echo $tr_km_master_ar2_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tr_km_master_ar2_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tr_km_master_ar2_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tr_km_master_ar2">
<input type="hidden" name="a_add" id="a_add" value="A">
<input type="hidden" name="modal" value="<?php echo intval($tr_km_master_ar2_add->IsModal) ?>">
<div id="tpd_tr_km_master_ar2add" class="ewCustomTemplate"></div>
<script id="tpm_tr_km_master_ar2add" type="text/html">
<div id="ct_tr_km_master_ar2_add"><div class="col-sm-12">
	<div class="row">
		<div class="col-sm-5">
...
<?php if (!$tr_km_master_ar2_add->IsMobileOrModal) { ?>
<div class="ewDesktop"><!-- desktop -->
<?php } ?>
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
<div class="ewAddDiv hidden"><!-- page* -->
<?php } else { ?>
<table id="tbl_tr_km_master_ar2add" class="table table-striped table-bordered table-hover table-condensed ewDesktopTable hidden"><!-- table* -->
<?php } ?>
<?php if ($tr_km_master_ar2->km_nomor->Visible) { // km_nomor ?>
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_km_nomor" class="form-group">
		<label id="elh_tr_km_master_ar2_km_nomor" for="x_km_nomor" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_km_nomor" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->km_nomor->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->km_nomor->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_km_nomor" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_km_nomor">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_km_nomor"><script id="tpc_tr_km_master_ar2_km_nomor" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->km_nomor->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_km_tanggal" class="form-group">
		<label id="elh_tr_km_master_ar2_km_tanggal" for="x_km_tanggal" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_km_tanggal" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->km_tanggal->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->km_tanggal->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_km_tanggal" class="tr_km_master_ar2add" type="text/html">
...
<script type="text/html" class="tr_km_master_ar2add_js">
ew_CreateDateTimePicker("ftr_km_master_ar2add", "x_km_tanggal", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php echo $tr_km_master_ar2->km_tanggal->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_km_tanggal">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_km_tanggal"><script id="tpc_tr_km_master_ar2_km_tanggal" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->km_tanggal->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_customer_id" class="form-group">
		<label id="elh_tr_km_master_ar2_customer_id" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_customer_id" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->customer_id->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->customer_id->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_customer_id" class="tr_km_master_ar2add" type="text/html">
...
<script type="text/html" class="tr_km_master_ar2add_js">
ftr_km_master_ar2add.CreateAutoSuggest({"id":"x_customer_id","forceSelect":true});
</script>
<?php echo $tr_km_master_ar2->customer_id->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_customer_id">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_customer_id"><script id="tpc_tr_km_master_ar2_customer_id" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->customer_id->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_customer_name" class="form-group">
		<label id="elh_tr_km_master_ar2_customer_name" for="x_customer_name" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_customer_name" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->customer_name->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->customer_name->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_customer_name" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_customer_name">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_customer_name"><script id="tpc_tr_km_master_ar2_customer_name" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->customer_name->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_km_type" class="form-group">
		<label id="elh_tr_km_master_ar2_km_type" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_km_type" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->km_type->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->km_type->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_km_type" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_km_type">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_km_type"><script id="tpc_tr_km_master_ar2_km_type" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->km_type->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_km_acc" class="form-group">
		<label id="elh_tr_km_master_ar2_km_acc" for="x_km_acc" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_km_acc" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->km_acc->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->km_acc->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_km_acc" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_km_acc">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_km_acc"><script id="tpc_tr_km_master_ar2_km_acc" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->km_acc->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_cek_no" class="form-group">
		<label id="elh_tr_km_master_ar2_cek_no" for="x_cek_no" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_cek_no" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->cek_no->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->cek_no->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_cek_no" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_cek_no">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_cek_no"><script id="tpc_tr_km_master_ar2_cek_no" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->cek_no->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_tgl_jt" class="form-group">
		<label id="elh_tr_km_master_ar2_tgl_jt" for="x_tgl_jt" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_tgl_jt" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->tgl_jt->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->tgl_jt->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_tgl_jt" class="tr_km_master_ar2add" type="text/html">
...
<script type="text/html" class="tr_km_master_ar2add_js">
ew_CreateDateTimePicker("ftr_km_master_ar2add", "x_tgl_jt", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php echo $tr_km_master_ar2->tgl_jt->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_tgl_jt">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_tgl_jt"><script id="tpc_tr_km_master_ar2_tgl_jt" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->tgl_jt->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_cek_amt" class="form-group">
		<label id="elh_tr_km_master_ar2_cek_amt" for="x_cek_amt" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_cek_amt" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->cek_amt->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->cek_amt->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_cek_amt" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_cek_amt">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_cek_amt"><script id="tpc_tr_km_master_ar2_cek_amt" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->cek_amt->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_ret_number1" class="form-group">
		<label id="elh_tr_km_master_ar2_ret_number1" for="x_ret_number1" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_ret_number1" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->ret_number1->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->ret_number1->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_ret_number1" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_ret_number1">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_ret_number1"><script id="tpc_tr_km_master_ar2_ret_number1" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->ret_number1->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_ret_date1" class="form-group">
		<label id="elh_tr_km_master_ar2_ret_date1" for="x_ret_date1" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_ret_date1" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->ret_date1->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->ret_date1->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_ret_date1" class="tr_km_master_ar2add" type="text/html">
...
<script type="text/html" class="tr_km_master_ar2add_js">
ew_CreateDateTimePicker("ftr_km_master_ar2add", "x_ret_date1", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php echo $tr_km_master_ar2->ret_date1->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_ret_date1">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_ret_date1"><script id="tpc_tr_km_master_ar2_ret_date1" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->ret_date1->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_retur_amt1" class="form-group">
		<label id="elh_tr_km_master_ar2_retur_amt1" for="x_retur_amt1" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_retur_amt1" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->retur_amt1->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->retur_amt1->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_retur_amt1" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_retur_amt1">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_retur_amt1"><script id="tpc_tr_km_master_ar2_retur_amt1" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->retur_amt1->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_ret_number2" class="form-group">
		<label id="elh_tr_km_master_ar2_ret_number2" for="x_ret_number2" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_ret_number2" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->ret_number2->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->ret_number2->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_ret_number2" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_ret_number2">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_ret_number2"><script id="tpc_tr_km_master_ar2_ret_number2" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->ret_number2->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_ret_date2" class="form-group">
		<label id="elh_tr_km_master_ar2_ret_date2" for="x_ret_date2" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_ret_date2" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->ret_date2->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->ret_date2->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_ret_date2" class="tr_km_master_ar2add" type="text/html">
...
<script type="text/html" class="tr_km_master_ar2add_js">
ew_CreateDateTimePicker("ftr_km_master_ar2add", "x_ret_date2", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php echo $tr_km_master_ar2->ret_date2->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_ret_date2">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_ret_date2"><script id="tpc_tr_km_master_ar2_ret_date2" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->ret_date2->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_retur_amt2" class="form-group">
		<label id="elh_tr_km_master_ar2_retur_amt2" for="x_retur_amt2" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_retur_amt2" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->retur_amt2->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->retur_amt2->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_retur_amt2" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_retur_amt2">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_retur_amt2"><script id="tpc_tr_km_master_ar2_retur_amt2" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->retur_amt2->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_ret_number3" class="form-group">
		<label id="elh_tr_km_master_ar2_ret_number3" for="x_ret_number3" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_ret_number3" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->ret_number3->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->ret_number3->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_ret_number3" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_ret_number3">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_ret_number3"><script id="tpc_tr_km_master_ar2_ret_number3" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->ret_number3->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_ret_date3" class="form-group">
		<label id="elh_tr_km_master_ar2_ret_date3" for="x_ret_date3" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_ret_date3" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->ret_date3->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->ret_date3->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_ret_date3" class="tr_km_master_ar2add" type="text/html">
...
<script type="text/html" class="tr_km_master_ar2add_js">
ew_CreateDateTimePicker("ftr_km_master_ar2add", "x_ret_date3", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php echo $tr_km_master_ar2->ret_date3->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_ret_date3">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_ret_date3"><script id="tpc_tr_km_master_ar2_ret_date3" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->ret_date3->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_retur_amt3" class="form-group">
		<label id="elh_tr_km_master_ar2_retur_amt3" for="x_retur_amt3" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_retur_amt3" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->retur_amt3->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->retur_amt3->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_retur_amt3" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_retur_amt3">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_retur_amt3"><script id="tpc_tr_km_master_ar2_retur_amt3" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->retur_amt3->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_tunai_amt" class="form-group">
		<label id="elh_tr_km_master_ar2_tunai_amt" for="x_tunai_amt" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_tunai_amt" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->tunai_amt->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->tunai_amt->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_tunai_amt" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_tunai_amt">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_tunai_amt"><script id="tpc_tr_km_master_ar2_tunai_amt" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->tunai_amt->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_dp_amt" class="form-group">
		<label id="elh_tr_km_master_ar2_dp_amt" for="x_dp_amt" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_dp_amt" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->dp_amt->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->dp_amt->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_dp_amt" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_dp_amt">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_dp_amt"><script id="tpc_tr_km_master_ar2_dp_amt" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->dp_amt->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_km_amt" class="form-group">
		<label id="elh_tr_km_master_ar2_km_amt" for="x_km_amt" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_km_amt" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->km_amt->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->km_amt->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_km_amt" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_km_amt">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_km_amt"><script id="tpc_tr_km_master_ar2_km_amt" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->km_amt->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_km_notes" class="form-group">
		<label id="elh_tr_km_master_ar2_km_notes" for="x_km_notes" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_km_notes" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->km_notes->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->km_notes->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_km_notes" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_km_notes">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_km_notes"><script id="tpc_tr_km_master_ar2_km_notes" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->km_notes->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_kas_amt" class="form-group">
		<label id="elh_tr_km_master_ar2_kas_amt" for="x_kas_amt" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_kas_amt" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->kas_amt->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->kas_amt->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_kas_amt" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_kas_amt">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_kas_amt"><script id="tpc_tr_km_master_ar2_kas_amt" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->kas_amt->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_kode_depo" class="form-group">
		<label id="elh_tr_km_master_ar2_kode_depo" for="x_kode_depo" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_kode_depo" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->kode_depo->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->kode_depo->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_kode_depo" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_kode_depo">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_kode_depo"><script id="tpc_tr_km_master_ar2_kode_depo" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->kode_depo->FldCaption() ?></span></script></span></td>
...
<?php if ($tr_km_master_ar2_add->IsMobileOrModal) { ?>
	<div id="r_sales_id" class="form-group">
		<label id="elh_tr_km_master_ar2_sales_id" for="x_sales_id" class="<?php echo $tr_km_master_ar2_add->LeftColumnClass ?>"><script id="tpc_tr_km_master_ar2_sales_id" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->sales_id->FldCaption() ?></span></script></label>
		<div class="<?php echo $tr_km_master_ar2_add->RightColumnClass ?>"><div<?php echo $tr_km_master_ar2->sales_id->CellAttributes() ?>>
<script id="tpx_tr_km_master_ar2_sales_id" class="tr_km_master_ar2add" type="text/html">
...
<?php } else { ?>
	<tr id="r_sales_id">
		<td class="col-sm-2"><span id="elh_tr_km_master_ar2_sales_id"><script id="tpc_tr_km_master_ar2_sales_id" class="tr_km_master_ar2add" type="text/html"><span><?php echo $tr_km_master_ar2->sales_id->FldCaption() ?></span></script></span></td>
...
<?php } ?>
<?php
	if (in_array("tr_km_item_ar2", explode(",", $tr_km_master_ar2->getCurrentDetailTable())) && $tr_km_item_ar2->DetailAdd) {
?>
...
<?php } ?>
<?php if (!$tr_km_master_ar2_add->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $tr_km_master_ar2_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tr_km_master_ar2_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
...
ew_ApplyTemplate("tpd_tr_km_master_ar2add", "tpm_tr_km_master_ar2add", "tr_km_master_ar2add", "<?php echo $tr_km_master_ar2->CustomExport ?>", ewVar.templateData.rows[0]);
jQuery("script.tr_km_master_ar2add_js").each(function(){ew_AddScript(this.text);});
</script>
<script type="text/javascript">
ftr_km_master_ar2add.Init();
</script>
<?php
$tr_km_master_ar2_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tr_km_master_ar2_add->Page_Terminate();
?>
